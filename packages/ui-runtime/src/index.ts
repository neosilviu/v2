...
export function removeZone(state: ShellState, zoneId: string, fallbackZoneId: string): ShellState {
  if (zoneId === fallbackZoneId || !state.zones.some((zone) => zone.id === fallbackZoneId)) return state;
  const placements = state.placements.map((item) => item.zoneId === zoneId ? { ...item, zoneId: fallbackZoneId } : item);
  return { ...state, zones: state.zones.filter((zone) => zone.id !== zoneId), placements };
}
...
export function createShellFromPlugins(zones: ZoneContribution[], plugins: PluginManifest[]): ShellState {
  ...
  const state = { zones, surfaces, layouts, placements: [] as SurfacePlacement[] };
  return surfaces.reduce((next, surface, order) => mountSurface(next, { surfaceId: surface.id, zoneId: surface.zone, order }), state);
}
