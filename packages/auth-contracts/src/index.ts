...
export const authRegistrationModeSchema = z.enum([
  "disabled",
  "open",
  "invitation-only",
  "admin-created",
...
export const authUiContributionStatusSchema = z.enum([
  "draft",
  "published",
  "unpublished",
]);
...
export const authPublicLoginConfigSchema = z.object({
  ...
  methods: z.array(
    authMethodSchema
      .omit({ configurationRef: true })
      .extend({ status: z.literal("enabled"), publicVisible: z.literal(true) }),
  ),
  uiContributions: z.array(
    authUiContributionSchema.extend({ status: z.literal("published") }),
  ),
  features: z.object({
    password: z.boolean(),
    passkey: z.boolean(),
...
export const authPolicyWriteSchema = authPolicySchema
  .omit({
    id: true,
    createdAt: true,
    updatedAt: true,
  ...
  .extend({ workspaceId: z.string().min(1).nullable().optional() });
...
export const ownerSetupSignupRequestSchema = z.object({
...
export type AuthPublicLoginConfig = z.output<
  typeof authPublicLoginConfigSchema
>;
...
export type AuthSignInEmailRequest = z.output<
  typeof authSignInEmailRequestSchema
>;
export type AuthUpdateUserRequest = z.output<
  typeof authUpdateUserRequestSchema
>;
export type AuthProfile = z.output<typeof authProfileSchema>["profile"];
export type OwnerSetupSignupRequest = z.output<
  typeof ownerSetupSignupRequestSchema
>;
