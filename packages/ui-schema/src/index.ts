...
export const publicRoutePatternSchema = z
  .string()
  .regex(
...
export const actionDefinitionSchema = z.object({
  ...
  intent: z
    .enum(["navigate", "submit", "execute", "approve", "deny"])
    .default("execute"),
  variant: z.enum(["default", "primary", "danger"]).default("default"),
  access: accessModeSchema.default("private"),
  risk: z
    .enum(["safe", "reversible", "sensitive", "dangerous"])
    .default("safe"),
...
export const runtimeResultEnvelopeSchema = z.discriminatedUnion("status", [
  z.object({
    ...
    auditEventId: z.string().nullable().default(null),
  }),
  z.object({
...
export type DeclarativePageContribution = z.output<
  typeof declarativePageContributionSchema
>;
...
export type PlatformSettingsTabId = z.output<
  typeof platformSettingsTabIdSchema
>;
...
export type RuntimeResultEnvelope = z.output<
  typeof runtimeResultEnvelopeSchema
>;
...
export const ui = {
  ...
  submit(
    ...
    return actionDefinitionSchema.parse({
      ...
      placement: "form",
      ...(permission
        ? { access: "permission-gated", requiredPermission: permission }
        : {}),
      effects: [
        { type: "toast", message: `${title} successful` },
        { type: "refresh" },
      ],
    });
  },
  rowAction(
    commandId: string,
    title: string,
    options: Partial<z.input<typeof actionDefinitionSchema>> = {},
  ): ActionDefinition {
    return actionDefinitionSchema.parse({
      id: commandId,
      commandId,
  ...
  form(
    value: Omit<z.input<typeof settingsSectionSchema>, "kind">,
  ): SettingsSection {
  ...
  crud(
    value: Omit<z.input<typeof settingsSectionSchema>, "kind" | "crud"> & {
      entity: {
        singular: string;
        plural?: string;
        rowId?: string;
  ...
  panel(value: {
      ...
      slots: [
        {
          ...
          blocks: [
            {
              type: "text",
              text: `Manage ${value.label.toLowerCase()} for this workspace.`,
              tone: "muted",
            },
          ],
        },
    ...
    return {
      tab: {
        ...tab,
        ownerName: pluginId === "platform" ? "Platform" : pluginId,
