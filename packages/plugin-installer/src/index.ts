...
export async function unpackPluginZip(
  bytes: ArrayBuffer,
  objectKey: string,
  ...
  if (!definition)
    throw new Error(
      "ZIP plugin package must include plugin.json at the archive root",
    );
  const descriptor = pluginPackageDescriptorSchema.parse(
    JSON.parse(strFromU8(definition)),
  );
...
export function extractDeclaredHtmlAsset(
  bytes: ArrayBuffer,
  entry: string,
): string | undefined {
