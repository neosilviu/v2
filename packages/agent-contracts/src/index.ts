...
export const providerBindingSchema = z.object({
  ...
  status: z.enum(["configured", "unavailable", "disabled"]),
  createdAt: z.string(),
});
...
export const setChannelProviderRequestSchema = z.object({
  providerId: z.string().nullable(),
});
...
export const providerDiscoveryRequestSchema = z.object({
  workspaceId: z.string().min(1),
  connectionId: z.string().min(1),
});
export const providerTestRequestSchema = providerDiscoveryRequestSchema.extend({
...
export const createToolCallRequestSchema = z.object({
  workspaceId: z.string().min(1),
  channelId: z.string().min(1),
...
export const refreshToolCallRequestSchema = z.object({
  workspaceId: z.string().min(1),
  approvalId: z.string().min(1).optional(),
});
