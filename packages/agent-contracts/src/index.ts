...
export const providerBindingSchema = z.object({ id: z.string().min(1), workspaceId: z.string().min(1), contributionId: z.string().min(1), title: z.string().min(1), model: z.string().min(1), status: z.enum(["configured", "missing-secret", "disabled"]), createdAt: z.string() });
...
export const createProviderBindingRequestSchema = z.object({ workspaceId: z.string().min(1), contributionId: z.string().min(1), title: z.string().min(1), model: z.string().min(1) });
export const providerCredentialsSchema = z.record(z.string(), z.string());
export const providerDiscoveryRequestSchema = z.object({ workspaceId: z.string().min(1), credentials: providerCredentialsSchema.default({}) });
export const providerTestRequestSchema = providerDiscoveryRequestSchema.extend({ modelId: z.string().optional() });
