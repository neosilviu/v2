...
export type CloudflareClientConfig = { accountId: string; apiToken: string; baseUrl?: string };
export type CloudflareEnvelope<T> = { success: boolean; result?: T; errors?: Array<{ code: number | string; message: string }> };
...
export class CloudflarePlatformClient {
  ...
  async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok || !body?.success) {
      const message = body?.errors?.[0]?.message ?? `Cloudflare API request failed with HTTP ${response.status}.`;
      throw new CloudflarePlatformError(message, response.status, "cloudflare_api_error", response.status >= 500 || response.status === 429);
    }
