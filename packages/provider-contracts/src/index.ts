...
export const createWorkersAiConnectionSchema = z.object({
  workspaceId: z.string().min(1),
  title: z.string().trim().min(1),
  modelId: z.string().trim().min(1).default("@cf/meta/llama-3.1-8b-instruct"),
});
...
export const providerChatRequestSchema = z.object({
  modelId: z.string().min(1).optional(),
  messages: z.array(providerChatMessageSchema).min(1).max(100),
});
...
export type CreateWorkersAiConnection = z.output<
  typeof createWorkersAiConnectionSchema
>;
