...
export const toolSchema = z.object({
  id: z.string().min(1), title: z.string(), description: z.string().optional(),
  permissions: z.array(z.string()).default([]), risk: riskSchema.default("safe"),
...
export const settingSchema = z.object({
  id: z.string().min(1), title: z.string(), section: z.string(),
  fields: z.array(z.object({ key: z.string(), label: z.string(), type: z.enum(["string", "boolean", "number", "color", "select"]), options: z.array(z.string()).optional() })).default([]),
});

export const pluginManifestSchema = z.object({
  id: z.string().min(1), name: z.string().min(1), version: z.string().min(1), builtIn: z.boolean().default(false),
  data: storageModeSchema.default({ mode: "none" }), capabilities: z.array(capabilitySchema).default([]),
  contributes: z.object({
    tools: z.array(toolSchema).default([]), providers: z.array(providerSchema).default([]), channels: z.array(channelSchema).default([]),
    surfaces: z.array(surfaceSchema).default([]), zones: z.array(zoneSchema).default([]), layouts: z.array(layoutSchema).default([]), settings: z.array(settingSchema).default([]),
  }).default({}),
});

export const pluginBundleSchema = z.object({
  manifest: pluginManifestSchema,
  package: z.object({ format: z.literal("zip"), sha256: z.string().min(32), sizeBytes: z.number().int().positive(), objectKey: z.string().min(1) }),
