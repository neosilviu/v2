...
export const providerSecretFieldSchema = z.object({
  id: z.string().min(1),
  title: z.string(),
...
export const publicContributionAccessSchema = z.enum([
  "anonymous",
  "authenticated",
]);
export const publicRouteContributionSchema = z.object({
  ...
  access: publicContributionAccessSchema.default("anonymous"),
});
export const publicSurfaceContributionSchema = z.object({
  id: z.string().min(1),
  title: z.string(),
...
export const publicToolContributionSchema = z.object({
  id: z.string().min(1),
  title: z.string(),
  toolId: z.string().min(1),
  path: publicRoutePatternSchema,
  access: publicContributionAccessSchema.default("authenticated"),
});
export type PluginValueContract =
  ...
  | {
      type: "object";
      properties: Record<string, PluginValueContract>;
      required?: string[];
      additionalProperties?: boolean;
    };
const pluginValueContractSchemaInternal: z.ZodTypeAny = z.lazy(() =>
  z.union([
    ...
    z.object({
      ...
      items: pluginValueContractSchemaInternal,
    }),
    z.object({
...
export const pluginValueContractSchema =
  pluginValueContractSchemaInternal as z.ZodType<PluginValueContract>;
...
export const declarativeUiSchema = z.object({
  body: z.array(declarativeUiBlockSchema).default([]),
});
export const runtimeDeclarativeRendererSchema = z.union([
  declarativePageContributionSchema,
  declarativeUiSchema,
]);
export const surfaceRendererSchema = z.discriminatedUnion("mode", [
  z.object({
    ...
    schema: runtimeDeclarativeRendererSchema.optional(),
  }),
  z.object({
    mode: z.literal("sandbox-frame"),
    entry: z.string().regex(/^ui\/[a-zA-Z0-9/_-]+\.html$/),
  }),
]);
export const surfaceSchema = z.object({
  ...
  renderer: surfaceRendererSchema.default({ mode: "declarative" }),
});
  ...
  fields: z
    .array(
      z.object({
        key: z.string(),
        label: z.string(),
        type: z.enum(["string", "boolean", "number", "color", "select"]),
...
export const pluginManifestSchema = z.object({
  ...
  contributes: z
    .object({
      ...
      settingsTabs: z.array(settingsTabContributionSchema).default([]),
      settingsPanels: z.array(settingsPanelContributionSchema).default([]),
      publicRoutes: z.array(publicRouteContributionSchema).default([]),
      publicSurfaces: z.array(publicSurfaceContributionSchema).default([]),
      publicTools: z.array(publicToolContributionSchema).default([]),
    })
    .default(emptyContributions),
...
export const pluginPackageDescriptorSchema = z.object({
  ...
  ui: z
    .object({
      mode: z.enum(["declarative", "sandbox-frame"]).default("declarative"),
      entry: z.string().optional(),
    })
    .default({ mode: "declarative" }),
});
export const pluginBundleSchema = pluginPackageDescriptorSchema.extend({
  package: z.object({
...
export type SettingsTabContribution = z.output<
  typeof settingsTabContributionSchema
>;
export type SettingsPanelContribution = z.output<
  typeof settingsPanelContributionSchema
>;
export type PublicContributionAccess = z.output<
  typeof publicContributionAccessSchema
>;
export type PublicRouteContribution = z.output<
  typeof publicRouteContributionSchema
>;
export type PublicSurfaceContribution = z.output<
  typeof publicSurfaceContributionSchema
>;
export type PublicToolContribution = z.output<
  typeof publicToolContributionSchema
>;
...
export type RuntimeDeclarativeRenderer = z.output<
  typeof runtimeDeclarativeRendererSchema
>;
