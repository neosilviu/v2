...
export const localNodeManifest = definePlugin({
  ...
  capabilities: [
    { id: "localnode.read", description: "Read local runner health and module status", risk: "safe" },
  ...
  contributes: {
    ...
    settingsPanels: [{
      ...
      schema: {
        ...
        fields: [
          ...
          { id: "pairingState", label: "Pairing state", type: "text", readOnly: true },
        ...
        actions: [
          { id: "local-node.health", title: "Check health", commandId: "localnode.health", intent: "execute", variant: "primary", access: "permission-gated" },
          { id: "local-node.save", title: "Save runner config", commandId: "localnode.saveConfig", intent: "submit", variant: "primary", access: "permission-gated", risk: "sensitive" }
        ],
    ...
    surfaces: [{ id: "local-node.production-status", title: "Local Production", zone: "workspace.main", kind: "page", renderer: { mode: "declarative", schema: {
      ...
      slots: [{ id: "local-node.production.header", slot: "header", blocks: [{ type: "text", text: "Operational state is explicit: configured, not-configured, mock-development-only or unavailable.", tone: "muted" }] }]
    } } }],
    tools: [
      { id: "localnode.health", title: "Check Local Node health", permissions: ["localnode.read"], risk: "safe", exposure: ["agent-ai", "mcp", "command"] },
      { id: "localnode.saveConfig", title: "Save Local Node runner config", permissions: ["localnode.configure"], risk: "sensitive", exposure: ["agent-ai", "mcp", "command"] },
      { id: "localnode.executeCommand", title: "Execute Local Node command", permissions: ["localnode.execute"], risk: "dangerous", exposure: ["agent-ai", "mcp"] }
    ]
  }
});
