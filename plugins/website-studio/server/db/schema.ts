...
export const websitePages = sqliteTable("website_pages", {
  ...
  uniqueIndex("website_pages_workspace_slug_idx").on(table.workspaceId, table.slug),
  index("website_pages_workspace_status_idx").on(table.workspaceId, table.status),
]);

export const websiteSections = sqliteTable("website_sections", {
  id: text("id").primaryKey(),
  pageId: text("page_id").notNull().references(() => websitePages.id, { onDelete: "cascade" }),
  kind: text("kind").notNull(),
  sortOrder: integer("sort_order").notNull().default(0),
  contentJson: text("content_json").notNull().default("{}"),
  aiContextEnabled: integer("ai_context_enabled", { mode: "boolean" }).notNull().default(false),
  createdAt: text("created_at").notNull().default(now),
  updatedAt: text("updated_at").notNull().default(now),
