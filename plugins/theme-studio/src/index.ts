...
export const themeStudioPlugin = definePlugin({
  ...
  contributes: {
    surfaces: [{ id: "theme-studio.appearance-settings", title: "Appearance", zone: "settings.appearance", kind: "settings", renderer: { mode: "declarative", schema: { body: [
      { type: "text", text: "Workspace appearance controls are rendered from manifest schema.", tone: "muted" },
      { type: "metric", label: "Runtime UI", value: "Declarative", detail: "No Web rebuild is required for this surface shape." },
      { type: "action", label: "Preview theme", commandId: "theme.preview", variant: "primary" }
    ] } } }],
