...
function MessageList({ messages }: { messages: AgentMessage[] }) {
  if (!messages.length)
    return <p className="message">Start a conversation in this channel.</p>;
  return (
...
function ToolCallList({
  ...
  if (!toolCalls.length)
    return <p className="message">No tool calls in this conversation.</p>;
  return (
    <div className="plugin-list">
      {toolCalls.map((toolCall) => (
        <div key={toolCall.id} className="plugin-row">
          <div>
            <strong>{toolCall.toolId}</strong>
            <small>
              {toolCall.approvalId
                ? `approval ${toolCall.approvalId.slice(0, 8)}`
                : `run ${toolCall.runId.slice(0, 8)}`}
            </small>
...
function ToolCallComposer({
  disabled,
  onSubmit,
}: {
  disabled: boolean;
  onSubmit: (toolId: string, input: unknown) => Promise<void>;
}) {
  ...
  return (
    <form className="plugin-list" onSubmit={submit}>
      <div className="surface-header">
        <div>
          <small>development trigger</small>
          <h2>Manual tool request</h2>
        </div>
        <Badge>test</Badge>
      </div>
      ...
      <Button type="submit" disabled={disabled || !toolId.trim()}>
        Request test tool
      </Button>
    </form>
  );
...
export function AgentSurface() {
  ...
  async function reloadConversation(nextChannelId: string) {
    const [nextMessages, nextToolCalls] = await Promise.all([
      loadMessages(nextChannelId),
      loadToolCalls(nextChannelId),
    ]);
  ...
  async function reload() {
    const [nextChannels, nextProviders, nextConnections] = await Promise.all([
      loadChannels(),
      loadProviderBindings(),
      loadAvailableConnections(),
    ]);
    ...
    setStatus(
      nextConnections.length
        ? "Ready"
        : "Configure Workers AI in Integrations to start chatting.",
    );
  }

  useEffect(() => {
    void reload().catch(() => setStatus("Agent AI service unavailable."));
  }, []);
  useEffect(() => {
    if (channelId)
      void reloadConversation(channelId).catch(() =>
  ...
  async function requestTool(toolId: string, input: unknown) {
    ...
    try {
      ...
      setToolCalls((current) => [
        ...current.filter((item) => item.id !== toolCall.id),
        toolCall,
      ]);
      setStatus(
        toolCall.status === "approval-required"
          ? "Tool call is pending Core approval."
          : "Tool call updated.",
      );
    } finally {
      setBusy(false);
  ...
  async function refreshTool(id: string) {
    ...
    try {
      ...
      setToolCalls((current) =>
        current.map((item) => (item.id === id ? toolCall : item)),
      );
      await reloadConversation(channel.id);
      setStatus(
  ...
  return (
    <SurfaceCard className="agent-surface chat">
      <div className="surface-header">
        <div>
          <small>assistant</small>
          <h2>Agent AI</h2>
        </div>
        <Badge>{channel?.title ?? "General"}</Badge>
      </div>
      <label className="provider-row">
        <select
          value={channel?.providerId ?? ""}
      ...
      <form className="composer" onSubmit={submitMessage}>
        <input
          ...
          disabled={!channel || busy}
        />
        <Button type="submit" disabled={!channel?.providerId || busy}>
          {busy ? "Working..." : "Send"}
        </Button>
      </form>
      <div className="surface-header">
        <h2>Tool calls</h2>
        <Badge>{toolCalls.length}</Badge>
      </div>
      <ToolCallList
        toolCalls={toolCalls}
        busy={busy}
        onRefresh={(id) => void refreshTool(id)}
      />
