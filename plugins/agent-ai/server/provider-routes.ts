...
export function createProviderRoutes() {
  ...
  function delegatedHeaders(headers: Headers) { const forwarded = new Headers(); const cookie = headers.get("cookie"); const authorization = headers.get("authorization"); if (cookie) forwarded.set("cookie", cookie); if (authorization) forwarded.set("authorization", authorization); return forwarded; }
  async function available(env: AgentAiEnv, workspaceId: string, providerId: string, headers: Headers) { const response = await env.CORE.fetch(`https://core.internal/runtime/providers?workspaceId=${encodeURIComponent(workspaceId)}`, { headers: delegatedHeaders(headers) }); if (!response.ok) return false; const payload = await response.json() as { providers: ProviderContribution[] }; return payload.providers.some((provider) => provider.id === providerId); }
  async function executeProviderTool(env: AgentAiEnv, workspaceId: string, toolId: string, input: unknown, headers: Headers) {
    const response = await env.CORE.fetch("https://core.internal/tools/execute", { method: "POST", headers: { ...Object.fromEntries(delegatedHeaders(headers)), "content-type": "application/json" }, body: JSON.stringify({ workspaceId, toolId, input }) });
  ...
  routes.get("/connections", async (c) => {
    ...
    const result = await executeProviderTool(c.env, workspaceId, "providers.listConnections", {}, c.req.raw.headers);
