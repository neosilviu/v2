...
import {
  providerConnectionOperationSchema,
  providerConnectionTestSchema,
} from "@v2/agent-contracts/provider-operations";
...
export function createProviderRoutes() {
  ...
  async function available(
    ...
    const response = await env.CORE.fetch(
      `https://core.internal/runtime/providers?workspaceId=${encodeURIComponent(workspaceId)}`,
  ...
  async function executeProviderTool(
    ...
    const response = await env.CORE.fetch(
      "https://core.internal/tools/execute",
      {
        method: "POST",
        headers: {
          ...Object.fromEntries(delegatedHeaders(headers)),
          "content-type": "application/json",
        },
        body: JSON.stringify({ workspaceId, toolId, input }),
      },
    );
      ...
      .catch(() =>
        errorResponse(
          failure(
            ...
            "Provider runtime returned an invalid response.",
            { retryable: true },
          ),
        ),
      );
    return {
      ok: response.ok,
      payload: payload as { status?: string; result?: unknown },
    };
  }
    ...
    if (!result.ok || result.payload.status !== "executed")
      return c.json(
        errorResponse(
          failure(
            "dependency_unavailable",
            "Provider connections are not available.",
          ),
        ),
        502,
      );
    return c.json(
      result.payload.result as { connections: ProviderConnection[] },
    );
  });
  routes.post("/:providerId/detect-models", async (c) => {
    ...
    if (
      ...
      return c.json(
        errorResponse(
          failure("not_found", "Provider is not available in this workspace."),
        ),
        404,
      );
    const result = await executeProviderTool(
      c.env,
      request.workspaceId,
      "providers.detectModels",
      { connectionId: request.connectionId },
      c.req.raw.headers,
    );
    return result.ok && result.payload.status === "executed"
      ? c.json(result.payload.result)
      : c.json(
          errorResponse(
            failure(
              "dependency_unavailable",
              "Provider runtime rejected the operation.",
              { retryable: true },
            ),
          ),
          502,
  ...
  routes.post("/:providerId/test", async (c) => {
    ...
    const result = await executeProviderTool(
      ...
      { connectionId: request.connectionId, modelId: request.modelId },
      c.req.raw.headers,
    );
    return result.ok && result.payload.status === "executed"
      ? c.json(result.payload.result)
