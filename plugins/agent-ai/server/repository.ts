...
export class AgentRepository {
  ...
  async listChannels(workspaceId: string): Promise<AgentChannel[]> {
    ...
    const rows = await this.db.prepare("SELECT id, workspace_id, title, provider_binding_id, created_at FROM agent_channels WHERE workspace_id = ? ORDER BY created_at")
      .bind(workspaceId).all<{ id: string; workspace_id: string; title: string; provider_binding_id: string | null; created_at: string }>();
    return rows.results.map((row) => ({ id: row.id, workspaceId: row.workspace_id, title: row.title, providerId: row.provider_binding_id, createdAt: row.created_at }));
  }
  ...
  async createChannel(workspaceId: string, title: string, providerId: string | null): Promise<AgentChannel> {
    ...
    await this.db.prepare("INSERT INTO agent_channels (id, workspace_id, title, provider_binding_id) VALUES (?, ?, ?, ?)")
  ...
  async createRun(channelId: string, providerId: string | null): Promise<AgentRun> {
    ...
    await this.db.prepare("INSERT INTO agent_runs (id, channel_id, provider_binding_id, status) VALUES (?, ?, ?, ?)").bind(id, channelId, providerId, status).run();
