...
export const agentMessages = sqliteTable(
  ...
  (table) => [
    index("agent_messages_channel_idx").on(table.channelId, table.createdAt),
  ],
);
...
export const agentProviderBindings = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("agent_provider_binding_name_idx").on(
      table.workspaceId,
      table.title,
    ),
  ],
...
export const agentRuns = sqliteTable(
  "agent_runs",
  {
    ...
    channelId: text("channel_id")
      .notNull()
      .references(() => agentChannels.id, { onDelete: "cascade" }),
    providerBindingId: text("provider_binding_id").references(
      () => agentProviderBindings.id,
    ),
    status: text("status", {
      enum: ["queued", "running", "completed", "failed", "provider-required"],
    }).notNull(),
    inputMessageId: text("input_message_id").references(() => agentMessages.id),
    outputMessageId: text("output_message_id").references(
      () => agentMessages.id,
    ),
    error: text("error"),
    createdAt: text("created_at").notNull().default(now),
  ...
  (table) => [
    index("agent_runs_channel_idx").on(table.channelId, table.createdAt),
  ],
);

export const agentKnowledgeSources = sqliteTable(
  "agent_knowledge_sources",
  {
    ...
    title: text("title").notNull(),
    kind: text("kind", {
      enum: ["cloudflare-ai-search", "vectorize", "page-content", "manual"],
    }).notNull(),
    status: text("status", { enum: ["active", "disabled", "syncing", "error"] })
      .notNull()
      .default("active"),
  ...
  (table) => [
    index("agent_knowledge_sources_workspace_idx").on(table.workspaceId),
  ],
);
...
export const agentResourceBindings = sqliteTable(
  "agent_resource_bindings",
  {
    id: text("id").primaryKey(),
    workspaceId: text("workspace_id").notNull(),
    resourceType: text("resource_type", {
      enum: ["kv", "do", "r2", "vectorize", "ai-search"],
    }).notNull(),
  ...
  (table) => [
    uniqueIndex("agent_resource_binding_idx").on(
      table.workspaceId,
      table.resourceType,
      table.bindingName,
    ),
