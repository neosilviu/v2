...
function contextInput(input: unknown): ContextInput | undefined {
  ...
  const workspaceId =
    typeof value.workspaceId === "string" ? value.workspaceId.trim() : "";
  const surfaceId =
    typeof value.surfaceId === "string" ? value.surfaceId.trim() : "";
  ...
  const entityId =
    typeof value.entityId === "string" && value.entityId.trim()
      ...
      : undefined;
  return entityId
    ? { workspaceId, surfaceId, entityId }
...
export function createKnowledgeRoutes() {
  ...
  routes.post("/search", async (c) => {
    ...
    if (!input)
      return c.json(
        errorResponse(
          failure("validation_failed", "A valid knowledge query is required."),
        ),
        400,
      );
    if (!c.env.KNOWLEDGE_RUNTIME)
      return c.json(
        errorResponse(
          failure(
            "dependency_unavailable",
            "Knowledge search is not configured.",
          ),
        ),
        503,
      );
    const response = await c.env.KNOWLEDGE_RUNTIME.fetch(
      "https://knowledge.internal/search",
      {
        method: "POST",
    ...
    if (!response.ok)
      return c.json(
        errorResponse(
          failure("dependency_unavailable", "Knowledge search failed."),
        ),
        502,
      );
  ...
  routes.post("/page-context", async (c) => {
    ...
    if (!input)
      return c.json(
        errorResponse(
          failure(
            "validation_failed",
            "A valid page context request is required.",
          ),
        ),
        400,
      );
    if (!c.env.PAGE_CONTEXT)
      return c.json(
        errorResponse(
          failure("dependency_unavailable", "Page context is not configured."),
        ),
        503,
      );
    const response = await c.env.PAGE_CONTEXT.fetch(
      "https://context.internal/read",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(input),
    ...
    if (!response.ok)
      return c.json(
        errorResponse(
          failure("dependency_unavailable", "Page context request failed."),
        ),
        502,
      );
    return c.json(await response.json());
