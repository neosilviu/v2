...
async function getCoreApproval(
  c: AppContext,
  workspaceId: string,
  approvalId: string,
): Promise<ToolApproval | null> {
  const response = await c.env.CORE.fetch(
    `https://core.internal/tool-approvals/${encodeURIComponent(approvalId)}?workspaceId=${encodeURIComponent(workspaceId)}`,
    { headers: delegatedHeaders(c) },
  );
...
async function executeViaCore(
  ...
  const response = await c.env.CORE.fetch(
    "https://core.internal/tools/execute",
    {
      method: "POST",
      headers: {
        ...Object.fromEntries(delegatedHeaders(c)),
  ...
  if (result.status === "approval-required")
    await repository.markToolCallApprovalRequired(
      toolCall.id,
      result.approvalId,
    );
  if (result.status === "executed")
    await repository.completeToolCall(toolCall.id, result);
  if (result.status === "denied")
    await repository.markToolCallDenied(toolCall.id, result.reason, result);
  return (await repository.getToolCall(toolCall.id)) ?? toolCall;
}

function extractModelToolCall(
  content: string,
  ...
  try {
    ...
    const call = (payload.toolCall ??
      (Array.isArray(payload.tool_calls) ? payload.tool_calls[0] : null)) as {
      toolId?: unknown;
      tool_id?: unknown;
      input?: unknown;
      arguments?: unknown;
    ...
    const toolId =
      typeof call?.toolId === "string"
        ? call.toolId
        : typeof call?.tool_id === "string"
          ? call.tool_id
          : "";
    if (!toolId.trim()) return null;
    return {
      toolId: toolId.trim(),
      input: call?.input ?? call?.arguments ?? null,
    };
  } catch {
    return null;
  }
...
async function callProvider(
  c: AppContext,
  binding: NonNullable<Awaited<ReturnType<AgentRepository["getProvider"]>>>,
  messages: Awaited<ReturnType<typeof providerMessages>>,
) {
  if (!binding.connectionId)
    throw new Error("Provider connection is not configured for this channel.");
  const response = await c.env.CORE.fetch(
    "https://core.internal/tools/execute",
    {
      method: "POST",
  ...
  if (payload.status !== "executed")
    throw new Error("Provider execution was not completed.");
  return providerChatResultSchema.parse(payload.result);
}

async function continueRunAfterToolResult(
  c: AppContext,
  repository: AgentRepository,
  runId: string,
  binding: NonNullable<Awaited<ReturnType<AgentRepository["getProvider"]>>>,
  ...
  const result = await callProvider(
    c,
    binding,
    await providerMessages(repository, toolCall.channelId),
  );
...
async function refreshToolCall(
  ...
  if (
    toolCall.approvalId &&
    suppliedApprovalId &&
    toolCall.approvalId !== suppliedApprovalId
  ) {
    await repository.markToolCallDenied(
      toolCall.id,
...
app = app.get("/workspaces/:workspaceId/channels", async (c) =>
  c.json({
    channels: await new AgentRepository(c.env.AGENT_DB).listChannels(
      c.req.param("workspaceId"),
    ),
  }),
);
app = app.post("/channels", async (c) => {
  const input = createChannelRequestSchema.parse(await c.req.json());
  return c.json(
    {
      channel: await new AgentRepository(c.env.AGENT_DB).createChannel(
        input.workspaceId,
        input.title,
...
app = app.put("/channels/:channelId/provider", async (c) => {
  const input = setChannelProviderRequestSchema.parse(await c.req.json());
  await new AgentRepository(c.env.AGENT_DB).setChannelProvider(
    c.req.param("channelId"),
    input.providerId,
  );
  return c.json({ saved: true });
});
app = app.get("/workspaces/:workspaceId/providers", async (c) =>
  c.json({
    providers: await new AgentRepository(c.env.AGENT_DB).listProviders(
      c.req.param("workspaceId"),
    ),
  }),
);
app = app.post("/providers", async (c) => {
  const input = createProviderBindingRequestSchema.parse(await c.req.json());
  return c.json(
    {
      provider: await new AgentRepository(c.env.AGENT_DB).createProvider(
        input.workspaceId,
...
app = app.get("/channels/:channelId/messages", async (c) =>
  c.json({
    messages: await new AgentRepository(c.env.AGENT_DB).listMessages(
      c.req.param("channelId"),
    ),
  }),
);
app = app.post("/messages", async (c) => {
  const input = sendMessageRequestSchema.parse(await c.req.json());
  return c.json(
    {
      message: await new AgentRepository(c.env.AGENT_DB).addMessage(
        input.channelId,
        "user",
        input.content,
      ),
...
app = app.get("/channels/:channelId/tool-calls", async (c) =>
  c.json({
    toolCalls: await new AgentRepository(
      c.env.AGENT_DB,
    ).listToolCallsForChannel(c.req.param("channelId")),
  }),
);
app = app.get("/runs/:runId/tool-calls", async (c) =>
  c.json({
    toolCalls: await new AgentRepository(c.env.AGENT_DB).listToolCallsForRun(
      c.req.param("runId"),
    ),
  }),
);
app = app.post("/tool-calls", async (c) => {
  ...
  if (!channel || channel.workspaceId !== input.workspaceId)
    return c.json(
      errorResponse(failure("not_found", "Channel is not available.")),
      404,
    );
  const run = input.runId
    ? await repository.getRun(input.runId)
    : await repository.createToolRun(channel.id);
  if (!run || run.channelId !== channel.id)
    return c.json(
      errorResponse(
        failure("not_found", "Run is not available for this channel."),
      ),
      404,
    );
  ...
  try {
    const updated = await executeViaCore(
      c,
      repository,
      input.workspaceId,
      toolCall,
    ...
    return c.json(
      { toolCall: updated },
      updated.status === "approval-required" ? 202 : 201,
    );
  } catch {
...
app = app.post("/tool-calls/:toolCallId/refresh", async (c) => {
  ...
  if (!toolCall)
    return c.json(
      errorResponse(failure("not_found", "Tool call is not available.")),
      404,
    );
  try {
    const updated = await refreshToolCall(
      c,
      repository,
      input.workspaceId,
      toolCall,
    ...
    if (updated.status === "completed") {
      ...
      const binding = run?.providerId
        ? await repository.getProvider(run.providerId)
        : undefined;
      if (binding?.connectionId)
        continuation = await continueRunAfterToolResult(
          c,
          repository,
          updated.runId,
          binding,
    ...
    if (updated.status === "denied" || updated.status === "failed")
      await repository.failRun(
        updated.runId,
        updated.error ?? `Tool call ${updated.status}.`,
      );
...
app = app.post("/runs", async (c) => {
  const input = createRunRequestSchema.parse(await c.req.json());
  const repository = new AgentRepository(c.env.AGENT_DB);
  const channel = (await repository.listChannels(input.workspaceId)).find(
  ...
  if (!channel.providerId)
    return c.json(
      errorResponse(
        failure(
          "conflict",
          "Select a provider connection before starting a run.",
        ),
      ),
      409,
    );
  ...
  if (!binding?.connectionId)
    return c.json(
      errorResponse(
        failure(
          "conflict",
          "Provider connection is not configured for this channel.",
        ),
      ),
      409,
    );
  ...
  try {
    const result = await callProvider(
      c,
      binding,
      await providerMessages(repository, channel.id),
    );
    ...
    if (requestedTool) {
      ...
      if (updated.status === "completed") {
        const continuation = await continueRunAfterToolResult(
          c,
          repository,
          run.id,
          binding,
      ...
      if (updated.status === "approval-required") {
        const message = await repository.addMessage(
          channel.id,
          "assistant",
          `Tool approval required for ${updated.toolId}.`,
        );
        return c.json(
          {
