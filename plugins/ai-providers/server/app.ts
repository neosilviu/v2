...
app.use("*", cors({ origin: (origin, c) => allowedOrigins(c.env).includes(origin) ? origin : "", allowHeaders: ["Content-Type", "Authorization"], allowMethods: ["GET", "POST", "OPTIONS"], credentials: true, maxAge: 600 }));
app.use("/manage/*", async (c, next) => await readSession(c.env, c.req.raw.headers) ? next() : c.json(errorResponse(failure("not_authenticated", "Authentication is required.")), 401));
app.get("/health", (c) => c.json({ ok: true, service: "ai-providers", storageConfigured: Boolean(c.env.PROVIDERS_DB), workersAiConfigured: Boolean(c.env.AI) }));
app.get("/providers", (c) => c.json({ providers }));
app.get("/manage/connections", async (c) => { const repo = repository(c.env); if (!repo) return c.json(unavailable("Provider connection storage is not configured."), 503); return c.json({ connections: await repo.list(c.req.query("workspaceId") ?? "default") }); });
...
app.get("/connections", async (c) => { if (!isInternalRequest(c.req.raw)) return c.json(errorResponse(failure("not_authorized", "Provider runtime access is internal only.")), 403); const repo = repository(c.env); if (!repo) return c.json(unavailable("Provider connection storage is not configured."), 503); return c.json({ connections: await repo.list(c.req.query("workspaceId") ?? "default") }); });
app.use("/connections/*", async (c, next) => isInternalRequest(c.req.raw) ? next() : c.json(errorResponse(failure("not_authorized", "Provider operations require an internal runtime request.")), 403));
