...
type ProviderServiceEnv = ProviderRuntimeEnv & ProviderAccessEnv & { PROVIDERS_DB?: D1Database };
...
const providers = [...catalog.values()].map((provider) => ({ id: provider.id, title: provider.title, externalConfigurationRequired: provider.adapter !== "cloudflare-workers-ai" }));
const unavailable = (message: string) => errorResponse(failure("dependency_unavailable", message));
const repository = (env: ProviderServiceEnv) => env.PROVIDERS_DB ? new ProviderRepository(env.PROVIDERS_DB) : null;
async function resolved(env: ProviderServiceEnv, connectionId: string) { const repo = repository(env); if (!repo) return { error: unavailable("Provider connection storage is not configured."), status: 503 as const }; const connection = await repo.get(connectionId); if (!connection) return { error: errorResponse(failure("not_found", "Provider connection is not available.")), status: 404 as const }; if (connection.status === "disabled") return { error: errorResponse(failure("conflict", "Provider connection is disabled.")), status: 409 as const }; const provider = catalog.get(connection.providerId); if (!provider) return { error: errorResponse(failure("not_found", "Provider definition is not available.")), status: 404 as const }; return { repo, connection, provider }; }
app = app.use("*", cors({ origin: (origin, c) => allowedOrigins(c.env).includes(origin) ? origin : "", allowHeaders: ["Content-Type", "Authorization"], allowMethods: ["GET", "POST", "OPTIONS"], credentials: true, maxAge: 600 }));
app = app.use("/manage/*", async (c, next) => await readSession(c.env, c.req.raw.headers) ? next() : c.json(errorResponse(failure("not_authenticated", "Authentication is required.")), 401));
app = app.get("/health", (c) => c.json({ ok: true, service: "ai-providers", storageConfigured: Boolean(c.env.PROVIDERS_DB), workersAiConfigured: Boolean(c.env.AI) }));
app = app.get("/providers", (c) => c.json({ providers }));
app = app.get("/manage/connections", async (c) => { const repo = repository(c.env); if (!repo) return c.json(unavailable("Provider connection storage is not configured."), 503); return c.json({ connections: await repo.list(c.req.query("workspaceId") ?? "default") }); });
app = app.post("/manage/connections/workers-ai", async (c) => { const repo = repository(c.env); if (!repo) return c.json(unavailable("Provider connection storage is not configured."), 503); const input = createWorkersAiConnectionSchema.parse(await c.req.json()); return c.json({ connection: await repo.createWorkersAi(input.workspaceId, input.title, input.modelId, Boolean(c.env.AI)) }, 201); });
app = app.get("/connections", async (c) => { if (!isInternalRequest(c.req.raw)) return c.json(errorResponse(failure("not_authorized", "Provider runtime access is internal only.")), 403); const repo = repository(c.env); if (!repo) return c.json(unavailable("Provider connection storage is not configured."), 503); return c.json({ connections: await repo.list(c.req.query("workspaceId") ?? "default") }); });
app = app.use("/connections/*", async (c, next) => isInternalRequest(c.req.raw) ? next() : c.json(errorResponse(failure("not_authorized", "Provider operations require an internal runtime request.")), 403));
app = app.post("/connections/:connectionId/detect-models", async (c) => { const result = await resolved(c.env, c.req.param("connectionId")); if ("error" in result) return c.json(result.error, result.status); try { const models = await detectProviderModels(c.env, result.provider, {}); await result.repo.replaceModels(result.connection.id, models); return c.json({ models }); } catch { return c.json(unavailable("Provider model discovery failed."), 502); } });
app = app.post("/connections/:connectionId/test", async (c) => { const result = await resolved(c.env, c.req.param("connectionId")); if ("error" in result) return c.json(result.error, result.status); const input = await c.req.json<{ modelId?: string }>(); try { return c.json(await testProvider(c.env, result.provider, {}, input.modelId ?? result.connection.defaultModelId ?? undefined)); } catch { return c.json(unavailable("Provider connection test failed."), 502); } });
app = app.post("/connections/:connectionId/chat", async (c) => { const result = await resolved(c.env, c.req.param("connectionId")); if ("error" in result) return c.json(result.error, result.status); const input = providerChatRequestSchema.parse(await c.req.json()); const modelId = input.modelId ?? result.connection.defaultModelId; if (!modelId) return c.json(errorResponse(failure("conflict", "No chat model is configured for this connection.")), 409); try { return c.json(await invokeProviderChat(c.env, result.provider, modelId, input.messages)); } catch { return c.json(unavailable("Provider chat execution is not available for this connection."), 502); } });
