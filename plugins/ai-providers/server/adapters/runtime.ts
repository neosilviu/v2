...
const gateway = (env: ProviderRuntimeEnv, slug: string, direct: string, suffix: string) =>
...
const secret = (credentials: ProviderCredentials, key = "api_key") => {
  ...
  if (!value) throw new Error("Provider credential is missing.");
  return value;
};
...
function normalizeModels(provider: ProviderContribution, payload: unknown): ProviderModel[] {
  ...
  for (const row of rows) {
    ...
    if (!id || (provider.adapter === "gemini" && methods.length > 0 && !methods.includes("generateContent"))) continue;
    if (!models.has(id)) models.set(id, { id, title: String(item.displayName ?? item.label ?? id), capabilities: ["chat"] });
  }
...
export async function detectProviderModels(env: ProviderRuntimeEnv, provider: ProviderContribution, credentials: ProviderCredentials): Promise<ProviderModel[]> {
  switch (provider.adapter) {
    case "cloudflare-workers-ai": {
      ...
      const accountId = credentials.account_id?.trim() || env.CLOUDFLARE_ACCOUNT_ID;
      if (!token || !accountId) return fallback(provider);
      return parseModels(provider, await fetch(`https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(accountId)}/ai/models/search?per_page=100`, { headers: { authorization: `Bearer ${token}` } }));
    }
    case "openai":
      return parseModels(provider, await fetch(gateway(env, "openai", "https://api.openai.com/v1/models", "/models"), { headers: { authorization: `Bearer ${secret(credentials)}` } }));
    case "groq":
      return parseModels(provider, await fetch(gateway(env, "groq", "https://api.groq.com/openai/v1/models", "/openai/v1/models"), { headers: { authorization: `Bearer ${secret(credentials)}` } }));
    case "github-models":
      return parseModels(provider, await fetch(gateway(env, "github", "https://models.github.ai/catalog/models", "/catalog/models"), { headers: { authorization: `Bearer ${secret(credentials, "token")}` } }));
    case "gemini":
      return parseModels(provider, await fetch(gateway(env, "google-ai-studio", `https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(secret(credentials))}`, `/v1beta/models?key=${encodeURIComponent(secret(credentials))}`)));
    case "openai-compatible":
      throw new Error("Custom compatible providers must contribute a discovery adapter.");
  }
}
...
export async function testProvider(env: ProviderRuntimeEnv, provider: ProviderContribution, credentials: ProviderCredentials, modelId?: string) {
  ...
  return { ok: true, providerId: provider.id, modelId: model, detectedModels: models.length, latencyMs: Date.now() - startedAt };
}
