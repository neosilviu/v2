...
type CoreTrustConfig = {
  baseURL: string | null;
  trustedOrigins: string[];
  passkey: { rpID: string; origin: string } | null;
};
  ...
  try {
    ...
    const trustedOrigins = [
      ...new Set([
        ...
        ...parseOrigins(env.TRUSTED_ORIGINS),
      ]),
    ];
    ...
    if (!secret || secret.length < 32)
      throw new Error(
        "Authentication signing value must be configured server-side with at least 32 characters",
      );
    if (production && (base.protocol !== "https:" || isLocalOrigin(base)))
      throw new Error(
        "Production authentication URL must be HTTPS and non-local",
      );
    if (
      production &&
      ...
      throw new Error("Production trusted origins must be HTTPS and non-local");
    const github =
    ...
    const adminEmails = uniqueLowerEmails(
      parseOrigins(env.PLATFORM_ADMIN_EMAILS),
      recoveryAdminEnabled ? parseOrigins(env.RECOVERY_ADMIN_EMAILS) : [],
    );
    return {
      ok: true,
      config: {
        ...
        production,
        ...(github ? { github } : {}),
        passkey: { rpID: rpIdFor(base), rpName: "v2", origin: base.origin },
        adminEmails,
        recoveryAdminEnabled,
        ...(env.CORE ? { core: env.CORE } : {}),
        workspaceId: env.AUTH_WORKSPACE_ID || "default",
      },
    };
  } catch (error) {
    return {
...
export async function resolveAuthConfig(
  ...
  try {
    const response = await env.CORE.fetch(
      `https://core.internal/internal/workspaces/${encodeURIComponent(workspaceId)}/auth/trust-config`,
    );
    if (!response.ok)
      return {
        ok: false,
        message:
          "Production authentication trust is not available from Core domains.",
      };
    const trust = (await response.json()) as CoreTrustConfig;
    if (!trust.baseURL || !trust.passkey)
      return {
        ok: false,
        message:
          "Production authentication requires an active verified auth domain.",
      };
    const base = parseUrl(trust.baseURL, "Core auth domain");
    const trustedOrigins = [...new Set([base.origin, ...trust.trustedOrigins])];
    for (const origin of trustedOrigins)
      parseUrl(origin, "Core trusted origin");
    return {
      ok: true,
      config: {
        ...parsed.config,
        ...
        trustedOrigins,
        passkey: {
          ...parsed.config.passkey,
          rpID: trust.passkey.rpID,
          origin: trust.passkey.origin,
        },
        workspaceId,
      },
    };
    ...
    return {
      ok: false,
      message:
        "Production authentication trust could not be resolved from Core domains.",
    };
  }
}
...
export function createAuth(config: AuthConfig) {
  ...
  async function sendCoreMail(input: {
    purpose: "verify_email" | "reset_password";
    templateKey: "verify_email" | "reset_password";
    to: string;
    url: string;
  }) {
    if (!config.core)
      throw new Error("Core Mail Runtime binding is not configured.");
    const response = await config.core.fetch(
      `https://core.internal/internal/workspaces/${encodeURIComponent(config.workspaceId)}/mail/send`,
      {
        method: "POST",
    ...
    if (result.ok !== true || result.status !== "sent")
      throw new Error(
        typeof result.errorSafe === "string"
          ? result.errorSafe
          : "Core Mail Runtime did not confirm delivery.",
      );
  }
  return betterAuth({
    ...
    session: {
      additionalFields: { impersonatedBy: { type: "string", required: false } },
    },
    emailAndPassword: {
      enabled: true,
    ...
    plugins: [
      ...
      twoFactor({
        issuer: config.passkey.rpName,
        allowPasswordless: true,
        totpOptions: { issuer: config.passkey.rpName, allowPasswordless: true },
