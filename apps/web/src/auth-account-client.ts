...
const isPasskeyCancellationError = (error: unknown) => {
  const message = error instanceof Error
    ...
    : typeof error === "object" && error && "message" in error
      ...
      : String(error ?? "");
  return /NotAllowedError|AbortError|The operation either timed out or was not allowed|REGISTRATION_CANCELLED/i.test(message);
};
...
export async function addPasskey(name?: string) {
  ...
  if (result.error) throw new Error(authErrorMessage(result.error, "Passkey registration failed."));
  return result;
}
...
export async function enableTwoFactor(password?: string): Promise<{ totpURI: string; backupCodes: string[] }> {
  ...
  if (result.error) throw new Error(authErrorMessage(result.error, "Two-factor enrollment failed."));
  return result.data as { totpURI: string; backupCodes: string[] };
}

export async function disableTwoFactor(password?: string): Promise<{ status: boolean }> {
  const result = await authClient.twoFactor.disable(password ? { password } : {});
  if (result.error) throw new Error(authErrorMessage(result.error, "Two-factor disable failed."));
  return result.data as { status: boolean };
}

export async function generateTwoFactorBackupCodes(password?: string): Promise<{ status: boolean; backupCodes: string[] }> {
  const result = await authClient.twoFactor.generateBackupCodes(password ? { password } : {});
  if (result.error) throw new Error(authErrorMessage(result.error, "Backup code generation failed."));
  return result.data as { status: boolean; backupCodes: string[] };
}

export async function verifyTwoFactorCode(code: string, trustDevice = false): Promise<{ token: string; user: unknown }> {
  const result = await authClient.twoFactor.verifyTotp({ code, trustDevice });
  if (result.error) throw new Error(authErrorMessage(result.error, "Two-factor verification failed."));
