...
export function ProfileLogoutCard({
  ...
  signOutAction: ActionDefinition | undefined;
}) {
  return (
    <SurfaceCard className="settings-subpanel profile-logout-card">
      <h3>Logout</h3>
      <p>End the current browser session and return to login.</p>
      {signOutAction ? (
        <div className="actions">
          <Button
            className={signOutAction.variant === "danger" ? "danger" : ""}
            onClick={() => void callbacks?.onAction?.(signOutAction)}
          >
            {signOutAction.title}
          </Button>
        </div>
