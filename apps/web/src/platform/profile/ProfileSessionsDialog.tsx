...
export function ProfileSessionsDialog({
  open,
  sessions,
  onClose,
}: {
  ...
  const revokeOne = async (sessionId: string) => {
    ...
    try {
      ...
      setFeedback(
        errorMessage(error, "Unable to log out the selected session."),
      );
    } finally {
      setBusy(false);
      setBusySessionId(null);
      ...
      <SurfaceCard className="settings-dialog profile-dialog profile-sessions-dialog">
        <div className="surface-header">
          <div>
            ...
            <p>
              See where this account is signed in and log out devices you no
              longer use.
            </p>
          </div>
        ...
        <div className="profile-session-list">
          {sessions.length ? (
            sessions.map((session) => (
              <div
                className="settings-subpanel-row profile-session-row"
                key={session.id}
              >
                <div>
                  <strong>
                    {sessionDeviceLabel(session.userAgent, session.current)}
                  </strong>
                  <small>
                    {session.current
                      ? "This device"
                      : session.ipAddress
                        ...
                        : "IP unavailable"}{" "}
                    · Started {dateTimeLabel(session.createdAt)} · Expires{" "}
                    {dateTimeLabel(session.expiresAt)}
                  </small>
                </div>
