...
export function CommandPalette({ tools, open, onClose, onExecute }: { tools: ToolContribution[]; open: boolean; onClose: () => void; onExecute: (tool: ToolContribution) => void }) {
  ...
  return <div className="palette-backdrop" onClick={onClose}>
    <div className="palette" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="palette-list">{results.map((tool) => <button className="palette-item" key={tool.id} onClick={() => onExecute(tool)}><div><strong>{tool.title}</strong><small>{tool.id}</small></div><Badge>{tool.risk}</Badge></button>)}</div>
    </div>
  </div>;
}
