...
function ProfilePasswordCard() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const revokeOtherSessions = values.revokeOtherSessions === "on";
  ...
  return <SurfaceCard className="settings-subpanel profile-password-card">
    ...
    {feedback ? <p className="settings-inline-error profile-feedback">{feedback}</p> : null}
...
function ProfilePasskeyCard({ passkeys }: { passkeys: Array<{ id: string; name: string | null; deviceType: string; backedUp: boolean; createdAt: number | string }> }) {
  const [busy, setBusy] = useState(false);
  const [feedback, setFeedback] = useState<string | null>(null);
  const [formKey, setFormKey] = useState(0);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (busy) return;
    const values = Object.fromEntries(new FormData(event.currentTarget));
    const passkeyLabel = String(values.passkeyLabel ?? "").trim();
    if (passkeyLabel.length < 2) { setFeedback("Passkey label must be at least 2 characters."); return; }
    if (passkeyLabel.length > 32) { setFeedback("Passkey label is too long."); return; }
    setBusy(true);
    setFeedback(null);
    try {
      const result = await addPasskey(passkeyLabel);
      if (!result) return;
      setFormKey((value) => value + 1);
      setFeedback("Passkey added. Refreshing profile...");
      window.setTimeout(() => window.location.reload(), 250);
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : "Passkey registration failed.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="settings-subpanel-table profile-passkey-table">
      {passkeys.length ? passkeys.map((passkey) => <div className="settings-subpanel-row" key={passkey.id}>
        <div>
          ...
          <small>{passkey.deviceType} · {passkey.backedUp ? "backed up" : "not backed up"} · {passkeyCreatedAtLabel(passkey.createdAt)}</small>
        </div>
        <Badge>passkey</Badge>
...
function ProfileWorkspaceCard({
  ...
  const openWorkspaceSettings = () => {
    ...
    url.searchParams.set("tab", "platform.settings.security");
    window.location.assign(`${url.pathname}${url.search}${url.hash}`);
  };
  ...
  return <SurfaceCard className="settings-subpanel profile-workspace-card">
    ...
    <form className="profile-workspace-form" onSubmit={switchWorkspace}>
      <label className="field">Switch workspace
        <select value={selectedWorkspaceId} onChange={(event) => setSelectedWorkspaceId(event.currentTarget.value)}>
          {workspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name} · {workspace.status}</option>)}
        </select>
      </label>
      <div className="actions">
        <Button className="primary" type="submit" disabled={!selectedWorkspaceId || selectedWorkspaceId === currentWorkspace?.id}>Open workspace</Button>
      </div>
    </form>
    ...
    <div className="settings-subpanel-row">
      <div>
        ...
        <small>Create, edit and delete workspaces from the generic Security CRUD.</small>
      </div>
      <Button onClick={openWorkspaceSettings}>Open security settings</Button>
    </div>
  </SurfaceCard>;
}
...
function ProfileSecurityCard({
  ...
  profile: { email?: string | null; emailVerified?: boolean; twoFactorEnabled?: boolean; passkeys?: Array<{ id: string; name: string | null; deviceType: string; backedUp: boolean; createdAt: number | string }>; sessions?: number; activeSessions?: Array<{ id: string; current: boolean; ipAddress: string | null; userAgent: string | null; createdAt: number | string; expiresAt: number | string; impersonatedBy: string | null }> };
  currentWorkspace: { id: string; name: string; status: string; roles?: Array<{ name: string }> } | null;
  selectedWorkspaceId: string;
  onOpenSessions: () => void;
}) {
  ...
  return <SurfaceCard className="settings-subpanel profile-security-card">
    ...
    <div className="settings-subpanel-row">
      <div>
        ...
        <small>{profile.email ? "Profile" : "Account"}</small>
      </div>
      <Badge>{profile.twoFactorEnabled ? "2FA on" : "2FA off"}</Badge>
    </div>
    <div className="actions">
      <Button onClick={onOpenSessions}>Active sessions</Button>
      <Button onClick={openSettings}>Open security settings</Button>
    </div>
  </SurfaceCard>;
}
...
function ProfileDetailsDialog({
  ...
  open: boolean;
  profile: { name?: string | null; email?: string | null; language?: string | null; location?: string | null; timezone?: string | null };
  saveLabel: string;
  onClose: () => void;
}) {
  const [busy, setBusy] = useState(false);
    ...
    <SurfaceCard className="settings-dialog profile-dialog">
      <div className="surface-header">
        <div>
          <small>Profile</small>
          <h3>Edit details</h3>
          <p>Keep the compact profile card clean and edit the personal details here.</p>
        </div>
        <Button onClick={onClose} disabled={busy} type="button">Close</Button>
      </div>
      ...
      <form className="profile-form profile-dialog-form" onSubmit={submit}>
        ...
        <div className="profile-dialog-note">
          <p>Email: <strong>{profile.email ?? "Unknown"}</strong></p>
          <p>These values are kept on your user profile, not in the Web shell.</p>
        </div>
...
function ProfileSessionsDialog({
  open,
  sessions,
  onClose,
}: {
  open: boolean;
  sessions: Array<{ id: string; current: boolean; ipAddress: string | null; userAgent: string | null; createdAt: number | string; expiresAt: number | string; impersonatedBy: string | null }>;
  onClose: () => void;
}) {
  const [busy, setBusy] = useState(false);
  ...
  const revokeOthers = async () => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Unable to log out other sessions.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const revokeOne = async (sessionId: string) => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Unable to log out the selected session.");
    } finally {
      setBusy(false);
      setBusySessionId(null);
    }
    ...
    <SurfaceCard className="settings-dialog profile-dialog profile-sessions-dialog">
      <div className="surface-header">
        <div>
          ...
          <p>See where this account is signed in and log out devices you no longer use.</p>
        </div>
        <Button onClick={onClose} disabled={busy} type="button">Close</Button>
      </div>
      {feedback ? <p className="settings-inline-error profile-feedback" role="alert">{feedback}</p> : null}
      <div className="actions profile-session-actions">
        <Button className="danger" onClick={() => void revokeOthers()} disabled={busy || sessions.length <= 1}>Log out other devices</Button>
      </div>
      <div className="profile-session-list">
        {sessions.length ? sessions.map((session) => <div className="settings-subpanel-row profile-session-row" key={session.id}>
          <div>
            <strong>{sessionDeviceLabel(session.userAgent, session.current)}</strong>
            <small>{session.current ? "This device" : session.ipAddress ? `IP ${session.ipAddress}` : "IP unavailable"} · Started {sessionTimestampLabel(session.createdAt)} · Expires {sessionTimestampLabel(session.expiresAt)}</small>
          </div>
          <div className="plugin-actions">
            {session.current ? <Badge>Current</Badge> : <Button className="danger" onClick={() => void revokeOne(session.id)} disabled={busy}>{busySessionId === session.id ? "Logging out..." : "Log out"}</Button>}
          </div>
...
function ProfileTwoFactorCard({ profile }: { profile: { twoFactorEnabled?: boolean } }) {
  const [busy, setBusy] = useState(false);
  const [feedback, setFeedback] = useState<string | null>(null);
  ...
  useEffect(() => {
    let alive = true;
    if (!setup?.totpURI || setupStep !== "verify") {
      setQrDataUrl(null);
      setQrError(null);
      return () => { alive = false; };
    }
  ...
  const startSetup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Two-factor enrollment failed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const verifySetup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setFeedback(null);
    try {
      await verifyTwoFactorCode(verificationCode.trim(), true);
      window.location.reload();
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : "Two-factor verification failed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const rotateBackupCodes = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Backup code regeneration failed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const disableSetup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Two-factor disable failed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return <SurfaceCard className="settings-subpanel profile-twofactor-card">
    <div className="surface-header">
      <div>
        ...
        <p>{profile.twoFactorEnabled ? "Extra verification is on for this account." : "Add a second verification step for sign in."}</p>
      </div>
      <Badge>{profile.twoFactorEnabled ? "enabled" : setup ? "pending" : "disabled"}</Badge>
    </div>
    {feedback ? <p className="settings-inline-error profile-feedback">{feedback}</p> : null}
    {!profile.twoFactorEnabled && setupStep === "setup" ? <form className="profile-form profile-inline-form" onSubmit={startSetup}>
    ...
    {setup && setupStep === "verify" ? <div className="profile-twofactor-setup">
      <div className="profile-qr-block">
        <strong>Scan this QR code</strong>
        {qrDataUrl ? <img className="profile-qr-code" src={qrDataUrl} alt="Two-factor authentication QR code" /> : <div className="profile-qr-placeholder">{qrError ?? "Generating QR code..."}</div>}
      </div>
      ...
      <div className="profile-otp-block">
        ...
        <div className="profile-backup-codes">{(regeneratedCodes ?? setup.backupCodes).map((code) => <code key={code}>{code}</code>)}</div>
      </div>
    ...
    {profile.twoFactorEnabled ? <>
      ...
      {regeneratedCodes ? <div className="profile-otp-block">
        <strong>Latest backup codes</strong>
        <div className="profile-backup-codes">{regeneratedCodes.map((code) => <code key={code}>{code}</code>)}</div>
      </div> : null}
...
export function AccountProfile({ page, data, callbacks }: ProfilePageProps) {
  const source = (data ?? page.data) as {
    profile?: { name?: string | null; email?: string | null; emailVerified?: boolean; twoFactorEnabled?: boolean; language?: string | null; location?: string | null; timezone?: string | null; passkeys?: Array<{ id: string; name: string | null; deviceType: string; backedUp: boolean; createdAt: number | string }>; sessions?: number; activeSessions?: Array<{ id: string; current: boolean; ipAddress: string | null; userAgent: string | null; createdAt: number | string; expiresAt: number | string; impersonatedBy: string | null }>; isPlatformAdmin?: boolean };
    workspaces?: Array<{ id: string; name: string; status: string; roles?: Array<{ name: string }> }>;
    currentWorkspace?: { id: string; name: string; status: string; roles?: Array<{ name: string }> } | null;
  } | undefined;
  ...
  const stats = [
    ...
    { label: "Language", value: profile.language?.trim() || "unset" },
    { label: "Location", value: profile.location?.trim() || "unset" },
    { label: "Time zone", value: profile.timezone?.trim() || "unset" },
    { label: "Passkeys", value: String(passkeyCount) },
    { label: "2FA", value: profile.twoFactorEnabled ? "enabled" : "disabled" },
    { label: "Sessions", value: String(activeSessions.length > 0 ? activeSessions.length : (profile.sessions ?? 0)) },
  ];
  useEffect(() => {
  ...
  return <div className="page-stack profile-layout">
    <SurfaceCard className="profile-panel profile-primary">
      ...
      <div className="actions profile-main-actions">
        <Button className="primary" onClick={() => setDetailsOpen(true)}>Edit details</Button>
    ...
    <div className="profile-columns">
      ...
      <div className="profile-column">
        <ProfileWorkspaceCard workspaces={workspaces} currentWorkspace={currentWorkspace} selectedWorkspaceId={selectedWorkspaceId} setSelectedWorkspaceId={setSelectedWorkspaceId} />
        <ProfileSecurityCard profile={profile} currentWorkspace={currentWorkspace} selectedWorkspaceId={selectedWorkspaceId} onOpenSessions={() => setSessionsOpen(true)} />
        <ProfileLogoutCard callbacks={callbacks} signOutAction={signOutAction} />
      </div>
    </div>
