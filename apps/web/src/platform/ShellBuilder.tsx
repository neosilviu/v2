...
import {
  createManualInterfacePage,
  deleteManualInterfacePage,
  loadInterfaceContributions,
  updateInterfaceContribution,
...
function visibilityLabel(item: InterfaceContribution) {
  return item.visibleInNavigation
    ? "Visible in sidebar"
    : "Hidden from sidebar";
}
...
export function ShellBuilder() {
  ...
  const [draft, setDraft] = useState<Draft>({
    ...
    icon: "list",
    section: "user",
    body: "Operational notes and links for this workspace.",
  });
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setStatus(
        error instanceof Error ? error.message : "Interface unavailable",
      );
    }
  };
    ...
    try {
      ...
      setStatus(
        error instanceof Error
          ? error.message
          : "Manual page could not be created",
      );
    } finally {
      setBusy(null);
    }
  ...
  return (
    <div className="shell-builder">
      <div className="surface-header">
        ...
        <Button onClick={() => void refresh()} disabled={Boolean(busy)}>
          Refresh
        </Button>
      </div>
      ...
      <section className="settings-subpanel">
        <div className="surface-header">
          <div>
            <h3>Navigation</h3>
            <p>Runtime pages grouped by user and administration sections.</p>
          </div>
          <Badge>
            {navigation.filter((item) => item.visibleInNavigation).length}
          </Badge>
        </div>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div className="plugin-actions">
                      <Button
                        disabled={
                          !item.configurable.canHide || busy === item.id
                        }
                        onClick={() =>
                          void update(item, {
                      ...
                      >
                        {item.visibleInNavigation ? "Hide" : "Show"}
                      </Button>
                      <Button
                        disabled={
                          !item.configurable.canReorder || busy === item.id
                        }
                        onClick={() =>
                          void update(item, {
                      ...
                      <Button
                        disabled={
                          !item.configurable.canMoveSection || busy === item.id
                        }
                        onClick={() =>
                          void update(item, {
                            section:
      ...
      <section className="settings-subpanel">
        <div className="surface-header">
          <div>
            <h3>Layout</h3>
            <p>Runtime shell regions currently supported by the platform.</p>
          </div>
        </div>
        <div className="settings-access-grid">
          <div className="settings-access-card">
            <small>Top bar</small>
            <strong>Workspace switcher and account menu</strong>
            <p>Fixed platform area.</p>
          </div>
          <div className="settings-access-card">
            <small>Sidebar</small>
            <strong>
            ...
            <p>User and administration groups.</p>
          </div>
          <div className="settings-access-card">
            <small>Main content</small>
            <strong>Runtime page renderer</strong>
            <p>Native or declarative page output.</p>
          </div>
          <div className="settings-access-card">
            <small>Secondary area</small>
            <strong>Available for active panels</strong>
            <p>Shown only when a contribution uses it.</p>
          </div>
        </div>
      </section>
      ...
      <section className="settings-subpanel">
        <div className="surface-header">
          <div>
            <h3>Pages</h3>
            <p>Create a declarative manual page without custom JavaScript.</p>
          </div>
        </div>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setDraft({
                  ...draft,
                  section: event.currentTarget.value as Draft["section"],
                })
              }
            >
        ...
        <div className="plugin-list">
          {items
            ...
            .map((item) => (
              <div className="plugin-row" key={item.id}>
                <div>
                  <strong>{item.label}</strong>
                  <small>{visibilityLabel(item)}</small>
                </div>
                <div className="plugin-actions">
                  <Badge>{item.active ? "active" : "inactive"}</Badge>
                  <Button
                    className="danger"
                    disabled={!item.configurable.canDelete || busy === item.id}
                    onClick={async () => {
                      setBusy(item.id);
                      await deleteManualInterfacePage(item.id).catch((error) =>
                        setStatus(
                          error instanceof Error
                            ? error.message
                            : "Delete failed",
      ...
      <section className="settings-subpanel">
        <div className="surface-header">
          <div>
            <h3>Contributions</h3>
            <p>Available Platform, Plugin and Manual UI contributions.</p>
          </div>
          <Badge>{items.length}</Badge>
        </div>
        <div className="plugin-list">
          {items.map((item) => (
            <div className="plugin-row" key={`${item.source}:${item.id}`}>
              <div>
                <strong>{item.label}</strong>
                <small>
                  {sourceLabel(item.source)} · {item.kind} ·{" "}
                  {destinationLabel(item)} · {accessLabel(item)}
                </small>
              </div>
              <div className="plugin-actions">
