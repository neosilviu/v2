  ...
  return <div className="shell-builder">
    ...
    <section className="settings-subpanel">
      ...
      <div className="template-table-wrap domain-table">
        <table>
          <thead><tr><th>Label</th><th>Path</th><th>Source</th><th>Section</th><th>Visible</th><th>Order</th><th>Required access</th><th>Actions</th></tr></thead>
          <tbody>{navigation.map((item) => <tr key={item.id}>
            ...
            <td>{item.requiredPermission ?? "Workspace member"}</td>
    ...
    <section className="settings-subpanel">
      ...
      <div className="plugin-list">{items.map((item) => <div className="plugin-row" key={`${item.source}:${item.id}`}>
        <div><strong>{item.label}</strong><small>{sourceLabel(item.source)} · {item.kind} · {item.path || "no navigation path"} · {item.requiredPermission ?? "member access"}</small></div>
