...
import {
  ...
  type RuntimeUiBootstrap,
  type StartupBootstrap,
...
export const coreUrl =
  import.meta.env.VITE_CORE_API_URL ?? "http://localhost:8787";
type ResponseLike = Pick<Response, "ok" | "status" | "json" | "text">;
export const coreApi = hc<CoreApi>(coreUrl, {
...
export function isCoreAuthRequiredError(
  error: unknown,
): error is CoreAuthRequiredError {
...
async function parseCoreError(response: ResponseLike) {
  ...
  try {
    ...
    const nested =
      typeof fallback.error === "object" && fallback.error
        ? (fallback.error as { code?: unknown; message?: unknown })
        : null;
    const code =
    ...
    return new CoreRequestError(
      response.status,
      undefined,
      text || `Core request failed: ${response.status}`,
    );
  }
}
...
async function resolvePluginOperation(
  operationId: string,
): Promise<{ pluginId: string; operation: PluginOperation } | null> {
  const plugins = await loadInstalledPlugins();
  for (const plugin of plugins) {
    const operation = plugin.api.operations.find(
...
async function invokePluginOperation(
  ...
  if (!resolved)
    throw new CoreRequestError(
      404,
      "not_found",
      `Plugin operation ${operationId} is not available.`,
    );
  return coreResponse(
    coreApi.workspaces[":workspaceId"].plugins[":pluginId"].operations[
      ":operationId"
    ].$post({
      param: { workspaceId, pluginId: resolved.pluginId, operationId },
      json: { input, routeParams, queryParams },
    }),
    runtimeResultEnvelopeSchema,
  );
}
...
export function loadShellBootstrap(
  ...
  if (!shellBootstrap) {
    const request = workspaceId
      ? coreApi.workspaces[":workspaceId"].bootstrap.$get({
          param: { workspaceId },
        })
      : coreApi.workspaces.current.bootstrap.$get();
    shellBootstrap = coreResponse(request, shellBootstrapSchema)
      .then((bootstrap) => {
...
export async function loadCurrentImpersonation(): Promise<ImpersonationContext | null> {
  return (
    await coreResponse(
      coreApi.session.impersonation.$get(),
      impersonationResponseSchema,
    )
  ).impersonation;
}

export async function stopCurrentImpersonation(): Promise<{
  restored: boolean;
  reauthenticationRequired: boolean;
}> {
  const response = await coreResponse(
    coreApi.session.impersonation.stop.$post({ json: {} }),
    stopImpersonationResponseSchema,
  );
  invalidateApiCaches();
  return {
    restored: response.restored,
    reauthenticationRequired: response.reauthenticationRequired,
  };
}

export async function loadOwnerSetup(token: string): Promise<{
  setup: {
    workspaceId: string;
    ownerEmail: string;
    status: string;
  ...
  return coreResponse(
    coreApi.setup.owner.$get({ query: { token } }),
    ownerSetupStatusSchema,
  );
}
...
export async function consumeOwnerSetup(
  token: string,
): Promise<{ status: "consumed"; workspaceId: string }> {
  const result = await coreResponse(
    coreApi.setup.owner.consume.$post({ json: { token } }),
    ownerSetupConsumeResponseSchema,
  );
...
export async function loadCurrentRbac(): Promise<RbacMe> {
  return coreResponse(
    coreApi.workspaces[":workspaceId"].rbac.me.$get({
  ...
  await coreResponse(
    coreApi.layouts.$put({
      json: {
        ...
        layout: { zones: state.zones, placements: state.placements },
      },
    }),
    { parse: () => undefined },
  );
...
export async function loadInterfaceContributions(): Promise<
  InterfaceContribution[]
> {
  return (
    await coreResponse(
      coreApi.workspaces[":workspaceId"].interface.contributions.$get({
        param: { workspaceId: currentWorkspaceId() },
      }),
...
export async function updateInterfaceContribution(
  contributionId: string,
  input: Record<string, unknown>,
): Promise<void> {
  await coreResponse(
    coreApi.workspaces[":workspaceId"].interface.contributions[
      ":contributionId"
    ].$put({
      param: { workspaceId: currentWorkspaceId(), contributionId },
      json: input,
    }),
    { parse: () => undefined },
  );
...
export async function createManualInterfacePage(
  input: Record<string, unknown>,
): Promise<{ contributionId: string; path: string }> {
  const response = await coreResponse(
    coreApi.workspaces[":workspaceId"].interface.pages.$post({
      param: { workspaceId: currentWorkspaceId() },
      json: input,
    }),
    z.object({
      page: z.object({ contributionId: z.string(), path: z.string() }),
    }),
  );
  invalidateApiCaches();
...
export async function deleteManualInterfacePage(
  contributionId: string,
): Promise<void> {
  await coreResponse(
    coreApi.workspaces[":workspaceId"].interface.pages[
      ":contributionId"
    ].$delete({ param: { workspaceId: currentWorkspaceId(), contributionId } }),
    { parse: () => undefined },
  );
  invalidateApiCaches();
}

export async function loadRuntimePage(contributionId: string): Promise<{
  contribution: InterfaceContribution;
  page: DeclarativePageContribution;
}> {
  return coreResponse(
    coreApi.workspaces[":workspaceId"].interface.pages[":contributionId"].$get({
      param: { workspaceId: currentWorkspaceId(), contributionId },
    }),
    z.object({
      contribution: interfaceContributionSchema,
      page: z.any() as z.ZodType<DeclarativePageContribution>,
    }),
  );
}
...
async function loadRuntimeUiBootstrap() {
  if (!runtimeUiBootstrap) {
    runtimeUiBootstrap = coreResponse(
      coreApi.runtime.ui.bootstrap.$get({
...
export async function loadWorkspaceUiSurfaces(): Promise<
  SurfaceContribution[]
> {
...
export async function loadSettingsTabs(): Promise<RuntimeSettingsTab[]> {
  return (
    await coreResponse(
      coreApi.workspaces[":workspaceId"].settings.tabs.$get({
        param: { workspaceId: currentWorkspaceId() },
      }),
...
export async function loadSettingsTab(
  tabId: string,
): Promise<RuntimeSettingsTabResolution> {
  return coreResponse(
    coreApi.workspaces[":workspaceId"].settings.tabs[":tabId"].$get({
      param: { workspaceId: currentWorkspaceId(), tabId },
    }),
    runtimeSettingsTabResolutionSchema,
  );
}

export async function loadPublicPage(pathname: string): Promise<{
  page: DeclarativePageContribution;
  routeParams: Record<string, string>;
...
export async function loadPublicRuntimeData(
  contributionId: string,
  dataSourceId: string,
  routeParams: Record<string, string> = {},
  publicWorkspaceId = currentWorkspaceId(),
): Promise<RuntimeResultEnvelope> {
  return coreResponse(
...
export async function deactivatePlugin(pluginId: string): Promise<void> {
  await coreResponse(
    coreApi.plugins.deactivate.$post({
      json: { workspaceId: currentWorkspaceId(), pluginId },
    }),
    { parse: () => undefined },
  );
  invalidateApiCaches();
...
export async function executeTool(
  ...
  return coreResponse(
    ...
    { parse: (value) => value as ToolExecutionResult },
  );
}
...
export async function decideToolApproval(
  ...
  return (
    await coreResponse(
      coreApi["tool-approvals"].decision.$post({
        json: { workspaceId: currentWorkspaceId(), approvalId, decision },
      }),
      { parse: (value) => value as { approval: ToolApproval } },
    )
  ).approval;
}
...
export async function loadPendingToolApprovals(): Promise<ToolApproval[]> {
  return (
    await coreResponse(
      coreApi.workspaces[":workspaceId"]["tool-approvals"].$get({
        param: { workspaceId: currentWorkspaceId() },
      }),
      { parse: (value) => value as { approvals: ToolApproval[] } },
    )
  ).approvals;
}
...
export async function loadPendingApprovalRequests(): Promise<
  ApprovalRequest[]
> {
  return (
    await coreResponse(
      coreApi.workspaces[":workspaceId"]["approval-requests"].$get({
        param: { workspaceId: currentWorkspaceId() },
      }),
      approvalRequestListSchema,
    )
...
export async function decideApprovalRequest(
  ...
  return (
    await coreResponse(
      coreApi["approval-requests"][":approvalId"].decision.$post({
        param: { approvalId },
        json: { workspaceId: currentWorkspaceId(), decision },
      }),
      z.object({ approval: approvalRequestSchema }),
    )
  ).approval;
}
...
export async function loadPluginCatalog(): Promise<PluginCatalogEntry[]> {
  return (
    await coreResponse(
      coreApi.marketplace.plugins.$get({
        query: { workspaceId: currentWorkspaceId() },
      }),
      z.object({ plugins: z.array(pluginCatalogEntrySchema) }),
    )
  ).plugins;
}
...
export async function installPluginFromCatalog(
  ...
  const result = await coreResponse(
    coreApi.marketplace.plugins[":pluginId"].install.$post({
      param: { pluginId },
      query: { workspaceId: currentWorkspaceId() },
...
export async function approveInstall(
  ...
  const manifest = (
    await coreResponse(
      coreApi.plugins.install.$post({
        json: { workspaceId: currentWorkspaceId(), approvalId },
      }),
      {
        parse: (value) => value as { status: string; manifest: PluginManifest },
      },
    )
  ).manifest;
  invalidateApiCaches();
