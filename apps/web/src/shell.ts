import type { PluginManifest, SurfaceContribution, ZoneContribution } from "@v2/plugin-contracts";
import { createShellFromPlugins, mountSurface, type ShellState } from "@v2/ui-runtime";
...
export const composeShellFromSurfaces = (surfaces: SurfaceContribution[]): ShellState => {
  const initial: ShellState = { zones: foundationZones, surfaces, layouts: [], placements: [] };
  return surfaces.reduce<ShellState>((state, surface, order) => mountSurface(state, { surfaceId: surface.id, zoneId: surface.zone, order }), initial);
};
