...
export function App() {
  ...
  useEffect(() => {
    void Promise.all([loadInstalledPlugins(), loadRuntimeTools(), loadLayout()])
      .then(([installed, runtimeTools, layout]) => {
  ...
  return <>
    <div className="app-shell">
      ...
      <aside className="sidebar"><div className="sidebar-label">PLUGINS</div>{plugins.length ? plugins.map((plugin) => <button key={plugin.id} className={selectedPlugin === plugin.id ? "nav active" : "nav"} onClick={() => setSelectedPlugin(plugin.id)}>{plugin.name}</button>) : <p className="message">No installed plugins</p>}<div className="sidebar-label">PLATFORM</div><button className="nav active">Plugin Manager</button><button className="nav">Settings</button></aside>
      <main className="workspace"><div className="workspace-header"><div><h1>{selected?.name ?? "Platform"}</h1><p>Runtime contribution inspector and UI editor</p></div><div className="header-actions"><Badge>workspace/default</Badge><Button onClick={persistLayout}>Save layout</Button></div></div><div className="cards"><PluginManagerPanel plugins={plugins} onChanged={() => void refreshPlugins()} /><RuntimeShellEditor state={shell} onChange={setShell} /><SettingsRenderer settings={settings} /></div></main>
