...
export function App() {
  ...
  const runTool = async (tool: ToolContribution, approved = false) => {
    ...
    try { const result = await executeTool(tool.id, approved); if (result.status === "approval-required") { setPendingTool(tool); return; } setPendingTool(null); setNotice(`${result.status}: ${tool.id}`); }
  ...
  return <>
    <div className="app-shell">
      ...
      <aside className="assistant">{assistant.length ? assistant.map((surface) => <AgentPanel key={surface.id} title={surface.title} />) : <div className="message">Assistant surface is mounted in another zone.</div>}</aside>
      <footer className="statusbar"><span>{notice}</span><span>{plugins.length} built-in plugins</span><span>MCP bridge ready</span></footer>
    </div><CommandPalette tools={tools} open={paletteOpen} onClose={() => setPaletteOpen(false)} onExecute={(tool) => void runTool(tool)} /><ToolApprovalDialog tool={pendingTool} onCancel={() => setPendingTool(null)} onApprove={() => pendingTool && void runTool(pendingTool, true)} />
  </>;
}
