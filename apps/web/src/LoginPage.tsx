...
import {
  ...
  authTwoFactorPendingStorageKey,
} from "./auth-client";
...
function safeRedirectTarget(redirectTo?: string) {
  const target =
    redirectTo ?? new URLSearchParams(window.location.search).get("redirectTo");
  if (
    !target ||
    !target.startsWith("/") ||
    target.startsWith("//") ||
...
function contributionsFor(
  slot: LoginSlot,
  contributions: AuthUiContribution[],
) {
...
function SlotRenderer({
  ...
  return (
    <div className="login-slot" data-slot={slot}>
      {items.map((item) => (
        <DeclarativeBlocks key={item.contributionId} schema={item.renderer} />
      ))}
    </div>
  );
}
...
function PasswordMethod({
    ...
    try {
      ...
      if (mode === "signin") {
        ...
        const twoFactorPending =
          window.sessionStorage.getItem(authTwoFactorPendingStorageKey) === "1";
        if (
          result &&
  ...
  return (
    <form
      className="login-method password-method"
      onSubmit={(event) => void submit(event)}
    >
      <div className="method-header">
        <strong>{method.title}</strong>
        <Badge>{mode === "signin" ? "sign in" : "sign up"}</Badge>
      </div>
      {mode === "signup" ? (
        <label>
          Name
      ...
      <label>
        ...
        <input
          ...
          autoComplete={passkeyAvailable ? "username webauthn" : "username"}
          required
        />
      </label>
      <label>
      ...
      <div className="login-actions">
        ...
        {allowSignup ? (
          ...
          >
            {mode === "signin" ? "Use sign up" : "Use sign in"}
          </Button>
        ) : null}
      </div>
...
function TwoFactorChallenge({
    ...
    try {
      const result =
        mode === "backup"
          ...
          : await verifyTwoFactorCode(value, true);
  ...
  return (
    <form
      className="login-method two-factor-method"
      onSubmit={(event) => void submit(event)}
    >
      <div className="method-header">
      ...
      <div className="login-segment-tabs">
        <button
          type="button"
          className={mode === "totp" ? "segment-active" : ""}
        ...
        <button
          type="button"
          className={mode === "backup" ? "segment-active" : ""}
...
function SocialMethod({
  ...
  const signIn = async () => {
    await authClient.signIn.social({
      provider,
      callbackURL: safeRedirectTarget(redirectTo),
    });
  };
  return (
    <button
      className="login-method social-method"
      type="button"
      onClick={() => void signIn()}
    >
      <span>{method.title}</span>
      <Badge>{method.providerId}</Badge>
    </button>
  );
}
...
function PasskeyMethod({
  ...
  return (
    <div className="login-method passkey-method">
      <div className="method-header">
        <strong>{method.title}</strong>
        <Badge>{supported ? "available" : "unsupported"}</Badge>
      </div>
      <Button type="button" disabled={!supported} onClick={() => void signIn()}>
...
function LoginMethods({
  ...
  const byType = (type: PublicLoginMethod["type"]) =>
    methods.filter((method) => method.type === type);

  return (
    <>
      {byType("password").map((method) => (
        <PasswordMethod
          key={method.id}
          method={method}
          passkeyAvailable={passkeySupported && config.features.passkey}
          allowSignup={config.policy.registrationMode === "open"}
          turnstile={{
            enabled: config.policy.turnstileEnabled,
            siteKey: config.policy.turnstileSiteKey,
          }}
          redirectTo={redirectTo}
        />
      ))}
      {config.policy.registrationMode === "invitation-only" ? (
        <div className="login-method">
          ...
          <p className="login-status">
            Account creation is available only through an invitation.
          </p>
        </div>
      ) : null}
      {byType("social").map((method) => (
        <SocialMethod key={method.id} method={method} redirectTo={redirectTo} />
      ))}
      {byType("passkey").map((method) => (
        <PasskeyMethod
          key={method.id}
          method={method}
  ...
  useEffect(() => {
    void loadLoginConfig()
      ...
      .catch((reason: unknown) =>
        setError(
          reason instanceof Error ? reason.message : "Login config unavailable",
        ),
      );
  }, []);
          ...
          <div className="login-intro-metrics">
            <div>
              <strong>
                {twoFactorPage
                  ? "2FA"
                  : config?.policy.registrationMode === "open"
                    ? "Open signup"
                    : "Published login"}
              </strong>
              <span>
                {twoFactorPage ? "Challenge mode" : "Workspace policy"}
              </span>
            </div>
            <div>
        ...
        <SurfaceCard className="login-panel">
          ...
          {twoFactorPage ? (
            ...
            loginSlots.map((slot) => {
              if (
                slot === "login.password" ||
                slot === "login.socialMethods" ||
                slot === "login.passkey"
              )
                return (
                  <div key={slot} className="login-slot">
                    <SlotRenderer
                      slot={slot}
                      contributions={config.uiContributions}
                    />
                    {slot === "login.password" ? (
                      <LoginMethods
                        config={{
                          ...config,
                          methods: config.methods.filter(
          ...
          !config.features.passkey ? (
            <p className="login-status">
              Social and passkey methods are hidden until explicitly published
              by an Auth administrator.
            </p>
          ) : null}
        </SurfaceCard>
      </div>
