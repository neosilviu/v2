...
type AuthApiClient = {
  ...
  api: {
    auth: {
      "update-user": { $post(args?: HonoRequestArgs): Promise<Response> };
      "sign-in": {
        email: { $post(args?: HonoRequestArgs): Promise<Response> };
      };
      "sign-up": {
        email: { $post(args?: HonoRequestArgs): Promise<Response> };
      };
      "sign-out": { $post(args?: HonoRequestArgs): Promise<Response> };
    };
  };
  setup: {
    owner: {
...
export const authApi = hc(authUrl, {
...
async function parseAuthError(response: Response) {
  ...
  try {
    ...
    return new AuthRequestError(
      response.status,
      undefined,
      text || `Auth request failed: ${response.status}`,
    );
  }
}

async function authResponse<T>(
...
export async function loadLoginConfig(
  workspaceId = "default",
): Promise<AuthPublicLoginConfig> {
  const key = workspaceId || "default";
  if (!loginConfigCache.has(key)) {
    loginConfigCache.set(
      key,
      authResponse(
        authApi.public.auth["login-config"].$get({ query: { workspaceId } }),
        authPublicLoginConfigSchema,
      ).catch((error) => {
...
export async function ownerSetupSignUp(input: {
  token: string;
  email: string;
  name: string;
  password: string;
  ...
  await authResponse(
    authApi.setup.owner["sign-up"].email.$post({
      json: ownerSetupSignupRequestSchema.parse(input),
    }),
    { parse: () => undefined },
  );
}
...
}): Promise<{ twoFactorRedirect?: boolean } | void> {
  const response = await authApi.api.auth["sign-in"].email.$post({
    json: input,
  });
  if (!response.ok) throw await parseAuthError(response);
...
export async function signUpEmail(input: {
  ...
  const response = await authApi.api.auth["sign-up"].email.$post({
    json: input,
  });
  if (!response.ok) throw await parseAuthError(response);
}
...
export async function signOutAuth(): Promise<void> {
  await authResponse(authApi.api.auth["sign-out"].$post(), {
    parse: () => undefined,
  });
}
