...
import { capabilityGrantRequestSchema, layoutWriteRequestSchema, pluginActivationRequestSchema, pluginInstallRequestSchema, settingScopeSchema, settingWriteRequestSchema, toolApprovalDecisionRequestSchema, toolApprovalLookupRequestSchema, toolExecutionRequestSchema, type SettingScope } from "@v2/rpc-contracts";
...
function requireShellRead(c: CoreContext): Response | undefined {
  if (c.get("internal") || c.get("user")) return undefined;
  const origin = c.req.header("origin");
  if (origin && allowedOrigins(c.env).includes(origin)) return undefined;
...
app.get("/workspaces/:workspaceId/settings/:scope", async (c) => { const denied = requireShellRead(c); if (denied) return denied; const scope = settingScopeSchema.parse(c.req.param("scope")) as SettingScope; return c.json({ settings: await new CoreRepository(c.env.CORE_DB).listSettings(c.req.param("workspaceId"), scope) }); });
...
app.get("/workspaces/:workspaceId/layout", async (c) => { const denied = requireShellRead(c); if (denied) return denied; return c.json({ layout: (await new CoreRepository(c.env.CORE_DB).getLayout(c.req.param("workspaceId"))) ?? null }); });
