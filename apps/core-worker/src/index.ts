...
app.get("/runtime/tools", async (c) => {
  const active = await activePluginIds(c, c.req.query("workspaceId") ?? defaultWorkspaceId);
...
app.post("/plugins/grants", async (c) => {
  ...
  return c.json({ pluginId: request.pluginId, capabilities: await new CoreRepository(c.env.CORE_DB).grantCapabilities(request.workspaceId, request.pluginId, request.capabilities) });
});
