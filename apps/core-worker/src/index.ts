...
import { declarativePageContributionSchema, runtimeActionRequestSchema, runtimeDataRequestSchema } from "@v2/ui-schema";
...
import { platformSettingsRuntimeAction, platformSettingsRuntimeData } from "./platform-settings-runtime";
...
async function installMarketplacePluginFromSettings(c: CoreContext, repo: CoreRepository, workspaceId: string, pluginId: string, approvalId?: string) {
  ...
  if (assessment.requiresApproval) {
    ...
    if (!approved) return c.json(pluginOperationEnvelope("denied", null, "A matching approved installation request is required."), 403);
...
async function runtimePage(c: CoreContext, repo: CoreRepository, workspaceId: string, contributionId: string) {
  ...
  if (contributionId === "platform.account") {
    const page = declarativePageContributionSchema.parse({ ...resolved.page, templateId: "account.profile", data: { ...resolved.page.data, ...(await platformAccountRuntimeData(c, repo, workspaceId, (path) => authInternalJson(c, path))) } });
...
app.post("/runtime/ui/actions", async (c) => {
  ...
  if (action.commandId === "platform.account.profile.save") {
    ...
    const response = await authForwardResponse(c, "/api/auth/update-user", {
      ...
      return c.json(pluginOperationEnvelope("ok", await platformAccountRuntimeData(c, repo, request.workspaceId, (path) => authInternalJson(c, path))));
    }
