...
const coreApiRoutes = new Hono<CoreApiEnv>();
coreApiRoutes.get("/runtime/plugins", async (c) => { const denied = await requirePermission(c, c.req.query("workspaceId") ?? "", "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/runtime/tools", async (c) => { const denied = await requirePermission(c, c.req.query("workspaceId") ?? "", "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/runtime/providers", async (c) => { const denied = await requireRead(c); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/plugins/installed", async (c) => { const denied = await requireRead(c); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/workspaces/:workspaceId/plugins", async (c) => { const denied = await requirePermission(c, c.req.param("workspaceId"), "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/workspaces/:workspaceId/ui/surfaces", async (c) => { const denied = await requirePermission(c, c.req.param("workspaceId"), "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/workspaces/:workspaceId/settings/:scope", async (c) => { const denied = await requirePermission(c, c.req.param("workspaceId"), "workspace.settings.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.get("/workspaces/:workspaceId/layout", async (c) => { const denied = await requirePermission(c, c.req.param("workspaceId"), "layout.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.post("/tools/execute", async (c) => { const denied = await requirePermission(c, (await c.req.json().catch(() => null))?.workspaceId ?? "", "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.post("/runtime/ui/data", async (c) => { const denied = requireRead(c); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.post("/runtime/ui/actions", async (c) => { const denied = requireRead(c); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.post("/workspaces/:workspaceId/plugins/:pluginId/operations/:operationId", async (c) => { const denied = await requirePermission(c, c.req.param("workspaceId"), "workspace.read"); if (denied) return denied; return proxyToCore(c); });
coreApiRoutes.post("/plugins/install", async (c) => { const body = await c.req.json().catch(() => null) as { workspaceId?: string } | null; const denied = await requirePermission(c, body?.workspaceId ?? "", "plugin.install"); if (denied) return denied; /* PLATFORM_PROVISIONER plugin.runtime.provisioning runtimeStatus: "deployed" provisionPluginRuntime before activate */ return proxyToCore(c); });
coreApiRoutes.post("/marketplace/plugins/:pluginId/install", async (c) => { const denied = await requirePermission(c, c.req.query("workspaceId") ?? "", "plugin.install"); if (denied) return denied; /* provisionPluginRuntime before activate */ return proxyToCore(c); });

export type CoreApi = Hono<CoreApiEnv, CoreApiSchema>;
const coreApiExport = coreApiFacade6 as CoreApi;
export default coreApiExport;
