...
import type {
  MailDeliveryResult,
  MailMessageRequest,
  MailProviderConfigure,
  MailProviderPublicSummary,
  MailTemplate,
...
import {
  ...
  type AccessMode,
  type DeclarativePageContribution,
  type SettingsPanelContribution,
  type SettingsTabContribution,
} from "@v2/ui-schema";
import { CloudflarePlatformClient } from "@v2/cloudflare-platform";
...
function publicSurfaceId(
  contribution: PublicContribution | undefined,
): string | undefined {
  if (!contribution || !("surfaceId" in contribution)) return undefined;
  return typeof contribution.surfaceId === "string"
    ? contribution.surfaceId
    : undefined;
}
function allPublicContributions(
  manifest: PluginManifest,
): Array<
  PublicRouteContribution | PublicSurfaceContribution | PublicToolContribution
> {
  return [
    ...manifest.contributes.publicRoutes,
    ...manifest.contributes.publicSurfaces,
    ...manifest.contributes.publicTools,
  ];
}
...
export type PrivateRuntimeContributionResolution =
  RuntimeContributionResolution & {
...
export type WorkspaceCloudflareConnection = {
  ...
  allowedZones: Array<{ id?: string; name?: string; status?: string }>;
  zoneCount: number;
  status: "untested" | "ok" | "error" | "disabled";
  defaultForProvisioning: boolean;
  lastCheckedAt: string | null;
  lastError: string | null;
  createdAt: string;
  updatedAt: string;
};
type WorkspaceCloudflareConnectionRow = {
  id: string;
  workspace_id: string;
  label: string;
  ...
  allowed_zones_json: string;
  status: WorkspaceCloudflareConnection["status"];
  default_for_provisioning: number;
...
type MailSecretConfig = {
  endpoint?: string;
  token?: string;
  headers?: Record<string, string>;
};
type MailDeliveryAdapterResult = {
  ok: boolean;
  providerMessageId?: string;
  errorSafe?: string;
};
type CloudflareConnectionSecretConfig = {
  accountId?: string;
  apiToken?: string;
  token?: string;
  baseUrl?: string;
...
type ActiveUserPlanRecord = PlanRecord & {
  assignmentId: string;
  startsAt: string | null;
  endsAt: string | null;
};
...
function parseSecretConfigs(
  env?: Pick<CoreEnv, "MAIL_PROVIDER_CONFIGS_JSON">,
): Record<string, MailSecretConfig> {
...
function parseCloudflareConnectionConfigs(
  env?: Pick<CoreEnv, "CLOUDFLARE_CONNECTION_CONFIGS_JSON">,
): Record<string, CloudflareConnectionSecretConfig> {
  if (!env?.CLOUDFLARE_CONNECTION_CONFIGS_JSON) return {};
  try {
    const parsed = JSON.parse(env.CLOUDFLARE_CONNECTION_CONFIGS_JSON) as unknown;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
      return {};
    return parsed as Record<string, CloudflareConnectionSecretConfig>;
  } catch {
    return {};
  }
}
...
class CoreMailDeliveryAdapter {
  constructor(
    private readonly env?: Pick<
      CoreEnv,
      "ENVIRONMENT" | "MAIL_PROVIDER_CONFIGS_JSON"
    >,
  ) {}

  async deliver(
    provider: MailProviderPublicSummary & { configurationRef: string | null },
    message: MailMessageRequest & {
      subject: string;
      text: string;
      html?: string;
    },
  ): Promise<MailDeliveryAdapterResult> {
    if (provider.kind === "mock-development-only") {
      if (this.env?.ENVIRONMENT === "production")
        return {
          ok: false,
          errorSafe:
            "Development-only mail providers are disabled in production.",
        };
    ...
    if (provider.kind === "smtp") {
      return {
        ok: false,
        errorSafe:
          "Direct SMTP delivery is not available in the Worker runtime. Configure a transactional-http provider.",
      };
    }
    if (!provider.configurationRef)
      return {
        ok: false,
        errorSafe: "Mail provider secret reference is not configured.",
      };
    ...
    if (!secret?.endpoint)
      return {
        ok: false,
        errorSafe:
          "Mail provider secret reference cannot be resolved server-side.",
      };
    ...
    try {
      ...
      if (!response.ok)
        return {
          ok: false,
          errorSafe: `Mail provider rejected delivery with HTTP ${response.status}.`,
        };
...
function routeMetadata(pattern: string) {
  ...
  const parameterNames = parsed
    .split("/")
    .filter((part) => part.startsWith(":"))
    .map((part) => part.slice(1));
  return {
    pattern: parsed,
    kind: parameterNames.length
      ? ("parameterized" as const)
      : ("exact" as const),
    parameterNames,
    staticSegments: parsed
      .split("/")
...
function config(
  canDelete = false,
  overrides: Partial<UiConfigurable> = {},
): UiConfigurable {
...
function manualContributionId(workspaceId: string, slug: string) {
  return `manual.${workspaceId}.${slug
...
export class CoreRepository {
  constructor(
    ...
    private readonly env?: Pick<
      ...
      | "AUTH"
      | "ENVIRONMENT"
      | "MAIL_PROVIDER_CONFIGS_JSON"
      | "CLOUDFLARE_CONNECTION_CONFIGS_JSON"
      | "PLATFORM_ADMIN_EMAILS"
      | "RECOVERY_ADMIN_EMAILS"
      | "RECOVERY_ADMIN_ENABLED"
    >,
  ) {}

  private async authInternalResponse(
    path: string,
  ...
  private async authInternalJson<T>(
    path: string,
    init?: { method?: string; body?: unknown },
  ) {
    const response = await this.authInternalResponse(path, init);
    if (!response.ok)
      throw new Error(`Auth internal request failed: ${response.status}`);
    return response.json() as Promise<T>;
  }

  private async internalSeedCurrent(
    workspaceId: string,
    key: string,
    version: string,
  ) {
    const row = await this.db
      .prepare(
        "SELECT value_json FROM workspace_settings WHERE workspace_id = ? AND scope = '__internal' AND key = ? LIMIT 1",
      )
  ...
  private internalSeedStatement(
    ...
    return this.db
      .prepare(
        `INSERT INTO workspace_settings (workspace_id, scope, key, value_json, updated_at)
  ...
  async ensureWorkspace(
    ...
    status:
      ...
      | "provisioning"
      | "active"
      | "suspended" = "unprovisioned",
  ) {
    await this.db
      .prepare(
        "INSERT OR IGNORE INTO workspaces (id, name, status) VALUES (?, ?, ?)",
      )
      .bind(workspaceId, name, status)
      .run();
  }
  ...
  async authUsers() {
    ...
    if (!response.ok)
      throw new Error(`Auth internal user lookup failed: ${response.status}`);
    return response.json() as Promise<
      AuthUserLookupRecord[] | { users?: AuthUserLookupRecord[] }
    >;
  }
  ...
  async disableAuthUser(userId: string) {
    return this.authInternalResponse(
      `/internal/auth/users/${encodeURIComponent(userId)}/disable`,
      { method: "POST" },
    );
  }
  ...
  async workspaceOwnerCount(workspaceId: string) {
    const row = await this.db
      .prepare(
        `SELECT COUNT(DISTINCT member_roles.user_id) AS count
  ...
  async countOwnedWorkspaces(userId: string) {
    const row = await this.db
      .prepare(
        `SELECT COUNT(DISTINCT member_roles.workspace_id) AS count
      FROM workspace_member_roles member_roles
      INNER JOIN workspace_roles roles ON roles.workspace_id = member_roles.workspace_id AND roles.id = member_roles.role_id AND roles.system_key = 'owner'
      INNER JOIN workspace_members members ON members.workspace_id = member_roles.workspace_id AND members.user_id = member_roles.user_id AND members.status = 'active'
  ...
  async activeUserPlan(userId: string): Promise<ActiveUserPlanRecord | null> {
    const row = await this.db
      .prepare(
        `SELECT assignments.id AS assignment_id, assignments.plan_id, assignments.starts_at, assignments.ends_at, plans.name, plans.status, plans.limits_json, plans.created_at, plans.updated_at
  ...
  async workspaces(): Promise<WorkspaceRecord[]> {
    const rows = await this.db
      .prepare(
        "SELECT id, name, status, created_at, updated_at FROM workspaces ORDER BY name",
      )
      .all<{
        id: string;
        name: string;
  ...
  async workspace(workspaceId: string): Promise<WorkspaceRecord | null> {
    const row = await this.db
      .prepare(
        "SELECT id, name, status, created_at, updated_at FROM workspaces WHERE id = ? LIMIT 1",
      )
      .bind(workspaceId)
      .first<{
        id: string;
  ...
  async upsertWorkspace(
    input: { id?: string; name: string; status: WorkspaceRecord["status"] },
    actorId?: string,
  ) {
    ...
    const isAdmin = actor
      ? this.env
        ? isPlatformAdmin(this.env, { id: actor.id, email: actor.email })
        : false
      : false;
    ...
    if (creating && !isAdmin && actorId) {
      ...
      if (!plan)
        throw new Error("An active plan is required to create a workspace.");
      const maxWorkspaces =
        typeof plan.limits.maxWorkspaces === "number"
          ? plan.limits.maxWorkspaces
          : null;
      if (
        maxWorkspaces !== null &&
        (await this.countOwnedWorkspaces(actorId)) >= maxWorkspaces
      ) {
        throw new Error(
          "Workspace limit for the active plan has been reached.",
        );
      }
    }
    if (existing && !isAdmin) {
      const actorMember = actorId
        ? (await this.workspaceMemberRecords(id)).find(
            (member) => member.user.id === actorId,
          )
        : null;
      if (!actorMember?.roles.some((role) => role.systemKey === "owner"))
        throw new Error(
          "Only a workspace Owner or platform Superadmin may update a workspace.",
        );
    }
    await this.db
      .prepare(
    ...
    if (creating && actorId && !isAdmin) {
      ...
      await this.db.batch([
        ...
        this.db
          .prepare(
            "INSERT OR IGNORE INTO workspace_member_roles (workspace_id, user_id, role_id) VALUES (?, ?, ?)",
          )
          .bind(id, actorId, `${id}:owner`),
      ]);
    } else {
      await this.ensureWorkspaceRbac(id);
    }
    await this.audit(
      ...
      "workspace.upsert",
      {
        ...
        status: input.status,
        created: creating,
        actorIsPlatformAdmin: isAdmin,
      },
      actorId,
    );
    return (
      (await this.workspaces()).find((workspace) => workspace.id === id) ?? null
    );
  }
  ...
  async deleteWorkspace(workspaceId: string, actorId?: string) {
    ...
    const isAdmin = actor
      ...
      : false;
    if (!isAdmin && actorId) {
      const actorMember = (await this.workspaceMemberRecords(workspaceId)).find(
        (member) => member.user.id === actorId,
      );
      if (!actorMember?.roles.some((role) => role.systemKey === "owner"))
        return false;
  ...
  private rolePermissions(
    systemKey: "owner" | "admin" | "editor" | "viewer",
  ): WorkspacePermission[] {
  ...
  private async migrateLegacyWorkspaceRoles(workspaceId: string) {
    const operator = await this.db
      .prepare(
        "SELECT id FROM workspace_roles WHERE workspace_id = ? AND system_key = 'operator' LIMIT 1",
      )
      .bind(workspaceId)
      .first<{ id: string }>();
    if (!operator) return;
    ...
    for (const permission of this.rolePermissions("editor")) {
      statements.push(
        this.db
          .prepare(
            "INSERT OR IGNORE INTO workspace_role_permissions (workspace_id, role_id, permission) VALUES (?, ?, ?)",
          )
          .bind(workspaceId, editorRoleId, permission),
      );
    }
    statements.push(
      this.db
        .prepare(
          "UPDATE workspace_member_roles SET role_id = ? WHERE workspace_id = ? AND role_id = ?",
        )
        .bind(editorRoleId, workspaceId, operator.id),
      this.db
        .prepare(
          "UPDATE workspace_invitations SET role_id = ? WHERE workspace_id = ? AND role_id = ?",
        )
        .bind(editorRoleId, workspaceId, operator.id),
      this.db
        .prepare(
          "DELETE FROM workspace_role_permissions WHERE workspace_id = ? AND role_id = ?",
        )
        .bind(workspaceId, operator.id),
      this.db
        .prepare(
          "DELETE FROM workspace_roles WHERE workspace_id = ? AND id = ?",
        )
        .bind(workspaceId, operator.id),
    );
  ...
  async ensureWorkspaceRbac(workspaceId: string) {
    ...
    if (
      await this.internalSeedCurrent(
        workspaceId,
        "rbac.seed",
        WORKSPACE_RBAC_SEED_VERSION,
      )
    )
      return;
    ...
    const roles = [
      ...
      [
        "admin",
        "Admin",
        "Workspace administration without owner recovery permissions",
      ],
    ...
    const statements = roles.flatMap(([key, name, description]) => {
      ...
      return [
        ...
        ...this.rolePermissions(key).map((permission) =>
          this.db
            .prepare(
              "INSERT OR IGNORE INTO workspace_role_permissions (workspace_id, role_id, permission) VALUES (?, ?, ?)",
            )
            .bind(workspaceId, roleId, permission),
        ),
      ];
    });
    statements.push(
      this.internalSeedStatement(
        workspaceId,
        "rbac.seed",
        WORKSPACE_RBAC_SEED_VERSION,
      ),
  ...
  async bootstrapOwner(
    ...
    const alreadyMember = await this.db
      .prepare(
        "SELECT user_id FROM workspace_members WHERE workspace_id = ? AND user_id = ? AND status = 'active'",
      )
    ...
    if ((activeMembers?.count ?? 0) === 0) {
      await this.db.batch([
        ...
        this.db
          ...
          .bind(workspaceId, user.id, `${workspaceId}:owner`),
      ]);
      await this.audit(
        workspaceId,
        "rbac.bootstrap.owner",
  ...
  async createOwnerProvisioningRequest(input: {
    workspaceId: string;
    ...
    await this.db.batch([
      this.db
        .prepare(
          "UPDATE workspaces SET name = ?, status = 'provisioning', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        )
        .bind(input.workspaceName, input.workspaceId),
      this.db
        .prepare(
          "UPDATE workspace_provisioning_requests SET status = 'revoked' WHERE workspace_id = ? AND status = 'pending'",
        )
        .bind(input.workspaceId),
      this.db
        .prepare(
          `INSERT INTO workspace_provisioning_requests
    ...
    await this.audit(
      input.workspaceId,
      "workspace.provisioning.owner.requested",
      {
  ...
  async ownerProvisioningStatus(tokenHash: string) {
    const row = await this.db
      .prepare(
        `SELECT workspace_id, owner_email, status, expires_at, consumed_at
  ...
  async consumeOwnerProvisioningToken(
    tokenHash: string,
    user: { id: string; email: string; name?: string } | null,
  ) {
    ...
    const row = await this.db
      .prepare(
        `SELECT id, workspace_id, owner_email, status, expires_at
      FROM workspace_provisioning_requests WHERE token_hash = ? LIMIT 1`,
      )
      .bind(tokenHash)
      .first<{
    ...
    if (Date.parse(row.expires_at) <= Date.now()) {
      await this.db
        .prepare(
          "UPDATE workspace_provisioning_requests SET status = 'expired' WHERE id = ?",
        )
        .bind(row.id)
        .run();
      return { status: "expired" as const };
    }
    if (row.owner_email.toLowerCase() !== user.email.toLowerCase())
    ...
    await this.db.batch([
      ...
      this.db
        ...
        .bind(row.workspace_id, user.id, `${row.workspace_id}:owner`),
      this.db
        .prepare(
          "UPDATE workspace_provisioning_requests SET status = 'consumed', consumed_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'pending'",
        )
        .bind(row.id),
      this.db
        .prepare(
          "UPDATE workspaces SET status = 'active', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        )
        .bind(row.workspace_id),
    ]);
    await this.audit(
      row.workspace_id,
      "workspace.provisioning.owner.consumed",
      { userId: user.id, email: user.email },
      user.id,
    );
  ...
  async memberPermissionOverrides(workspaceId: string, userId: string) {
    const rows = await this.db
      .prepare(
        "SELECT permission, effect FROM workspace_member_permission_overrides WHERE workspace_id = ? AND user_id = ? ORDER BY permission",
      )
      .bind(workspaceId, userId)
  ...
  private async permissionEvaluationForUser(
    workspaceId: string,
    userId: string,
  ) {
    const email = await this.userEmail(userId);
    if (this.env && email && isPlatformAdmin(this.env, { id: userId, email }))
      return {
        permissions: [...workspacePermissions],
        denied: new Set<WorkspacePermission>(),
      };
    const [basePermissions, overrides] = await Promise.all([
      this.permissionsForUser(workspaceId, userId),
      this.db
        .prepare(
          `SELECT overrides.permission, overrides.effect
  ...
  private async userEmail(userId: string) {
    const row = await this.db
      .prepare("SELECT email FROM workspace_members WHERE user_id = ? LIMIT 1")
      .bind(userId)
  ...
  async effectivePermissionsForUser(
    ...
    return (await this.permissionEvaluationForUser(workspaceId, userId))
      .permissions;
  }
  ...
  async hasPermission(
    ...
    const evaluation = await this.permissionEvaluationForUser(
      workspaceId,
      user.id,
    );
    if (evaluation.denied.has(permission)) return false;
    return (
      evaluation.permissions.includes(permission) ||
      evaluation.permissions.includes("workspace.admin")
    );
  }
  ...
  async hasAllPermissions(
    ...
    if (!permissions.length)
      return await this.hasPermission(workspaceId, user, "workspace.read");
    if (!user) return false;
    if (this.env && isPlatformAdmin(this.env, user)) return true;
    const evaluation = await this.permissionEvaluationForUser(
      workspaceId,
    ...
    if (permissions.some((permission) => evaluation.denied.has(permission)))
      return false;
  ...
  async memberSummary(
    ...
    if (!user)
      return { user: null, roles: [], permissions: [], bootstrap: false };
    ...
    const rows = await this.db
      .prepare(
        `SELECT roles.name, roles.system_key
      FROM workspace_member_roles member_roles
    ...
    return {
      user: { id: user.id, email: user.email },
      roles: rows.results,
      permissions: await this.effectivePermissionsForUser(workspaceId, user.id),
      bootstrap: false,
    };
  }
  ...
  async accessibleWorkspaces(
    ...
    if (!user) return [];
    if (this.env && isPlatformAdmin(this.env, user)) {
      return (await this.workspaces()).map((workspace) => ({
        id: workspace.id,
        name: workspace.name,
    ...
    const rows = await this.db
      .prepare(
        `SELECT members.workspace_id, workspaces.name, workspaces.status, roles.name AS role_name, roles.system_key, permissions.permission
      ...
      .all<{
        workspace_id: string;
        name: string;
        status: AccessibleWorkspace["status"];
    ...
    for (const row of rows.results) {
      const current = workspaces.get(row.workspace_id) ?? {
        id: row.workspace_id,
        name: row.name,
        status: row.status,
        roles: [],
      ...
      if (
        ...
        !current.roles.some(
          (role) =>
            role.name === row.role_name && role.system_key === row.system_key,
        )
      )
        current.roles.push({ name: row.role_name, system_key: row.system_key });
      if (row.permission && !current.permissions.includes(row.permission))
        current.permissions.push(row.permission);
      workspaces.set(row.workspace_id, current);
    }
    return [...workspaces.values()].map((workspace) => ({
      ...workspace,
      permissions: workspace.permissions.sort(),
      roles: workspace.roles.sort((left, right) =>
        left.name.localeCompare(right.name),
      ),
    }));
  }

  async workspaceMemberRecords(
    workspaceId: string,
  ): Promise<WorkspaceMemberRecord[]> {
    const [memberRows, permissionRows, overrideRows] = await Promise.all([
      this.db
        .prepare(
          `SELECT members.user_id, members.email, members.status, roles.id AS role_id, roles.name, roles.system_key
        FROM workspace_members members
        LEFT JOIN workspace_member_roles member_roles ON member_roles.workspace_id = members.workspace_id AND member_roles.user_id = members.user_id
        LEFT JOIN workspace_roles roles ON roles.workspace_id = member_roles.workspace_id AND roles.id = member_roles.role_id
      ...
      this.db
        .prepare(
          `SELECT DISTINCT member_roles.user_id, permissions.permission
        FROM workspace_member_roles member_roles
        INNER JOIN workspace_members members
      ...
      this.db
        .prepare(
          "SELECT user_id, permission, effect FROM workspace_member_permission_overrides WHERE workspace_id = ? ORDER BY user_id, permission",
        )
        .bind(workspaceId)
        .all<{
          user_id: string;
    ...
    for (const row of permissionRows.results) {
      ...
      if (member && !member.permissions.includes(row.permission))
        member.permissions.push(row.permission);
    }
    for (const override of overrideRows.results) {
      const member = members.get(override.user_id);
      if (!member) continue;
      member.overrides.push({
      ...
      else if (!member.permissions.includes(override.permission))
        member.permissions.push(override.permission);
    }
    for (const member of members.values()) {
      member.permissions.sort();
      member.overrides.sort((left, right) =>
  ...
  async workspacePermissionsCatalog(
    workspaceId: string,
  ): Promise<WorkspacePermissionRecord[]> {
    const [roles, overrides] = await Promise.all([
      this.db
        .prepare(
          "SELECT permission, COUNT(DISTINCT role_id) AS count FROM workspace_role_permissions WHERE workspace_id = ? GROUP BY permission",
        )
        .bind(workspaceId)
        .all<{ permission: string; count: number }>(),
      this.db
        .prepare(
          "SELECT permission, COUNT(DISTINCT user_id) AS count FROM workspace_member_permission_overrides WHERE workspace_id = ? GROUP BY permission",
        )
        .bind(workspaceId)
        .all<{ permission: string; count: number }>(),
    ]);
    const roleCounts = new Map(
      roles.results.map((row) => [row.permission, row.count]),
    );
    const overrideCounts = new Map(
      overrides.results.map((row) => [row.permission, row.count]),
    );
    return workspacePermissions.map((permission) => ({
      id: permission,
      name: permission,
      category: permission.split(".")[0] ?? "workspace",
      roleCount: roleCounts.get(permission) ?? 0,
      memberOverrideCount: overrideCounts.get(permission) ?? 0,
    }));
  }

  async settingsTab(workspaceId: string, tabId: string) {
    return this.resolveSettingsTab(
      workspaceId,
      tabId,
      new Set(workspacePermissions),
    );
  }

  async reorderSettingsTabs(workspaceId: string, tabIds: string[]) {
    const tabs = await this.settingsTabs(
      workspaceId,
      new Set(workspacePermissions),
    );
    const statements = tabIds.flatMap((tabId, index) => {
      ...
      return [
        this.db
          .prepare(
            "UPDATE workspace_ui_activations SET order_index = ?, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND plugin_id = ? AND contribution_id = ?",
          )
          .bind(index + 1, workspaceId, tab.pluginId, tab.panelContributionId),
      ];
    });
  ...
  async rbacOverview(
    ...
    const [membership, roles, permissions, members, invitations] =
      await Promise.all([
    ...
    return {
      ...
      overview: { roles, permissions, members, invitations },
    };
  }
  ...
  async workspaceInvitations(
    ...
    const rows = await this.db
      .prepare(
        `SELECT invites.id, invites.workspace_id, invites.email, invites.role_id, roles.name AS role_name, invites.status, invites.expires_at, invites.created_at, invites.updated_at
      ...
      .all<{
        ...
        role_id: string | null;
        role_name: string | null;
        status: WorkspaceInvitationRecord["status"];
  ...
  async createWorkspaceInvitation(
    workspaceId: string,
    input: { email: string; roleId?: string | null; expiresAt?: string | null },
    actorId?: string,
  ) {
    await this.ensureWorkspaceRbac(workspaceId);
    ...
    await this.db
      .prepare(
        `INSERT INTO workspace_invitations (id, workspace_id, email, role_id, status, expires_at, created_at, updated_at)
    ...
    return (
      (await this.workspaceInvitations(workspaceId)).find(
        (invite) => invite.id === id,
      ) ?? null
    );
  }
  ...
  async revokeWorkspaceInvitation(
    ...
    await this.db
      .prepare(
        "UPDATE workspace_invitations SET status = 'revoked', updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ? AND status = 'pending'",
      )
  ...
  async workspaceRoles(workspaceId: string): Promise<WorkspaceRoleRecord[]> {
    const [roles, permissionRows] = await Promise.all([
      ...
      this.db
        .prepare(
          "SELECT role_id, permission FROM workspace_role_permissions WHERE workspace_id = ? ORDER BY role_id, permission",
        )
        .bind(workspaceId)
  ...
  async rolePermissionsFor(workspaceId: string, roleId: string) {
    const rows = await this.db
      .prepare(
        "SELECT permission FROM workspace_role_permissions WHERE workspace_id = ? AND role_id = ? ORDER BY permission",
      )
  ...
  async updateWorkspaceRole(
    ...
    const current = await this.db
      .prepare(
        "SELECT id, name, system_key, description FROM workspace_roles WHERE workspace_id = ? AND id = ? LIMIT 1",
      )
      .bind(workspaceId, roleId)
      .first<{
        id: string;
    ...
    if (current.system_key)
      return {
        ...current,
        permissions: await this.rolePermissionsFor(workspaceId, roleId),
      };
    await this.db
      .prepare(
        "UPDATE workspace_roles SET name = COALESCE(?, name), description = COALESCE(?, description), updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
      )
  ...
  async deleteWorkspaceRole(
    ...
    const current = await this.db
      .prepare(
        "SELECT system_key FROM workspace_roles WHERE workspace_id = ? AND id = ? LIMIT 1",
      )
      .bind(workspaceId, roleId)
      .first<{ system_key: string | null }>();
    if (!current || current.system_key) return false;
    await this.db.batch([
      this.db
        .prepare(
          "DELETE FROM workspace_member_roles WHERE workspace_id = ? AND role_id = ?",
        )
        .bind(workspaceId, roleId),
      this.db
        .prepare(
  ...
  async setWorkspaceMemberRoles(
    ...
    await this.db.batch([
      this.db
        .prepare(
          "DELETE FROM workspace_member_roles WHERE workspace_id = ? AND user_id = ?",
        )
        .bind(workspaceId, userId),
      ...roleIds.map((roleId) =>
        this.db
          ...
          .bind(workspaceId, userId, roleId),
      ),
    ]);
    await this.audit(
      workspaceId,
      "rbac.member.roles.assign",
      { userId, roleIds },
      actorId,
    );
  ...
  async setMemberPermissionOverride(
    workspaceId: string,
    userId: string,
    permission: WorkspacePermission,
    effect: "allow" | "deny",
    ...
    await this.db
      .prepare(
        `INSERT INTO workspace_member_permission_overrides (workspace_id, user_id, permission, effect, created_at, updated_at)
      VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  async removeMemberPermissionOverride(
    workspaceId: string,
    userId: string,
    permission: WorkspacePermission,
    actorId?: string,
    ...
    await this.db
      .prepare(
        "DELETE FROM workspace_member_permission_overrides WHERE workspace_id = ? AND user_id = ? AND permission = ?",
      )
      .bind(workspaceId, userId, permission)
      .run();
    await this.audit(
      workspaceId,
      "rbac.member.override.remove",
      { userId, permission },
      actorId,
    );
    return this.workspaceMemberRecords(workspaceId);
  ...
  async assignWorkspaceMemberRoles(
    ...
    if (target?.isPlatformAdmin)
      throw new Error(
        "Platform Superadmin accounts are protected and cannot be assigned workspace memberships.",
      );
    ...
    await this.audit(
      workspaceId,
      "rbac.member.roles.assign",
      { userId, roleIds, createdMembership: !existing },
      actorId,
    );
    return this.workspaceMemberRecords(workspaceId);
  }

  async removeWorkspaceMemberRoles(
    workspaceId: string,
    userId: string,
    roleIds: string[],
    actorId?: string,
    ...
    if (target?.isPlatformAdmin)
      throw new Error(
        "Platform Superadmin accounts are protected and cannot be removed from workspace roles.",
      );
    const currentMember = (await this.workspaceMemberRecords(workspaceId)).find(
      (item) => item.user.id === userId,
    );
    const removingOwner = currentMember?.roles.some(
      (role) => role.systemKey === "owner" && roleIds.includes(role.id),
    );
    if (removingOwner && (await this.workspaceOwnerCount(workspaceId)) <= 1)
      throw new Error(
        "The last active Owner of a workspace cannot be demoted.",
      );
    await this.db.batch(
      roleIds.map((roleId) =>
        this.db
          .prepare(
            "DELETE FROM workspace_member_roles WHERE workspace_id = ? AND user_id = ? AND role_id = ?",
          )
          .bind(workspaceId, userId, roleId),
      ),
    );
  ...
  async removeWorkspaceMember(
    ...
    if (target?.isPlatformAdmin)
      throw new Error(
        "Platform Superadmin accounts are protected and cannot be removed from workspace memberships.",
      );
    const currentMember = (await this.workspaceMemberRecords(workspaceId)).find(
      (item) => item.user.id === userId,
    );
    if (
      currentMember?.roles.some((role) => role.systemKey === "owner") &&
      (await this.workspaceOwnerCount(workspaceId)) <= 1
    ) {
      throw new Error(
        "The last active Owner of a workspace cannot be removed.",
      );
    }
    await this.db.batch([
      this.db
        .prepare(
          "DELETE FROM workspace_member_permission_overrides WHERE workspace_id = ? AND user_id = ?",
        )
        .bind(workspaceId, userId),
      this.db
        .prepare(
  ...
  async upsertPlan(
    ...
    await this.db
      .prepare(
        `INSERT INTO plans (id, name, status, limits_json, created_at, updated_at)
      VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    await this.audit(
      null,
      "plan.upsert",
      { planId: input.id, status: input.status },
      actorId,
    );
  ...
  async deletePlan(planId: string, actorId?: string) {
    await this.db
      .prepare("DELETE FROM user_plan_assignments WHERE plan_id = ?")
      .bind(planId)
      .run();
  ...
  async plan(planId: string): Promise<PlanRecord | null> {
    const row = await this.db
      .prepare(
        "SELECT id, name, status, limits_json, created_at, updated_at FROM plans WHERE id = ? LIMIT 1",
      )
      .bind(planId)
      .first<{
        id: string;
  ...
  async userPlanAssignments(): Promise<UserPlanAssignmentRecord[]> {
    const rows = await this.db
      ...
      .all<{
        ...
        user_id: string;
        plan_id: string;
        status: UserPlanAssignmentRecord["status"];
        starts_at: string | null;
        ends_at: string | null;
        created_at: string;
        updated_at: string;
  ...
  async upsertUserPlanAssignment(
    input: {
      id?: string;
      userId: string;
      planId: string;
    ...
    await this.db
      .prepare(
        `INSERT INTO user_plan_assignments (id, user_id, plan_id, status, starts_at, ends_at, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  async deleteUserPlanAssignment(id: string, actorId?: string) {
    await this.db
      .prepare("DELETE FROM user_plan_assignments WHERE id = ?")
      .bind(id)
      .run();
    await this.audit(
      null,
      "plan.assignment.delete",
      { assignmentId: id },
      actorId,
    );
    return true;
  ...
  async userPlanAssignment(
    id: string,
  ): Promise<UserPlanAssignmentRecord | null> {
    const row = await this.db
      .prepare(
        "SELECT id, user_id, plan_id, status, starts_at, ends_at, created_at, updated_at FROM user_plan_assignments WHERE id = ? LIMIT 1",
      )
      .bind(id)
      .first<{
        id: string;
  ...
  async audit(
    ...
    await this.db
      .prepare(
        "INSERT INTO audit_events (id, workspace_id, actor_id, action, payload_json) VALUES (?, ?, ?, ?, ?)",
      )
      .bind(
        crypto.randomUUID(),
        workspaceId,
    ...
    const rows = await this.db
      .prepare(
        options.includeGlobal
          ? "SELECT id, workspace_id, actor_id, action, payload_json, created_at FROM audit_events WHERE workspace_id = ? OR workspace_id IS NULL ORDER BY created_at DESC LIMIT 200"
          : "SELECT id, workspace_id, actor_id, action, payload_json, created_at FROM audit_events WHERE workspace_id = ? ORDER BY created_at DESC LIMIT 200",
      )
      .bind(workspaceId)
      .all<AuditEventRow>();
    return rows.results.map((row) => ({
      ...
      action: row.action,
      payload: safeJson<Record<string, unknown> | null>(row.payload_json, null),
      createdAt: row.created_at,
    }));
  }

  async workspaceAuditPolicy(
    workspaceId: string,
  ): Promise<WorkspaceAuditPolicyRecord> {
    const settings = await this.getSettings(workspaceId, "platform");
    const raw = settings.auditPolicy;
    const categories =
      raw && typeof raw === "object" && "categories" in raw
  ...
  async saveWorkspaceAuditPolicy(
    workspaceId: string,
    policy: WorkspaceAuditPolicyRecord,
    actorId?: string,
  ) {
    await this.saveSetting(workspaceId, "platform", "auditPolicy", {
      categories: policy,
    });
    await this.audit(
      workspaceId,
      "settings.audit.policy.update",
      { categories: policy },
      actorId,
    );
  ...
  async listDomains(workspaceId: string): Promise<WorkspaceDomain[]> {
    const rows = await this.db
      .prepare(
        "SELECT id, workspace_id, hostname, kind, status, verification_method, verification_instructions_json, publication_id, is_primary, created_at, verified_at, updated_at FROM workspace_domains WHERE workspace_id = ? ORDER BY is_primary DESC, hostname",
      )
      .bind(workspaceId)
      .all<{
        id: string;
  ...
  async updateDomainStatus(
    ...
    await this.db
      .prepare(
        "UPDATE workspace_domains SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
      )
  ...
  async verifyDomain(workspaceId: string, id: string, actorId?: string) {
    await this.db
      .prepare(
        "UPDATE workspace_domains SET status = 'verified', verified_at = COALESCE(verified_at, CURRENT_TIMESTAMP), updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
      )
      .bind(workspaceId, id)
      .run();
  ...
  async mailSummary(workspaceId: string) {
    const [providers, templates, events] = await Promise.all([
      this.listMailProviders(workspaceId),
      this.listMailTemplates(workspaceId),
      this.listMailEvents(workspaceId),
    ]);
    return {
      activeProvider:
        providers.find(
          (provider) => provider.status === "active" && provider.enabled,
        ) ?? null,
      providers,
      templates,
      events,
  ...
  async listMailProviders(
    workspaceId: string,
  ): Promise<MailProviderPublicSummary[]> {
    const rows = await this.db
      .prepare(
        "SELECT id, workspace_id, kind, label, status, enabled, from_name, from_email, reply_to_email, configuration_ref, safe_config_json, is_default_transactional, last_tested_at, last_test_status, last_error, created_at, updated_at FROM workspace_mail_providers WHERE workspace_id = ? ORDER BY is_default_transactional DESC, label",
      )
      .bind(workspaceId)
      .all<MailProviderRow>();
    return rows.results.map((row) => ({
  ...
  async saveMailProvider(
    ...
    await this.db
      ...
      .bind(
        ...
        JSON.stringify(input.safeConfig),
      )
      .run();
    await this.audit(
      workspaceId,
      "mail.provider.configure",
  ...
  async activateMailProvider(
    ...
    await this.db.batch([
      this.db
        .prepare(
          "UPDATE workspace_mail_providers SET status = 'configured', is_default_transactional = 0, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND status = 'active'",
        )
        .bind(workspaceId),
      this.db
        .prepare(
          "UPDATE workspace_mail_providers SET status = 'active', enabled = 1, is_default_transactional = 1, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
        )
        .bind(workspaceId, id),
    ]);
    await this.audit(
      workspaceId,
      "mail.provider.activate",
      { providerId: id },
      actorId,
    );
    return this.mailProviderById(workspaceId, id);
  ...
  async disableMailProvider(workspaceId: string, id: string, actorId?: string) {
    await this.db
      .prepare(
        "UPDATE workspace_mail_providers SET status = 'disabled', enabled = 0, is_default_transactional = 0, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
      )
      .bind(workspaceId, id)
      .run();
    await this.audit(
      workspaceId,
      "mail.provider.disable",
      { providerId: id },
      actorId,
    );
    return this.mailProviderById(workspaceId, id);
  ...
  async listMailTemplates(workspaceId: string): Promise<MailTemplate[]> {
    const rows = await this.db
      .prepare(
        "SELECT id, workspace_id, template_key, subject_template, body_text_template, body_html_template, status, locale, created_at, updated_at FROM workspace_mail_templates WHERE workspace_id = ? ORDER BY template_key, locale",
      )
      .bind(workspaceId)
      .all<{
        id: string;
  ...
  async listMailEvents(workspaceId: string) {
    const rows = await this.db
      .prepare(
        "SELECT id, workspace_id, provider_id, template_key, recipient_hash_or_safe_reference, status, purpose, error_safe, audit_event_id, created_at, completed_at FROM mail_delivery_events WHERE workspace_id = ? ORDER BY created_at DESC LIMIT 100",
      )
      .bind(workspaceId)
      .all<{
        id: string;
        ...
        template_key: MailTemplate["templateKey"] | null;
        recipient_hash_or_safe_reference: string;
    ...
    return rows.results.map((row) => ({
      ...
      templateKey: row.template_key,
      recipientHashOrSafeReference: row.recipient_hash_or_safe_reference,
      status: row.status,
      purpose: row.purpose,
  ...
  async sendMail(
    ...
    const subject =
      request.subject ??
      (template
        ? interpolate(template.subjectTemplate, request.variables ?? {})
        : "Notification");
    const text =
      request.text ??
      (template
        ? interpolate(template.bodyTextTemplate, request.variables ?? {})
        : "");
    const html =
      request.html ??
      (template?.bodyHtmlTemplate
        ? interpolate(template.bodyHtmlTemplate, request.variables ?? {})
        : undefined);
    ...
    await this.db
      .prepare(
        "INSERT INTO mail_delivery_events (id, workspace_id, provider_id, template_key, recipient_hash_or_safe_reference, status, purpose) VALUES (?, ?, ?, ?, ?, 'queued', ?)",
      )
      .bind(
        eventId,
        workspaceId,
    ...
    const message =
      html === undefined
        ? { ...baseRequest, subject, text }
        : { ...baseRequest, subject, text, html };
    const result = await new CoreMailDeliveryAdapter(this.env).deliver(
      provider,
      message,
    );
    if (!result.ok) {
      await this.db
        .prepare(
          "UPDATE mail_delivery_events SET status = 'failed', error_safe = ?, completed_at = CURRENT_TIMESTAMP WHERE id = ?",
        )
        .bind(result.errorSafe ?? "Delivery failed.", eventId)
        .run();
      await this.audit(
        workspaceId,
        "mail.delivery.failed",
        { eventId, providerId: provider.id, purpose: request.purpose },
        actorId,
      );
      return {
        ok: false,
        ...
        errorSafe: result.errorSafe ?? "Delivery failed.",
      };
    }
    await this.db
      .prepare(
        "UPDATE mail_delivery_events SET status = 'sent', completed_at = CURRENT_TIMESTAMP WHERE id = ?",
      )
      .bind(eventId)
      .run();
    await this.audit(
  ...
  async testMailProvider(
    ...
    return this.sendMail(
      workspaceId,
      {
        workspaceId,
        to,
        subject: "v2 mail provider test",
        text: "Your mail provider is configured.",
        purpose: "test",
        variables: {},
      },
      actorId,
  ...
  async declaredCapabilities(pluginId: string) {
    const rows = await this.db
      .prepare(
        "SELECT capability_id FROM plugin_capabilities WHERE plugin_id = ? ORDER BY capability_id",
      )
      .bind(pluginId)
  ...
  async grantedCapabilities(workspaceId: string, pluginId: string) {
    const rows = await this.db
      .prepare(
        "SELECT capability_id FROM workspace_capability_grants WHERE workspace_id = ? AND plugin_id = ? ORDER BY capability_id",
      )
  ...
  async grantCapabilities(
    ...
    await this.db.batch(
      uniqueCapabilities.map((capability) =>
        this.db
          .prepare(
            "INSERT INTO workspace_capability_grants (workspace_id, plugin_id, capability_id, granted_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(workspace_id, plugin_id, capability_id) DO UPDATE SET granted_at = CURRENT_TIMESTAMP",
          )
    ...
    await this.audit(
      ...
      { pluginId, capabilities: uniqueCapabilities },
      undefined,
    );
  ...
  async catalogPlugins(): Promise<CatalogPlugin[]> {
    const rows = await this.db
      .prepare(
        "SELECT plugin_id, manifest_json, category, demo_available, source FROM plugin_catalog ORDER BY category, name",
      )
      .all<{
        plugin_id: string;
        manifest_json: string;
    ...
    return rows.results.map((row) => ({
      ...
      demoAvailable: row.demo_available === 1,
      source: row.source,
    }));
  }

  async catalogPlugin(pluginId: string) {
    return (await this.catalogPlugins()).find(
      (entry) => entry.manifest.id === pluginId,
    );
  }

  async publishedCatalogRelease(
    pluginId: string,
  ): Promise<CatalogRelease | null> {
    const row = await this.db
      .prepare(
        "SELECT id, plugin_id, version, manifest_json, package_object_key, sha256, size_bytes, format, worker_isolation, ui_mode, status, source FROM plugin_catalog_releases WHERE plugin_id = ? AND status = 'published' ORDER BY published_at DESC, updated_at DESC LIMIT 1",
      )
      .bind(pluginId)
      .first<{
        id: string;
        ...
        size_bytes: number;
        format: string;
        worker_isolation: PluginBundle["worker"]["isolation"];
        ui_mode: PluginBundle["ui"]["mode"];
        status: CatalogRelease["status"];
        source: string;
      }>();
    return row
  ...
  async catalogRelease(
    ...
    const row = await this.db
      .prepare(
        "SELECT id, plugin_id, version, manifest_json, package_object_key, sha256, size_bytes, format, worker_isolation, ui_mode, status, source FROM plugin_catalog_releases WHERE plugin_id = ? AND id = ? AND status = 'published' LIMIT 1",
      )
  ...
  async catalogReleaseOptions(
    pluginId: string,
  ): Promise<CatalogReleaseOption[]> {
    const rows = await this.db
      .prepare(
        "SELECT id, version, sha256, size_bytes, worker_isolation, ui_mode, status, source FROM plugin_catalog_releases WHERE plugin_id = ? AND status = 'published' ORDER BY published_at DESC, updated_at DESC",
      )
      .bind(pluginId)
      .all<{
        id: string;
  ...
  async installedById(id: string) {
    const row = await this.db
      .prepare("SELECT manifest_json FROM installed_plugins WHERE id = ?")
      .bind(id)
  ...
  async workspacePlugins(workspaceId: string): Promise<PluginWorkspaceState[]> {
    const rows = await this.db
      .prepare(
        "SELECT workspace_id, plugin_id, active, updated_at FROM workspace_plugins WHERE workspace_id = ? ORDER BY plugin_id",
      )
      .bind(workspaceId)
      .all<{
        workspace_id: string;
  ...
  async pluginCatalogRows(workspaceId: string) {
    ...
    const provisioningTargetOptions =
      await this.cloudflareProvisioningTargetOptions(workspaceId);
    const states = await this.workspacePlugins(workspaceId);
    const stateByPluginId = new Map(
      states.map((state) => [state.pluginId, state]),
    );
    const releases = await Promise.all(
      catalog.map((entry) => this.publishedCatalogRelease(entry.manifest.id)),
    );
    const releaseByPluginId = new Map(
      releases
        .filter((release): release is CatalogRelease => Boolean(release))
        .map((release) => [release.pluginId, release]),
    );
    return Promise.all(
      catalog.map(async (entry) => {
        const state = stateByPluginId.get(entry.manifest.id);
        const release = releaseByPluginId.get(entry.manifest.id);
        const releaseOptions = await this.catalogReleaseOptions(
          entry.manifest.id,
        );
        const deployment = state
          ? await this.pluginRuntimeDeployment(workspaceId, entry.manifest.id)
          : null;
        const demoOperation = entry.manifest.api.operations.find(
          (operation) =>
            /demo/i.test(operation.id) && /install|seed/i.test(operation.id),
        );
        return {
          ...
          runtimeStatus:
            deployment?.runtimeStatus ?? (state ? "pending" : "not installed"),
          runtimeKind:
            deployment?.runtimeKind ?? (state ? "none" : "not installed"),
  ...
  async pluginInstalledRows(workspaceId: string) {
    ...
    const pluginById = new Map(
      plugins.results.map((plugin) => [plugin.id, plugin]),
    );
    return states
      .map((state) => {
  ...
  async activePlugins(workspaceId: string) {
    return (await this.workspacePlugins(workspaceId))
  ...
  async activate(workspaceId: string, pluginId: string) {
    ...
    await this.db.batch([
      this.db
        .prepare(
          `INSERT INTO workspace_plugins (workspace_id, plugin_id, active, activated_at, updated_at) VALUES (?, ?, 1, ?, ?) ON CONFLICT(workspace_id, plugin_id) DO UPDATE SET active = 1, activated_at = excluded.activated_at, updated_at = excluded.updated_at`,
        )
    ...
    return {
      ...
      active: true,
      updatedAt: now,
    } satisfies PluginWorkspaceState;
  }
  ...
  async deactivate(workspaceId: string, pluginId: string) {
    ...
    const row = await this.db
      .prepare(
        "SELECT plugin_id FROM workspace_plugins WHERE workspace_id = ? AND plugin_id = ?",
      )
      .bind(workspaceId, pluginId)
    ...
    await this.db.batch([
      this.db
        .prepare(
          "UPDATE workspace_plugins SET active = 0, updated_at = ? WHERE workspace_id = ? AND plugin_id = ?",
        )
        .bind(now, workspaceId, pluginId),
      this.db
        .prepare(
          "UPDATE plugin_runtime_deployments SET runtime_status = 'disabled', disabled_at = COALESCE(disabled_at, ?), updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND plugin_id = ? AND runtime_status <> 'deleted'",
        )
        .bind(now, workspaceId, pluginId),
    ]);
    return {
  ...
  async uninstall(workspaceId: string, pluginId: string) {
    ...
    await this.db.batch([
      this.db
        .prepare(
          "DELETE FROM workspace_plugins WHERE workspace_id = ? AND plugin_id = ?",
        )
        .bind(workspaceId, pluginId),
      this.db
        .prepare(
          "UPDATE plugin_runtime_deployments SET runtime_status = 'deleted', disabled_at = COALESCE(disabled_at, ?), updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND plugin_id = ?",
        )
        .bind(now, workspaceId, pluginId),
      this.db
        .prepare(
          "DELETE FROM workspace_capability_grants WHERE workspace_id = ? AND plugin_id = ?",
        )
        .bind(workspaceId, pluginId),
      this.db
        .prepare(
          "DELETE FROM workspace_ui_activations WHERE workspace_id = ? AND plugin_id = ?",
        )
        .bind(workspaceId, pluginId),
    ]);
    return {
  ...
  async updateRuntimeDeployment(input: {
    workspaceId: string;
    pluginId: string;
    releaseId: string;
    runtimeKey: string;
    runtimeKind: PluginRuntimeDeployment["runtimeKind"];
    runtimeStatus: PluginRuntimeDeployment["runtimeStatus"];
    deployedVersion?: string | null;
    deploymentId?: string | null;
    activatedAt?: string | null;
    disabledAt?: string | null;
    ...
    await this.db
      .prepare(
        `INSERT INTO plugin_runtime_deployments (workspace_id, plugin_id, release_id, runtime_key, runtime_kind, runtime_status, deployed_version, deployment_id, activated_at, disabled_at, last_error, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
  ...
  async upsertPluginRuntimeDeployment(input: {
    workspaceId: string;
    pluginId: string;
    releaseId: string;
    runtimeKey: string;
  ...
  async pluginRuntimeDeployment(
    workspaceId: string,
    pluginId: string,
  ): Promise<PluginRuntimeDeployment | null> {
    const row = await this.db
      .prepare(
        "SELECT workspace_id, plugin_id, release_id, runtime_key, runtime_kind, runtime_status, deployed_version, deployment_id, created_at, activated_at, disabled_at, last_error FROM plugin_runtime_deployments WHERE workspace_id = ? AND plugin_id = ? LIMIT 1",
      )
      .bind(workspaceId, pluginId)
      .first<{
        workspace_id: string;
        ...
        release_id: string;
        runtime_key: string;
        runtime_kind: PluginRuntimeDeployment["runtimeKind"];
        runtime_status: PluginRuntimeDeployment["runtimeStatus"];
  ...
  async activePluginRuntime(workspaceId: string, pluginId: string) {
    const deployment = await this.pluginRuntimeDeployment(
      workspaceId,
      pluginId,
    );
    return deployment &&
      (deployment.runtimeStatus === "active" ||
        deployment.runtimeStatus === "deployed")
      ? deployment
      : null;
  }
  ...
  private uiContributionsFor(manifest: PluginManifest) {
    const complete = <
      T extends Omit<
        ...
        | "displayOrder"
      > &
        Partial<Pick<PluginUiContribution, "source" | "displayOrder">>,
    >(
      value: T,
    ): PluginUiContribution => ({
      ...value,
    ...
    const declarativeSurfacePage = (
      surface: PluginManifest["contributes"]["surfaces"][number],
    ) => {
      const renderer =
        surface.renderer.mode === "declarative"
          ? surface.renderer.schema
          : undefined;
      if (renderer && typeof renderer === "object" && "templateId" in renderer)
      ...
      return declarativePageContributionSchema.parse({
        ...
        title: surface.title,
        templateId:
          surface.kind === "settings" ? "admin.settings" : "admin.dashboard",
        access: surface.kind === "settings" ? "permission-gated" : "private",
        ...(surface.renderer.mode === "sandbox-frame"
          ? {
              data: {
                rendererType: "sandbox-frame",
                sandbox: surface.renderer,
              },
            }
          : {}),
      });
    ...
    const surfaces = manifest.contributes.surfaces.flatMap((surface) => {
      ...
      return [
        complete({
          ...
          source: "plugin",
          accessMode:
            surface.kind === "settings" ? "permission-gated" : "private",
          zoneId: surface.zone,
          displayOrder: 0,
          templateId:
            surface.renderer.mode === "sandbox-frame"
    ...
    const settingsTabs = manifest.contributes.settingsTabs.map((tab) =>
      complete({
        ...
        source: "plugin",
        accessMode: tab.requiredPermission
          ? ("permission-gated" as const)
          : ("private" as const),
    ...
    const settingsPanels = manifest.contributes.settingsPanels.map((panel) =>
      complete({
        ...
        source: "plugin",
        accessMode: panel.requiredPermission
          ? ("permission-gated" as const)
          : ("private" as const),
        ...
        schema: declarativePageContributionSchema.parse({
          ...panel.schema,
          dataSources: panel.dataSources.length
            ? panel.dataSources
            : panel.schema.dataSources,
          actions: panel.actions.length ? panel.actions : panel.schema.actions,
        }),
        requiredPermission: panel.requiredPermission ?? null,
        version: manifest.version,
      }),
    );
    const explicitTabIds = new Set(
      manifest.contributes.settingsTabs.map((tab) => tab.id),
    );
    const explicitPanelIds = new Set(
      manifest.contributes.settingsPanels.map((panel) => panel.id),
    );
    const settingsSurfaceTabs = manifest.contributes.surfaces
      .filter((surface) => surface.kind === "settings")
      .flatMap((surface, index) => {
        const page = declarativeSurfacePage(surface);
        const tabId = `${surface.id}.settings-tab`;
        const panelId = `${surface.id}.settings-panel`;
        if (explicitTabIds.has(tabId) || explicitPanelIds.has(panelId))
          return [];
        ...
        const panel = settingsPanelContributionSchema.parse({
          ...
          tabId,
          templateId:
            page.templateId === "admin.table" ||
            page.templateId === "admin.form" ||
            page.templateId === "admin.dashboard"
              ? page.templateId
              : "admin.settings",
          schema: page,
  ...
  async ensurePlatformSettingsContributions(workspaceId: string) {
    if (
      await this.internalSeedCurrent(
        workspaceId,
        "settings.seed",
        PLATFORM_SETTINGS_SEED_VERSION,
      )
    )
      return;
    await this.ensureWorkspace(workspaceId);
    ...
    await this.db
      .prepare(
        `INSERT INTO installed_plugins (id, name, version, manifest_json, worker_isolation, ui_mode, updated_at)
      ...
      .bind(JSON.stringify(manifest))
      .run();
    await this.db
      .prepare(
        `INSERT INTO workspace_plugins (workspace_id, plugin_id, active, activated_at, updated_at)
    ...
    const statements = obsoleteSettingsContributionIds.flatMap(
      (contributionId) => [
        this.db
          .prepare(
            "DELETE FROM workspace_ui_activations WHERE workspace_id = ? AND plugin_id = 'platform' AND contribution_id = ?",
          )
          .bind(workspaceId, contributionId),
        this.db
          .prepare(
            "DELETE FROM plugin_ui_contributions WHERE plugin_id = 'platform' AND contribution_id = ?",
          )
          .bind(contributionId),
      ],
    );
    for (const item of platformSettingsTabs()) {
      statements.push(
        this.db
          .prepare(
            `INSERT INTO plugin_ui_contributions
        ...
        ON CONFLICT(plugin_id, contribution_id, version) DO UPDATE SET
          ...
          .bind(
            `platform:${item.tab.id}:0.0.0`,
            item.tab.id,
            item.tab.label,
            item.tab.icon ?? null,
            item.tab.displayOrder,
            JSON.stringify(
              config(false, { canDelete: false, canMoveSection: false }),
            ),
            JSON.stringify(item.tab),
            item.tab.requiredPermission ?? null,
          ),
      );
      statements.push(
        this.db
        ...
        ON CONFLICT(plugin_id, contribution_id, version) DO UPDATE SET schema_json = excluded.schema_json, template_id = excluded.template_id, updated_at = CURRENT_TIMESTAMP`,
          ...
          .bind(
            `platform:${item.panel.id}:0.0.0`,
            item.panel.id,
            `settings.panel.${item.tab.id}`,
            item.panel.schema.title,
            item.tab.displayOrder,
            JSON.stringify(
              config(false, {
                canHide: false,
            ...
            item.panel.templateId,
            JSON.stringify(item.panel),
            item.panel.requiredPermission ?? null,
          ),
      );
      statements.push(
        this.db
          .prepare(
            `INSERT INTO workspace_ui_activations
    ...
    return [
      {
        ...
        componentId: "platform.home",
        configurable: config(false, { canDelete: false }),
        visibleInNavigation: true,
      },
    ];
  }

  private platformShellPageSchema(
    page: ReturnType<CoreRepository["platformShellPages"]>[number],
  ) {
    return declarativePageContributionSchema.parse({
      ...
      slots: [
        {
          id: `${page.contributionId}.header`,
          slot: "header",
          blocks: [{ type: "heading", text: page.label, level: "h2" }],
        },
      ],
      data: {},
    });
  ...
  async ensurePlatformShellContributions(workspaceId: string) {
    ...
    const statements = this.platformShellPages().flatMap((page) => [
      ...
      this.db
        .prepare(
          `INSERT OR IGNORE INTO workspace_ui_activations
  ...
  async installManifest(manifest: PluginManifest, bundle?: PluginBundle) {
    ...
    const statements = [
      ...
      this.db
        .prepare("DELETE FROM plugin_capabilities WHERE plugin_id = ?")
        .bind(manifest.id),
      this.db
        .prepare("DELETE FROM plugin_permissions WHERE plugin_id = ?")
        .bind(manifest.id),
      this.db
        .prepare(
          "DELETE FROM plugin_ui_contributions WHERE plugin_id = ? AND version = ?",
        )
        .bind(manifest.id, manifest.version),
      ...manifest.capabilities.map((capability) =>
        this.db
          .prepare(
            "INSERT INTO plugin_capabilities (plugin_id, capability_id, description, risk) VALUES (?, ?, ?, ?)",
          )
          .bind(
            manifest.id,
            capability.id,
      ...
      ...manifest.capabilities.map((capability) =>
        this.db
          .prepare(
            "INSERT INTO plugin_permissions (plugin_id, permission, description, risk) VALUES (?, ?, ?, ?)",
          )
          .bind(
            manifest.id,
            capability.id,
      ...
      ...uiContributions.map((contribution) =>
        ...
        ON CONFLICT(plugin_id, contribution_id, version) DO UPDATE SET
          ...
          .bind(
            `${manifest.id}:${contribution.contributionId}:${manifest.version}`,
            manifest.id,
            contribution.contributionId,
            contribution.contributionType,
            contribution.accessMode,
            contribution.zoneId,
            contribution.schema.title,
            JSON.stringify(config(false)),
            contribution.templateId,
            JSON.stringify(contribution.schema),
    ...
    if (packageData) {
      statements.push(
        this.db
          .prepare(
            "INSERT OR REPLACE INTO plugin_packages (id, plugin_id, version, object_key, sha256, size_bytes, format) VALUES (?, ?, ?, ?, ?, ?, ?)",
          )
          .bind(
            `${manifest.id}@${manifest.version}:${packageData.sha256}`,
            manifest.id,
            manifest.version,
            packageData.objectKey,
            packageData.sha256,
            packageData.sizeBytes,
            packageData.format,
          ),
      );
    }
  ...
  async seedCatalogPlugin(plugin: CatalogPlugin, release?: CatalogRelease) {
    await this.db
      .prepare(
        `INSERT INTO plugin_catalog (plugin_id, name, version, manifest_json, category, demo_available, source, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
  ...
  async createPublicPolicy(
    ...
    await this.db
      .prepare(
        "INSERT INTO public_access_policies (id, workspace_id, name, access, authentication_mode, rules_json, allowed_operations_json, enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      )
      .bind(
        id,
        workspaceId,
  ...
  async createPublication(publication: WorkspacePublication) {
    const route = routeMetadata(
      publication.routePattern ?? publication.publicPath,
    );
    await this.db
      .prepare(
        `INSERT INTO workspace_publications (id, workspace_id, plugin_id, contribution_kind, publication_type, contribution_id, public_path, route_pattern, route_kind, route_priority, parameter_names_json, title, template_id, schema_json, status, policy_id, access, authentication_mode, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
      )
      .bind(
        publication.id,
        ...
        publication.contributionKind,
        publication.publicationType ?? publication.contributionKind,
        publication.contributionId,
        publication.publicPath,
        route.pattern,
        route.kind,
        publication.routePriority ?? route.staticSegments,
        JSON.stringify(route.parameterNames),
        publication.title,
        publication.templateId ?? "public.page",
        JSON.stringify(publication.schema ?? {}),
        publication.status,
        publication.policyId,
        publication.access,
        publication.authenticationMode ?? "anonymous",
      )
      .run();
  }
  ...
  async publish(publicationId: string) {
    await this.db
      .prepare(
        "UPDATE workspace_publications SET status = 'published', published_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
      )
      .bind(publicationId)
      .run();
  }
  ...
  async deactivatePublication(workspaceId: string, publicationId: string) {
    await this.db
      .prepare(
        "UPDATE workspace_publications SET status = 'disabled', updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
      )
  ...
  async publications(
    ...
    const rows = await this.db
      .prepare(
        "SELECT id, workspace_id, plugin_id, contribution_kind, publication_type, contribution_id, public_path, route_pattern, route_kind, route_priority, parameter_names_json, title, template_id, schema_json, status, policy_id, access, authentication_mode, created_at, published_at, updated_at FROM workspace_publications WHERE workspace_id = ? ORDER BY updated_at DESC",
      )
      .bind(workspaceId)
      .all<WorkspacePublicationRow>();
    return rows.results.map((row) => {
      const schemaResult = declarativePageContributionSchema.safeParse(
        safeJson<Record<string, unknown>>(row.schema_json, {}),
      );
      return {
        id: row.id,
        workspaceId: row.workspace_id,
  ...
  async publicDeliveryForPath(
    path: string,
  ): Promise<PublicDelivery | undefined> {
    const rows = await this.db
      .prepare(
        `SELECT publications.id, publications.workspace_id, publications.plugin_id, publications.contribution_kind, publications.publication_type, publications.contribution_id, publications.public_path, publications.route_pattern, publications.route_kind, publications.route_priority, publications.parameter_names_json, publications.title, publications.template_id, publications.schema_json, publications.status, publications.policy_id, publications.access, publications.authentication_mode, policies.enabled AS policy_enabled, plugins.manifest_json
    ...
    for (const row of rows.results) {
      ...
      const contribution = allPublicContributions(manifest).find(
        (item) => item.id === row.contribution_id,
      );
      const storedPage = declarativePageContributionSchema.safeParse(
        safeJson<Record<string, unknown>>(row.schema_json, {}),
      );
      const page = storedPage.success
        ? storedPage.data
        : declarativePageContributionSchema.parse({
            ...
            templateId: row.template_id,
            access:
              row.access === "anonymous" ? "public-candidate" : "authenticated",
          });
      return {
        publication: {
          id: row.id,
  ...
  async publishWorkspaceContribution(input: {
    workspaceId: string;
    pluginId: string;
    contributionKind: PublicationKind;
    contributionId: string;
    ...
    const publicContribution = allPublicContributions(manifest).find(
      (item) => item.id === input.contributionId,
    );
    const authenticationMode: "anonymous" | "customer" | "verified" =
      input.access === "anonymous" ? "anonymous" : "customer";
    const page = declarativePageContributionSchema.parse({
      ...
      access:
        input.access === "authenticated" ? "authenticated" : "public-candidate",
    });
  ...
  async settingsTabs(
    workspaceId: string,
    permissions: Set<string> = new Set(workspacePermissions),
  ) {
    await this.ensurePlatformSettingsContributions(workspaceId);
    const rows = await this.db
      .prepare(
        `SELECT contributions.plugin_id, contributions.contribution_id, contributions.schema_json, contributions.required_permission, activations.order_index, plugins.name
    ...
    return rows.results.flatMap((row) => {
      if (
        ...
        !permissions.has(row.required_permission) &&
      ...
      return [
        {
          ...tab,
          ownerName: row.name,
          orderIndex: row.order_index ?? tab.displayOrder,
        },
      ];
    });
  }

  async resolveSettingsTab(
    workspaceId: string,
    tabId: string,
    permissions: Set<string>,
  ): Promise<SettingsTabResolution | null> {
    const tabs = await this.settingsTabs(workspaceId, permissions);
    const tab = tabs.find((entry) => entry.id === tabId);
    if (!tab) return null;
    const row = await this.db
      .prepare(
        `SELECT contributions.schema_json, contributions.required_permission
      FROM plugin_ui_contributions contributions
      INNER JOIN workspace_plugins workspace_plugins ON workspace_plugins.plugin_id = contributions.plugin_id AND workspace_plugins.workspace_id = ? AND workspace_plugins.active = 1
      LEFT JOIN workspace_ui_activations activations ON activations.workspace_id = ? AND activations.plugin_id = contributions.plugin_id AND activations.contribution_id = contributions.contribution_id
  ...
  async uiContributions(workspaceId: string, permissions: Set<string>) {
    const rows = await this.db
      ...
      .all<{
        plugin_id: string;
        contribution_id: string;
        contribution_type: PluginUiContribution["contributionType"];
        source: UiSource;
        access_mode: AccessMode;
        zone_id: string | null;
        default_path: string | null;
        visible_in_navigation: number;
        label: string | null;
        icon: string | null;
        navigation_section: NavigationSection | null;
        display_order: number;
        renderer_mode: UiRendererMode;
        component_id: string | null;
        configurable_json: string;
        template_id: string;
        schema_json: string;
        required_permission: string | null;
        version: string;
        ...
        row.contribution_type === "menu"
          ? (() => {
              ...
              return declarativePageContributionSchema.parse({
                ...
                title: tab.label,
                templateId: "admin.settings",
                access: row.required_permission
                  ? "permission-gated"
                  : "private",
                data: { settingsTab: tab },
              });
      ...
      return [
        {
          ...
          configurable: safeJson<UiConfigurable>(
            row.configurable_json,
            config(false),
          ),
          templateId: row.template_id,
          schema,
  ...
  async workspaceUiSurfaces(workspaceId: string) {
    ...
    return (await this.uiContributions(workspaceId, permissions))
      .filter(
        (item) =>
          item.contributionType === "surface" ||
          item.contributionType === "slot",
      )
      .map((item) => {
        const schema = item.schema as DeclarativePageContribution & {
          data?: { sandbox?: { entry?: string } };
        };
        return {
          id: item.contributionId,
          title: item.schema.title,
          zone: item.zoneId ?? "workspace.main",
          kind: item.contributionType === "surface" ? "page" : "panel",
          renderer:
            item.rendererMode === "sandbox-frame"
              ? {
                  mode: "sandbox-frame" as const,
                  entry: schema.data?.sandbox?.entry ?? "",
                }
              : { mode: "declarative" as const, schema: item.schema },
        };
      });
  }

  async resolveSandboxSurface(
    workspaceId: string,
    surfaceId: string,
  ): Promise<SandboxSurfaceAsset | null> {
    const row = await this.db
      .prepare(
        `SELECT contributions.plugin_id, packages.object_key, contributions.schema_json
      FROM plugin_ui_contributions contributions
  ...
  async runtimeContribution(
    ...
    return contribution
      ? ({
          ...
          contributionId,
          page: contribution.schema,
          requiredPermission: contribution.requiredPermission,
        } satisfies RuntimeContributionResolution)
      : null;
  }
  ...
  async privateRuntimeContribution(
    workspaceId: string,
    contributionId: string,
  ) {
  ...
  async publicRuntimeContribution(workspaceId: string, contributionId: string) {
    const publication = (await this.publications(workspaceId)).find(
      (item) =>
    ...
    const page =
      publication.schema ??
      declarativePageContributionSchema.parse({
        id: publication.contributionId,
        title: publication.title,
        templateId: publication.templateId ?? "public.contentPage",
        access:
          publication.access === "anonymous"
            ? "public-candidate"
            : "authenticated",
        ...(publicSurface ? { data: { publicSurfaceId: publicSurface } } : {}),
      });
    const policyRow = publication.policyId
      ? await this.db
          .prepare(
            "SELECT id, access, authentication_mode, allowed_operations_json, enabled FROM public_access_policies WHERE id = ? LIMIT 1",
          )
          .bind(publication.policyId)
          .first<{
            id: string;
            access: PublicContributionAccess;
            authentication_mode: "anonymous" | "customer" | "verified";
    ...
    return {
      ...
      policy: policyRow
        ? {
            id: policyRow.id,
            access: policyRow.access,
            authenticationMode: policyRow.authentication_mode,
            allowedOperations: safeJson<string[]>(
              policyRow.allowed_operations_json,
              [],
            ),
            enabled: policyRow.enabled === 1,
          }
        : undefined,
    };
  }

  async updatePluginUiContribution(
    workspaceId: string,
    contributionId: string,
    changes: {
      enabled?: boolean;
    ...
    const updated = await this.updateInterfaceContribution(
      workspaceId,
      contributionId,
      changes,
    );
    if (updated)
      await this.audit(
        workspaceId,
        "plugin.ui.contribution.update",
        { contributionId, changes },
        actorId,
      );
    return updated;
  }
    ...
    return contributions
      .filter(
        (item) =>
          ...
          item.visibleInNavigation !== false,
      )
      ...
      .map((item) => ({
        ...
        label: item.label ?? item.schema.title,
        ...(item.icon ? { icon: item.icon } : {}),
        section: item.navigationSection!,
        ...
        ...(item.requiredPermission
          ...
          : {}),
        visibleInNavigation: item.visibleInNavigation,
      }));
  }

  async interfaceContributions(
    workspaceId: string,
  ): Promise<InterfaceContributionRow[]> {
    const contributions = await this.uiContributions(
      workspaceId,
      new Set(workspacePermissions),
    );
    return contributions
      ...
      .map((item) => ({
        ...
        label: item.label ?? item.schema.title,
        ...(item.icon ? { icon: item.icon } : {}),
        section: item.navigationSection ?? "administration",
        displayOrder: item.displayOrder,
        rendererMode: item.rendererMode,
        ...(item.componentId ? { componentId: item.componentId } : {}),
        source: item.source,
        ...(item.requiredPermission
          ? { requiredPermission: item.requiredPermission }
          : {}),
        kind: item.contributionType,
        active: true,
        visibleInNavigation: item.visibleInNavigation !== false,
        status: "active",
  ...
  async createManualPage(workspaceId: string, input: ManualPageInput) {
    ...
    if (!slug || reservedShellPaths.has(path))
    ...
    const schema = declarativePageContributionSchema.parse({
      ...
      slots: [
        {
          id: `${contributionId}.body`,
          slot: "body",
          blocks: input.blocks ?? [
            { type: "heading", text: input.title.trim(), level: "h2" },
          ],
        },
      ],
    });
    await this.db.batch([
      this.db
        ...
        .bind(
          `platform:${contributionId}:manual`,
          contributionId,
          path,
          input.label?.trim() || input.title.trim(),
          input.icon?.trim() || null,
          input.navigationSection ?? "administration",
          input.displayOrder ?? 500,
          JSON.stringify(config(true)),
          JSON.stringify(schema),
        ),
      this.db
        .prepare(
          `INSERT INTO workspace_ui_activations (workspace_id, plugin_id, contribution_id, enabled, visible_in_navigation, zone_override, order_index, configuration_json)
        ...
        .bind(
          ...
          contributionId,
          input.enabled === false ? 0 : 1,
          input.visibleInNavigation === false ? 0 : 1,
          input.displayOrder ?? 500,
        ),
    ]);
  ...
  async updateInterfaceContribution(
    workspaceId: string,
    contributionId: string,
    changes: {
      enabled?: boolean;
    ...
    if (protectedPlatformPages.has(contributionId) && changes.enabled === false)
      throw new Error("Protected platform pages cannot be disabled.");
    const current = (await this.interfaceContributions(workspaceId)).find(
      (item) => item.id === contributionId,
    );
    if (!current) return null;
    await this.db
      ...
      .bind(
        ...
        current.pluginId,
        contributionId,
        changes.enabled === false ? 0 : 1,
        changes.visibleInNavigation === false ? 0 : 1,
        changes.label ?? current.label,
        changes.icon ?? current.icon ?? null,
        changes.section ?? current.section,
        changes.displayOrder ?? current.displayOrder,
      )
      .run();
    return (
      (await this.interfaceContributions(workspaceId)).find(
        (item) => item.id === contributionId,
      ) ?? null
    );
  }

  async deleteInterfaceContribution(
    workspaceId: string,
    contributionId: string,
  ) {
    ...
    await this.db.batch([
      this.db
        .prepare(
          "DELETE FROM workspace_ui_activations WHERE workspace_id = ? AND plugin_id = ? AND contribution_id = ?",
        )
        .bind(workspaceId, current.pluginId, contributionId),
      this.db
        .prepare(
          "DELETE FROM plugin_ui_contributions WHERE plugin_id = ? AND contribution_id = ?",
        )
        .bind(current.pluginId, contributionId),
    ]);
    return true;
  }
  ...
  async getSettings(workspaceId: string, scope: SettingScope) {
    const rows = await this.db
      .prepare(
        "SELECT key, value_json FROM workspace_settings WHERE workspace_id = ? AND scope = ? ORDER BY key",
      )
      .bind(workspaceId, scope)
      .all<{ key: string; value_json: string }>();
    return Object.fromEntries(
      rows.results.map((row) => [row.key, JSON.parse(row.value_json)]),
    );
  }
  ...
  async saveSetting(
    ...
    await this.db
      .prepare(
        "INSERT INTO workspace_settings (workspace_id, scope, key, value_json, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(workspace_id, scope, key) DO UPDATE SET value_json = excluded.value_json, updated_at = CURRENT_TIMESTAMP",
      )
      .bind(workspaceId, scope, key, JSON.stringify(value))
      .run();
  }
  ...
  private cloudflareConnectionRecord(
    row: WorkspaceCloudflareConnectionRow,
  ): WorkspaceCloudflareConnection {
    const allowedZones = safeJson<
      Array<{ id?: string; name?: string; status?: string }>
    >(row.allowed_zones_json, []);
    return {
      id: row.id,
      workspaceId: row.workspace_id,
      label: row.label,
      ...
      allowedZones,
      zoneCount: allowedZones.length,
      status: row.status,
      defaultForProvisioning: row.default_for_provisioning === 1,
  ...
  async listCloudflareConnections(
    workspaceId: string,
  ): Promise<WorkspaceCloudflareConnection[]> {
    const rows = await this.db
      .prepare(
        ...
        WHERE workspace_id = ?
        ORDER BY default_for_provisioning DESC, label`,
      )
      .bind(workspaceId)
      .all<WorkspaceCloudflareConnectionRow>();
  ...
  async cloudflareProvisioningTargetOptions(workspaceId: string) {
    ...
    return [
      ...
      ...connections
        ...
        .map((connection) => ({
          value: `cloudflare:${connection.id}`,
          label: connection.defaultForProvisioning
  ...
  async upsertCloudflareConnection(
    ...
    input: {
      ...
      configurationRef?: string | null;
      tokenHint?: string | null;
      status?: WorkspaceCloudflareConnection["status"] | null;
      defaultForProvisioning?: boolean;
    },
    actorId?: string,
  ) {
    ...
    const slug = label
      ...
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "");
    const id = input.id?.trim() || `cloudflare:${workspaceId}:${slug || crypto.randomUUID()}`;
    if (input.defaultForProvisioning) {
      await this.db
        .prepare(
          "UPDATE workspace_cloudflare_accounts SET default_for_provisioning = 0, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ?",
        )
        .bind(workspaceId)
        .run();
    }
    await this.db
      .prepare(
        `INSERT INTO workspace_cloudflare_accounts
          (id, workspace_id, label, account_id, configuration_ref, token_hint, status, default_for_provisioning, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(id) DO UPDATE SET
          label = excluded.label,
          account_id = excluded.account_id,
          configuration_ref = excluded.configuration_ref,
          token_hint = COALESCE(excluded.token_hint, workspace_cloudflare_accounts.token_hint),
          status = excluded.status,
          default_for_provisioning = excluded.default_for_provisioning,
          updated_at = CURRENT_TIMESTAMP`,
      )
      .bind(
        id,
        ...
        input.configurationRef?.trim() || null,
        input.tokenHint?.trim() || null,
        input.status ?? "untested",
        input.defaultForProvisioning ? 1 : 0,
      )
      .run();
    await this.audit(
      workspaceId,
      "cloudflare.connection.upsert",
  ...
  async deleteCloudflareConnection(
    ...
    await this.db
      .prepare(
        "DELETE FROM workspace_cloudflare_accounts WHERE workspace_id = ? AND id = ?",
      )
      .bind(workspaceId, connectionId)
      .run();
    await this.audit(
      workspaceId,
      "cloudflare.connection.delete",
      { connectionId },
      actorId,
    );
    return true;
  ...
  private async cloudflareClientForConnection(
    workspaceId: string,
    connectionId: string,
  ) {
    ...
    const secret =
      parseCloudflareConnectionConfigs(this.env)[connection.configurationRef];
    const apiToken = secret?.apiToken ?? secret?.token;
    const accountId = connection.accountId ?? secret?.accountId;
    if (!apiToken)
      throw new Error("Cloudflare API token is not configured server-side.");
    if (!accountId)
      throw new Error("Cloudflare account id is not configured.");
    return {
      connection,
      client: new CloudflarePlatformClient({
        accountId,
        apiToken,
  ...
  async testCloudflareConnection(
    ...
    try {
      const { client, accountId } = await this.cloudflareClientForConnection(
        workspaceId,
        connectionId,
      );
      const account = await client.request<{ id?: string; name?: string }>(
        client.accountPath(""),
      );
      await this.db
        .prepare(
          "UPDATE workspace_cloudflare_accounts SET account_id = ?, status = 'ok', last_checked_at = CURRENT_TIMESTAMP, last_error = NULL, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
        )
        .bind(account.id ?? accountId, workspaceId, connectionId)
        .run();
      await this.audit(
        workspaceId,
        "cloudflare.connection.test",
        { connectionId, accountId: account.id ?? accountId },
        actorId,
      );
      return {
        ok: true,
      ...
      const message =
        error instanceof Error ? error.message : "Cloudflare test failed.";
      await this.db
        .prepare(
          "UPDATE workspace_cloudflare_accounts SET status = 'error', last_checked_at = CURRENT_TIMESTAMP, last_error = ?, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
        )
        .bind(message, workspaceId, connectionId)
        .run();
      await this.audit(
        workspaceId,
  ...
  async syncCloudflareConnectionZones(
    workspaceId: string,
    connectionId: string,
    actorId?: string,
  ) {
    try {
      ...
      await this.db
        .prepare(
          "UPDATE workspace_cloudflare_accounts SET allowed_zones_json = ?, status = 'ok', last_checked_at = CURRENT_TIMESTAMP, last_error = NULL, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
        )
        .bind(JSON.stringify(zones), workspaceId, connectionId)
        .run();
      await this.audit(
        workspaceId,
        "cloudflare.connection.zones.sync",
        { connectionId, zoneCount: zones.length },
        actorId,
      );
      return {
        ok: true,
      ...
      const message =
        error instanceof Error ? error.message : "Cloudflare zones sync failed.";
      await this.db
        .prepare(
          "UPDATE workspace_cloudflare_accounts SET status = 'error', last_checked_at = CURRENT_TIMESTAMP, last_error = ?, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?",
        )
      ...
      await this.audit(
        workspaceId,
        "cloudflare.connection.zones.sync.failed",
        { connectionId, error: message },
        actorId,
      );
      return { ok: false, error: message };
  ...
  async saveGeneralSettings(
    ...
    if (typeof input.workspaceName === "string" && input.workspaceName.trim())
      await this.db
        ...
        .bind(input.workspaceName.trim(), workspaceId)
        .run();
    for (const key of [
      "language",
      "timezone",
      ...
      if (key in input)
        await this.saveSetting(workspaceId, "platform", key, input[key]);
    }
    await this.audit(
      workspaceId,
  ...
  async themeSettings(workspaceId: string) {
    const preset = await this.db
      .prepare(
        "SELECT token_value FROM workspace_theme_tokens WHERE workspace_id = ? AND token_key = 'theme_preset' LIMIT 1",
      )
      .bind(workspaceId)
      .first<{ token_value: string }>();
    const accent = await this.db
      .prepare(
        "SELECT token_value FROM workspace_theme_tokens WHERE workspace_id = ? AND token_key = 'accent' LIMIT 1",
      )
      .bind(workspaceId)
      .first<{ token_value: string }>();
    const density = await this.db
      .prepare(
        "SELECT token_value FROM workspace_theme_tokens WHERE workspace_id = ? AND token_key = 'density' LIMIT 1",
      )
      .bind(workspaceId)
      .first<{ token_value: string }>();
    const fontFamily = await this.db
      .prepare(
        "SELECT token_value FROM workspace_theme_tokens WHERE workspace_id = ? AND token_key = 'fontFamily' LIMIT 1",
      )
      .bind(workspaceId)
      .first<{ token_value: string }>();
    return {
      theme_preset: preset?.token_value ?? "default",
      accent: accent?.token_value ?? "#3b82f6",
      density: density?.token_value ?? "comfortable",
      fontFamily: fontFamily?.token_value ?? "Inter",
    };
  }
  ...
  async saveThemeSettings(
    ...
    const keys = ["theme_preset", "accent", "density", "fontFamily"] as const;
    const now = new Date().toISOString();
    for (const key of keys) {
      if (key in input && typeof input[key] === "string") {
        await this.db
          .prepare(
            `
          INSERT INTO workspace_theme_tokens (workspace_id, token_key, token_value, scope, updated_at)
          VALUES (?, ?, ?, 'both', ?)
          ON CONFLICT(workspace_id, token_key, scope) DO UPDATE SET token_value = excluded.token_value, updated_at = excluded.updated_at
        `,
          )
          .bind(workspaceId, key, input[key] as string, now)
          .run();
      }
    }
    await this.audit(
    ...
    return this.themeSettings(workspaceId);
  }
}
