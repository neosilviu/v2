...
export const pluginUiContributions = sqliteTable("plugin_ui_contributions", {
  ...
  source: text("source", { enum: ["platform", "plugin", "manual"] }).notNull().default("plugin"),
  accessMode: text("access_mode", { enum: ["private", "authenticated", "permission-gated", "public-candidate"] }).notNull().default("private"),
  zoneId: text("zone_id"),
  defaultPath: text("default_path"),
  label: text("label"),
  icon: text("icon"),
  navigationSection: text("navigation_section", { enum: ["user", "administration"] }),
  displayOrder: integer("display_order").notNull().default(0),
  rendererMode: text("renderer_mode", { enum: ["native", "declarative", "sandbox-frame"] }).notNull().default("declarative"),
...
export const workspaceUiActivations = sqliteTable("workspace_ui_activations", {
  ...
  contributionId: text("contribution_id").notNull(),
  enabled: integer("enabled", { mode: "boolean" }).notNull().default(true),
  visibleInNavigation: integer("visible_in_navigation", { mode: "boolean" }).notNull().default(true),
  labelOverride: text("label_override"),
  iconOverride: text("icon_override"),
  navigationSectionOverride: text("navigation_section_override", { enum: ["user", "administration"] }),
