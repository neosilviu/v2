...
type PlatformSettingsRuntimeContext = Context<{
  Bindings: CoreEnv;
  Variables: { user: CoreSessionUser | null; internal: boolean };
}>;

type AuthAdminJson = <T>(
  path: string,
  init?: { method?: string; headers?: HeadersInit; body?: string },
) => Promise<T>;
type AuthForwardResponse = (
  path: string,
...
export function toWorkspaceStatus(
  value: unknown,
): WorkspaceRecord["status"] | null {
...
export function toPlanStatus(
  value: unknown,
): "active" | "draft" | "disabled" | null {
  return value === "active" || value === "draft" || value === "disabled"
    ? value
    : null;
}

function toCloudflareConnectionStatus(
  value: unknown,
): "untested" | "ok" | "error" | "disabled" | null {
...
async function dispatchPluginOperation(
  ...
  if (!deployment) {
    await repo.audit(
      workspaceId,
      "marketplace.plugin.operation.unavailable",
    ...
    return {
      ok: false as const,
      status: 503 as const,
      error: "Plugin runtime is not active.",
    };
  }
  if (!c.env.PLUGIN_RUNTIME) {
    await repo.audit(
      workspaceId,
      "marketplace.plugin.operation.unavailable",
      { pluginId, operationId, runtimeKey: deployment.runtimeKey },
      actorId ?? undefined,
    );
    return {
      ok: false as const,
      status: 501 as const,
      error: "Plugin runtime dispatch is not configured.",
    };
  }
  ...
  if (!response.ok) {
    await repo.audit(
      workspaceId,
      "marketplace.plugin.operation.denied",
      {
        pluginId,
        operationId,
        runtimeKey: deployment.runtimeKey,
    ...
    return {
      ok: false as const,
      status: response.status === 404 ? (404 as const) : (403 as const),
      error: "Plugin runtime rejected the operation.",
    };
  }
  await repo.audit(
    workspaceId,
    "marketplace.plugin.operation.execute",
    { pluginId, operationId, runtimeKey: deployment.runtimeKey },
    actorId ?? undefined,
  );
...
export async function platformSettingsRuntimeData(
  ...
  switch (dataSourceId) {
    ...
    case "platform.settings.theme.read":
      return await repo.themeSettings(workspaceId);
    case "platform.settings.cloudflare.connections":
      return await repo.listCloudflareConnections(workspaceId);
    case "platform.settings.security.bootstrap": {
      const bootstrap = await deps.authInternalJson<{
        summary: { policy: Record<string, unknown> };
      }>(
    ...
    case "platform.settings.users.list": {
      const users = await deps.authAdminJson<{ users: unknown[] }>(
        "/internal/auth/users",
      );
      return users.users;
    }
    ...
    case "platform.settings.sessions.list": {
      const response = await deps.authAdminJson<{ sessions: unknown[] }>(
        "/admin/auth/sessions",
      );
      return response.sessions;
    }
    case "platform.settings.audit.events":
      return await repo.auditEvents(workspaceId, { includeGlobal: true });
    ...
    case "platform.settings.interface.navigation":
      return await repo.interfaceContributions(workspaceId);
    case "platform.settings.interface.zones": {
      const surfaces = await repo.workspaceUiSurfaces(workspaceId);
      const counts = new Map<string, number>();
      for (const surface of surfaces)
        counts.set(surface.zone, (counts.get(surface.zone) ?? 0) + 1);
      return [...counts.entries()]
        .sort(([left], [right]) => left.localeCompare(right))
        .map(([zoneId, surfaceCount]) => ({ zoneId, surfaceCount }));
    }
    ...
    case "platform.settings.approvals.list":
      return await new ApprovalRequestRepository(_c.env.CORE_DB).listPending(
        workspaceId,
      );
    default:
      return null;
  ...
  try {
    switch (action.commandId) {
      case "platform.settings.general.save": {
        const settings = await repo.saveGeneralSettings(
          workspaceId,
          payload,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.cloudflare.connections.upsert": {
        const label = firstString(payload, "label", "name");
        if (!label) return denied(400, "label is required.");
        const connection = await repo.upsertCloudflareConnection(
          workspaceId,
          {
            id: firstString(payload, "id", "connectionId"),
            label,
            accountId: firstString(payload, "accountId"),
            configurationRef: firstString(payload, "configurationRef"),
            tokenHint: firstString(payload, "tokenHint"),
            status: toCloudflareConnectionStatus(payload.status),
            defaultForProvisioning:
              firstBoolean(payload, "defaultForProvisioning") ?? false,
          },
          actorId ?? undefined,
        );
        return c.json(operationEnvelope("ok", connection));
      }
      case "platform.settings.cloudflare.connections.delete": {
        const connectionId = firstString(payload, "connectionId", "id");
        if (!connectionId) return denied(400, "connectionId is required.");
        const deleted = await repo.deleteCloudflareConnection(
          workspaceId,
          connectionId,
          actorId ?? undefined,
        );
        return c.json(operationEnvelope("ok", { connectionId, deleted }));
      }
      case "platform.settings.cloudflare.connections.test": {
        const connectionId = firstString(payload, "connectionId", "id");
        if (!connectionId) return denied(400, "connectionId is required.");
        const result = await repo.testCloudflareConnection(
          workspaceId,
          connectionId,
          actorId ?? undefined,
        );
        if (!result.ok)
          return denied(409, result.error ?? "Cloudflare test failed.");
        return c.json(operationEnvelope("ok", result.connection));
      }
      case "platform.settings.cloudflare.connections.syncZones": {
        const connectionId = firstString(payload, "connectionId", "id");
        if (!connectionId) return denied(400, "connectionId is required.");
        const result = await repo.syncCloudflareConnectionZones(
          workspaceId,
          connectionId,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.security.policy.save": {
        const body = {
          workspaceId,
          registrationMode:
            firstString(payload, "registrationMode") ?? "disabled",
          requireEmailVerification:
            firstBoolean(payload, "requireEmailVerification") ?? false,
          allowPasskeyRegistration:
            firstBoolean(payload, "allowPasskeyRegistration") ?? false,
          allowPasskeySignin:
            firstBoolean(payload, "allowPasskeySignin") ?? false,
        ...
        const response = await deps.authAdminJson<{ policy: unknown }>(
          "/admin/auth/policy",
          { method: "PUT", body: JSON.stringify(body) },
        );
        await repo.audit(
          workspaceId,
          "platform.settings.security.policy.save",
          {
            workspaceId,
      ...
      case "platform.settings.audit.policy.save": {
        if (!isSuperadmin)
          return denied(
            403,
            "Platform admin access is required to update audit policy.",
          );
        ...
        const saved = await repo.saveWorkspaceAuditPolicy(
          workspaceId,
          policy,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.users.delete": {
        const target = firstString(
          ...
          : firstString(payload, "email", "targetEmail")
            ? await repo.authUserByEmail(
                firstString(payload, "email", "targetEmail")!,
              )
            : null;
        ...
        if (target.isPlatformAdmin)
          return denied(
            403,
            "Protected Superadmin accounts cannot be modified.",
          );
        const response =
          action.commandId === "platform.settings.users.disable"
        ...
        return c.json(
          operationEnvelope("ok", { userId: target.id, email: target.email }),
        );
      }
      case "platform.settings.workspaces.upsert": {
        ...
        const existingWorkspace = targetWorkspaceId
          ? await repo.workspace(targetWorkspaceId)
          : null;
        const workspaceInput: {
          ...
          name,
          status:
            toWorkspaceStatus(payload.status) ??
      ...
      case "platform.settings.rbac.member.role.assign": {
        if (
          ...
          return denied(
            403,
            "Workspace membership management permission is required.",
          );
        ...
        if (!target) return denied(404, "User is not available.");
        const updated = await repo.assignWorkspaceMemberRoles(
          workspaceId,
          target.id,
          roleIds,
          actorId ?? undefined,
      ...
      case "platform.settings.rbac.member.role.remove": {
        ...
        const target = await repo.authUserById(targetUserId);
        if (!target) return denied(404, "User is not available.");
        const updated = await repo.removeWorkspaceMemberRoles(
          workspaceId,
          target.id,
          roleIds,
          actorId ?? undefined,
      ...
      case "platform.settings.rbac.member.remove": {
        ...
        const target = await repo.authUserById(targetUserId);
        if (!target) return denied(404, "User is not available.");
        const updated = await repo.removeWorkspaceMember(
          workspaceId,
          target.id,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.rbac.member.override.deny": {
        ...
        if (!targetUserId || !permission)
          return denied(400, "userId and permission are required.");
        const target = await repo.authUserById(targetUserId);
        if (!target) return denied(404, "User is not available.");
        if (target.isPlatformAdmin)
          return denied(
            403,
            "Protected Superadmin accounts cannot receive workspace overrides.",
          );
        if (
          action.commandId === "platform.settings.rbac.member.override.deny" &&
          (await repo.workspaceOwnerCount(workspaceId)) <= 1
        )
          return denied(
            409,
            "The last active Owner cannot be denied a permission.",
          );
        const updated = await repo.setMemberPermissionOverride(
          workspaceId,
          target.id,
          permission as WorkspacePermission,
          action.commandId.endsWith("allow") ? "allow" : "deny",
          actorId ?? undefined,
        );
        return c.json(operationEnvelope("ok", updated));
      }
      case "platform.settings.rbac.member.override.remove": {
        ...
        const target = await repo.authUserById(targetUserId);
        if (!target) return denied(404, "User is not available.");
        const updated = await repo.removeMemberPermissionOverride(
          workspaceId,
          target.id,
          permission as WorkspacePermission,
          actorId ?? undefined,
      ...
      case "platform.settings.rbac.role.create": {
        ...
        if (!name) return denied(400, "name is required.");
        const role = await repo.createWorkspaceRole(
          workspaceId,
          { name, description: firstString(payload, "description") },
          actorId ?? undefined,
        );
      ...
      case "platform.settings.rbac.role.update": {
        ...
        const role = await repo.updateWorkspaceRole(
          workspaceId,
          roleId,
          roleInput,
          actorId ?? undefined,
      ...
      case "platform.settings.rbac.role.delete": {
        const roleId = firstString(payload, "roleId", "id");
        if (!roleId) return denied(400, "roleId is required.");
        const deleted = await repo.deleteWorkspaceRole(
          workspaceId,
          roleId,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.plans.upsert": {
        ...
        try {
          limits =
            typeof limitsRaw === "string"
              ? (JSON.parse(limitsRaw) as Record<string, unknown>)
        ...
        const plan = await repo.upsertPlan(
          { id: planId, name, status: planStatus, limits },
          actorId ?? undefined,
        );
      ...
      case "platform.settings.plans.assignment.upsert": {
        ...
        if (!userId || !planId)
          return denied(400, "userId and planId are required.");
        const target = await repo.authUserById(userId);
        if (!target) return denied(404, "User is not available.");
        const assignment = await repo.upsertUserPlanAssignment(
          {
            ...(firstString(payload, "id", "assignmentId")
              ? { id: firstString(payload, "id", "assignmentId")! }
              : {}),
            userId,
            planId,
            ...
            payload.status === "disabled"
              ? payload.status
              : "active") as "active" | "scheduled" | "expired" | "disabled",
      ...
      case "platform.settings.invites.create": {
        ...
        const invite = await repo.createWorkspaceInvitation(
          workspaceId,
          {
            email,
            roleId: firstString(payload, "roleId"),
            expiresAt: firstString(payload, "expiresAt"),
          },
          actorId ?? undefined,
        );
      ...
      case "platform.settings.invites.delete": {
        ...
        await repo.deleteWorkspaceInvitation(
          workspaceId,
          inviteId,
          actorId ?? undefined,
        );
        return c.json(
          operationEnvelope("ok", { invitationId: inviteId, deleted: true }),
        );
      }
      case "platform.settings.invites.revoke": {
        const inviteId = firstString(payload, "id", "invitationId");
        if (!inviteId) return denied(400, "invitationId is required.");
        const invite = await repo.revokeWorkspaceInvitation(
          workspaceId,
          inviteId,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.marketplace.demo.remove": {
        ...
        const operation = action.commandId.endsWith("install")
          ? manifest.api.operations.find(
              (item) =>
                ...
                (/demo/i.test(item.id) && /install|seed/i.test(item.id)),
            )
          : manifest.api.operations.find(
              (item) =>
                item.id ===
                  firstString(
                    payload,
                    "demoRemoveOperationId",
                    "operationId",
                  ) ||
                (/demo/i.test(item.id) &&
                  /remove|cleanup|purge/i.test(item.id)),
            );
        if (!operation)
          return c.json(
            operationEnvelope(
              "unavailable",
              null,
              action.commandId.endsWith("install")
                ? "Plugin does not expose a demo install operation."
                : "Plugin does not expose a demo cleanup operation.",
            ),
            501,
          );
        ...
        if (
          permissions.length &&
          !isSuperadmin &&
          !(await repo.hasAllPermissions(workspaceId, actor, permissions))
        )
          return denied(403, "Plugin operation permission is required.");
        const result = await dispatchPluginOperation(
          c,
          repo,
          workspaceId,
          pluginId,
        ...
        if (!result.ok)
          return c.json(
            operationEnvelope("unavailable", null, result.error),
            result.status,
          );
      ...
      case "platform.settings.approvals.deny": {
        ...
        const decision =
          action.commandId === "platform.settings.approvals.approve"
            ? "approved"
            : "denied";
        const approval = await new ApprovalRequestRepository(
          c.env.CORE_DB,
        ).decide(workspaceId, approvalId, decision, actorId ?? undefined);
      ...
      case "platform.settings.mail.provider.activate": {
        ...
        const provider = await repo.activateMailProvider(
          workspaceId,
          providerId,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.mail.provider.disable": {
        const providerId = firstString(payload, "id", "providerId");
        if (!providerId) return denied(400, "providerId is required.");
        const provider = await repo.disableMailProvider(
          workspaceId,
          providerId,
          actorId ?? undefined,
        );
      ...
      case "platform.settings.mail.provider.test": {
        ...
        if (!providerId || !to)
          return denied(400, "providerId and to are required.");
        const result = await repo.testMailProvider(
          workspaceId,
          providerId,
          to,
          actorId ?? undefined,
      ...
      case "platform.settings.interface.nav.hide": {
        ...
        if (section === "user" || section === "administration")
          changes.section = section;
        if (typeof payload.displayOrder === "number")
          changes.displayOrder = payload.displayOrder;
        const visibleInNavigation = firstBoolean(
          payload,
          "visibleInNavigation",
        );
        if (action.commandId.endsWith("hide"))
          changes.visibleInNavigation = false;
        else if (visibleInNavigation !== undefined)
          changes.visibleInNavigation = visibleInNavigation;
        const enabled = firstBoolean(payload, "enabled");
        if (enabled !== undefined) changes.enabled = enabled;
        const updated = await repo.updatePluginUiContribution(
          workspaceId,
          contributionId,
          changes,
          actorId ?? undefined,
        ...
        if (!updated)
          return denied(404, "Interface contribution is not available.");
        return c.json(operationEnvelope("ok", updated));
      }
      ...
      case "platform.settings.users.edit": {
        ...
        const target = targetUserId
          ? await repo.authUserById(targetUserId)
          : targetEmail
            ? await repo.authUserByEmail(targetEmail)
            : null;
        if (!target) return denied(404, "User is not available.");
        await repo.audit(
          workspaceId,
      ...
      case "platform.settings.rbac.member.impersonate": {
        ...
        if (!targetUserId || !reason)
          return denied(400, "userId and reason are required.");
        const target = await repo.authUserById(targetUserId);
        if (!target) return denied(404, "User is not available.");
        if (target.isPlatformAdmin)
          return denied(
        ...
        const response = await deps.authForwardResponse(
        ...
        await repo.audit(
          workspaceId,
          action.commandId,
          { targetUserId: target.id, targetEmail: target.email, reason },
          actorId ?? undefined,
        );
        return c.json(operationEnvelope("ok", body));
      }
      case "platform.settings.theme.preview": {
        return c.json(operationEnvelope("ok", { preview: payload }));
      }
      case "platform.settings.theme.save": {
        const settings = await repo.saveThemeSettings(
          workspaceId,
          payload,
          actorId ?? undefined,
        );
        return c.json(operationEnvelope("ok", { saved: true, settings }));
      }
      default:
        return null;
    }
    ...
    if (/not available|not found|could not be found/i.test(message))
      return denied(404, message);
    if (/protected/i.test(message)) return denied(403, message);
    if (
      /limit|last active Owner|cannot|required|not allowed|forbidden/i.test(
        message,
      )
    )
      return denied(409, message);
