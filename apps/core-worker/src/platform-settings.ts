...
const roles = ui.crud({
  ...
  description: "Reusable access profiles assigned to workspace users. System roles are protected.",
  ...
  fields: [
    ...
    field({ id: "permissions", label: "Permissions (comma-separated)", type: "textarea" }),
  ],
  rowActions: [
    ui.rowAction("platform.settings.rbac.role.clone", "Clone", { access: "permission-gated", requiredPermission: "workspace.members.manage", confirmation: { title: "Clone role", fields: [field({ id: "name", label: "New role name", type: "text", required: true })] } }),
  ],
});

const members = ui.crud({
  id: "security.members",
  title: "Workspace users",
  description: "Create invited members, assign roles, disable access and start audited impersonation for support.",
  dataSourceId: "platform.settings.rbac.members",
  entity: { singular: "User", plural: "Users", rowId: "userId", titleField: "user" },
  operations: { create: "platform.settings.rbac.member.invite", update: "platform.settings.rbac.member.update", delete: "platform.settings.rbac.member.delete" },
  columns: [
  ...
  fields: [
    field({ id: "user", label: "Email", type: "email", required: true }),
    field({ id: "status", label: "Status", type: "select", required: true, options: [{ value: "invited", label: "Invited" }, { value: "active", label: "Active" }, { value: "disabled", label: "Disabled" }] }),
    field({ id: "roleIds", label: "Role IDs (comma-separated)", type: "text" }),
  ],
  rowActions: [
...
const permissions = ui.table({
  ...
  description: "Effective permission keys assigned by roles and user-specific overrides.",
  dataSourceId: "platform.settings.rbac.permissions",
  columns: [
    column({ id: "permission", label: "Permission", field: "permission" }),
...
export function platformSettingsTabs() {
  ...
  const mailSections: SettingsSection[] = [ui.form({ id: "mail.provider", title: "Mail provider", description: "Transactional provider configuration and verification.", dataSourceId: "platform.settings.mail.summary", fields: [field({ id: "kind", label: "Provider kind", type: "select", required: true, options: [{ value: "transactional-http", label: "Transactional HTTP" }, { value: "smtp", label: "SMTP" }, { value: "mock-development-only", label: "Mock development only" }] }), field({ id: "label", label: "Label", type: "text", required: true }), field({ id: "fromName", label: "From name", type: "text", required: true }), field({ id: "fromEmail", label: "From email", type: "email", required: true }), field({ id: "replyToEmail", label: "Reply-to email", type: "email" }), field({ id: "configurationRef", label: "Secret reference", type: "text" })], actions: [ui.submit("platform.settings.mail.provider.save", "mail.configure", "Save provider")] }), ui.table({ id: "mail.providers", title: "Providers", dataSourceId: "platform.settings.mail.providers", columns: [column({ id: "label", label: "Label", field: "label" }), column({ id: "kind", label: "Kind", field: "kind" }), column({ id: "status", label: "Status", field: "status", type: "badge" }), column({ id: "fromEmail", label: "From email", field: "fromEmail" })], rowActions: [ui.rowAction("platform.settings.mail.provider.activate", "Activate", { variant: "primary", access: "permission-gated", requiredPermission: "mail.configure" }), ui.rowAction("platform.settings.mail.provider.disable", "Disable", { variant: "danger", access: "permission-gated", requiredPermission: "mail.configure" })] }), ui.table({ id: "mail.templates", title: "Templates", dataSourceId: "platform.settings.mail.templates", columns: [column({ id: "templateKey", label: "Template", field: "templateKey" }), column({ id: "status", label: "Status", field: "status", type: "badge" }), column({ id: "locale", label: "Locale", field: "locale" }), column({ id: "subjectTemplate", label: "Subject", field: "subjectTemplate" })] }), ui.table({ id: "mail.events", title: "Delivery events", dataSourceId: "platform.settings.mail.events", columns: [column({ id: "purpose", label: "Purpose", field: "purpose" }), column({ id: "status", label: "Status", field: "status", type: "badge" }), column({ id: "templateKey", label: "Template", field: "templateKey" }), column({ id: "errorSafe", label: "Result", field: "errorSafe" })] })];
  const mail = ui.panel({ id: "platform.settings.mail", label: "Email", icon: "mail", order: 50, permission: "mail.read", sections: mailSections });
  const appearance = ui.panel({ id: "platform.settings.interface", label: "Appearance & Navigation", icon: "layout", order: 60, permission: "interface.read", sections: [ui.table({ id: "interface.builder", title: "Interface builder", description: "Navigation, layout, manual pages and UI contributions.", dataSourceId: "platform.settings.interface.summary", actions: [ui.submit("platform.settings.interface.save", "interface.write", "Save interface")] })] });
