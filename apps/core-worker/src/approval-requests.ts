import { approvalRequestSchema, type ApprovalRequest, type ApprovalRequestKind } from "@v2/rpc-contracts";
...
export class ApprovalRequestRepository {
  ...
  async create(input: ApprovalRequestCreate): Promise<ApprovalRequest> {
    ...
    const row = await this.db.prepare(`INSERT INTO approval_requests
      ...
      .bind(id, input.workspaceId, input.kind, input.subjectId, input.pluginId ?? null, input.risk, JSON.stringify(input.payload), input.requestedBy ?? null, input.expiresAt ?? null)
  ...
  async listPending(workspaceId: string): Promise<ApprovalRequest[]> {
    const rows = await this.db.prepare(`SELECT id, workspace_id, kind, subject_id, plugin_id, risk, payload_json, status, requested_by, decided_by, requested_at, decided_at, expires_at, consumed_at, reason
  ...
  async decide(workspaceId: string, approvalId: string, decision: "approved" | "denied", actorId?: string, reason?: string): Promise<ApprovalRequest | undefined> {
  ...
  async claimApproved(input: { workspaceId: string; approvalId: string; kind: ApprovalRequestKind; subjectId?: string; pluginId?: string }): Promise<ApprovalRequest | undefined> {
    const row = await this.db.prepare(`UPDATE approval_requests
      ...
      RETURNING id, workspace_id, kind, subject_id, plugin_id, risk, payload_json, status, requested_by, decided_by, requested_at, decided_at, expires_at, consumed_at, reason`)
      .bind(input.approvalId, input.workspaceId, input.kind, input.subjectId ?? null, input.subjectId ?? null, input.pluginId ?? null, input.pluginId ?? null)
      .first<ApprovalRequestRow>();
    return row ? this.mapped(row) : undefined;
  }

  async get(workspaceId: string, approvalId: string): Promise<ApprovalRequest | undefined> {
    const row = await this.db.prepare(`SELECT id, workspace_id, kind, subject_id, plugin_id, risk, payload_json, status, requested_by, decided_by, requested_at, decided_at, expires_at, consumed_at, reason
      FROM approval_requests
