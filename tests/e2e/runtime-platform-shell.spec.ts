...
function observeFailures(page, failures: string[]) {
  ...
  page.on("response", (response) => {
    ...
    if (
      [400, 401, 403, 409, 500, 503].includes(status) &&
      (url.startsWith(coreUrl) || url.startsWith(authUrl))
    )
...
test.describe("runtime platform shell", () => {
  ...
  test("owner setup and runtime-declared settings render without browser failures", async ({
    page,
  }) => {
    const failures: string[] = [];
    observeFailures(page, failures);

    const anonymousBootstrapResponse = page.waitForResponse(
      (response) => new URL(response.url()).pathname === "/bootstrap",
    );
    await page.goto("/");
    await expect(page.getByRole("heading", { name: /sign in/i })).toBeVisible();
    expect(await (await anonymousBootstrapResponse).json()).toEqual({
      authenticated: false,
    });
    ...
    const ownerForm = page
      .locator("form")
      .filter({ hasText: "Authorized email" });
    ...
    await page.goto(
      `/settings?workspace=${encodeURIComponent(workspaceId)}&tab=platform.settings.general`,
    );
