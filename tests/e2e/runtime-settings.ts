...
export async function verifyRuntimeDeclaredSettings(
  page: Page,
  coreUrl: string,
  workspaceId: string,
) {
  const tabsResponse = await page.request.get(
    `${coreUrl}/workspaces/${encodeURIComponent(workspaceId)}/settings/tabs`,
  );
  ...
  for (const tab of payload.tabs) {
    const panelResponse = await page.request.get(
      `${coreUrl}/workspaces/${encodeURIComponent(workspaceId)}/settings/tabs/${encodeURIComponent(tab.id)}`,
    );
    expect(panelResponse.ok(), await panelResponse.text()).toBeTruthy();
    const resolution =
      (await panelResponse.json()) as RuntimeSettingsResolution;
    expect(resolution.tab.id).toBe(tab.id);
    expect(resolution.panel.id).toBe(tab.panelContributionId);

    await page.goto(
      `/settings?workspace=${encodeURIComponent(workspaceId)}&tab=${encodeURIComponent(tab.id)}`,
    );
    await expect(
      page.getByRole("heading", { name: tab.label }).first(),
    ).toBeVisible();
    await expect(
      page.locator(".settings-tabs").getByRole("button", { name: tab.label }),
    ).toBeVisible();
