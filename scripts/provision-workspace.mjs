...
const provisioningSecret = value(
  "--provisioning-secret",
  process.env.CORE_PROVISIONING_SECRET || "",
);
...
if (!local && coreUrl) {
  const response = await fetch(
    new URL("/internal/provision/workspace", coreUrl),
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
        ...(provisioningSecret
          ? { "x-v2-provisioning-secret": provisioningSecret }
          : {}),
      },
      body: JSON.stringify({
        workspaceId,
        ...
        breakGlassPrintToken: printToken,
      }),
    },
  );
  ...
  if (printToken && payload.setupUrl)
    console.log(`Break-glass one-time setup URL: ${payload.setupUrl}`);
  else
    console.log(
      "One-time setup URL suppressed. Core Mail Runtime sent owner_setup.",
    );
  process.exit(0);
}
...
if (!local && !coreUrl) {
  console.error(
    "Production provisioning must call Core. Set --core-url or CORE_PROVISION_URL. Use --local only for development D1 bootstrap.",
  );
  process.exit(1);
}
...
VALUES (${sqlString(requestId)}, ${sqlString(workspaceId)}, ${sqlString(ownerEmail)}, 'pending', ${sqlString(tokenHash)}, ${sqlString(expiresAt)}, 'provision:workspace:prod', ${sqlString(JSON.stringify({ adminDomain, authDomain }))});`,
  ...
  adminDomain
    ? `INSERT OR IGNORE INTO workspace_domains (id, workspace_id, hostname, kind, status, verification_method, is_primary) VALUES (${sqlString(crypto.randomUUID())}, ${sqlString(workspaceId)}, ${sqlString(adminDomain.toLowerCase())}, 'admin', 'verifying', 'manual', 1);`
    : "",
  authDomain
    ? `INSERT OR IGNORE INTO workspace_domains (id, workspace_id, hostname, kind, status, verification_method, is_primary) VALUES (${sqlString(crypto.randomUUID())}, ${sqlString(workspaceId)}, ${sqlString(authDomain.toLowerCase())}, 'auth', 'verifying', 'manual', 1);`
    : "",
...
if (printToken) {
  ...
  console.log(
    "One-time setup URL suppressed. Send owner_setup through Core Mail Runtime or rerun with --break-glass-print-token for explicit recovery.",
  );
}
