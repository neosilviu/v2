...
if (occupied.length > 0) {
  console.error("Dev ports are already in use. Run `pnpm dev:stop` and try again:");
  for (const { endpoint, pid } of occupied) {
    console.error(`- ${endpoint.name} ${endpoint.kind} port ${endpoint.port} is used by pid ${pid}`);
  }
  process.exit(1);
}
