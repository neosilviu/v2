...
import { codeExtensions, readJson, relativePath, walk, workspaceDirectories } from "./common.mjs";

const platformApps = new Set(["apps/core-worker", "apps/auth-worker", "apps/runtime-bridge", "apps/web"]);
const importPattern = /\b(?:import|export)\b(?:[^"'`]*?\bfrom\s*)?["']([^"']+)["']|import\(\s*["']([^"']+)["']\s*\)/g;
...
export function readPackages(root) {
  return workspaceDirectories(root).flatMap((directory) => {
    ...
    return [{ directory: relativePath(root, directory), name: manifest.name ?? relativePath(root, directory), dependencies, scripts: manifest.scripts ?? {} }];
  });
}

export function analyzeBoundaries(root, packages) {
  const directories = new Map(packages.map((item) => [item.name, item.directory]));
  const featurePluginPackages = new Set(packages.filter((item) => item.directory.startsWith("plugins/")).map((item) => item.name));
  ...
  for (const app of platformApps) {
    ...
    for (const file of files) {
      ...
      source.split(/\r?\n/).forEach((line, lineNumber) => {
        const imports = [...line.matchAll(importPattern)].map((match) => match[1] ?? match[2]).filter(Boolean);
