...
const parseSizeString = (value) => {
  const match = String(value || "")
    .trim()
    .match(/^([\d.]+)\s*(B|KiB|MiB|GiB|KB|MB|GB)$/i);
  if (!match) return 0;
...
const measureWorkerSize = (root, config) => {
  ...
  const match = combined.match(
    /Total Upload:\s*([0-9.]+\s*(?:B|KiB|MiB|GiB|KB|MB|GB))\s*\/\s*gzip:\s*([0-9.]+\s*(?:B|KiB|MiB|GiB|KB|MB|GB))/i,
  );
  const firstErrorLine = combined
    ...
    .find(
      (line) =>
        ...
        line.includes("Failed to fetch auth token") ||
        line.includes("Error"),
    );
  if (!match) {
    return {
      ...
      gzipBytes: 0,
      ok: false,
      note: firstErrorLine || "wrangler dry-run did not return size line",
    };
  }
  return {
