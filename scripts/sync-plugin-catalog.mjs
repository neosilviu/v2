...
const catalog = [
  {
    module: "../plugins/ai-providers/manifest.ts",
    exportName: "aiProvidersPlugin",
    category: "ai",
    demoAvailable: false
  },
  {
    module: "../plugins/agent-ai/manifest.ts",
    exportName: "agentAiManifest",
    category: "ai",
    demoAvailable: false
  },
  {
    module: "../plugins/theme-studio/manifest.ts",
    exportName: "themeStudioPlugin",
    category: "design",
    demoAvailable: false
  },
  {
    module: "../plugins/local-node/manifest.ts",
    exportName: "localNodeManifest",
    category: "operations",
    demoAvailable: false
  },
  {
    module: "../plugins/website-studio/manifest.ts",
    exportName: "websiteStudioManifest",
    category: "site",
    demoAvailable: true
  },
  {
    module: "../plugins/commerce/manifest.ts",
    exportName: "commerceManifest",
    category: "commerce",
    demoAvailable: false
}
];
...
function run(args) {
  const result = spawnSync(
    "pnpm",
    ["--dir", coreDir, "exec", "wrangler", ...args],
    { stdio: "inherit" },
  );
...
const database = config.d1_databases?.find(
  (item) => item.binding === "CORE_DB",
)?.database_name;
if (!database)
  throw new Error(
    "CORE_DB database_name is missing from apps/core-worker/wrangler.jsonc",
  );
...
const rows = await Promise.all(
  catalog.map(async (entry) => {
    ...
    if (!manifest?.id)
      throw new Error(`Missing ${entry.exportName} in ${entry.module}`);
...
const statements = rows.flatMap(
  ({ manifest, category, demoAvailable, sha256: digest, objectKey, zip }) => [
...
try {
  ...
  for (const row of rows) {
    const zipFile = path.join(
      os.tmpdir(),
      `v2-${row.manifest.id}-${process.pid}.zip`,
    );
    ...
    try {
      run([
        ...
        "object",
        "put",
        `${config.r2_buckets?.find((item) => item.binding === "PLUGIN_PACKAGES")?.bucket_name ?? "v2-plugin-packages"}/${row.objectKey}`,
        "--file",
        zipFile,
        "--local",
        "--content-type",
  ...
  console.log(
    `Seeded ${rows.length} Marketplace catalog identities and published local ZIP releases into ${database}. This is a dev helper only; production release publishing stays runtime/API driven.`,
  );
} finally {
