...
const skipMarketplaceSync =
  args.has("--skip-marketplace-sync") ||
  process.env.V2_DEV_SKIP_MARKETPLACE_SYNC === "1";
const skipDemoSeed =
  args.has("--skip-demo-seed") || process.env.V2_DEV_SKIP_DEMO_SEED === "1";
...
function coreSql(sql) {
  return run("core local SQL", "pnpm", [
    "--dir",
    "apps/core-worker",
    "exec",
    "wrangler",
...
async function waitFor(name, base, path) {
  ...
  throw new Error(
    `${name} is not reachable at ${base}${path}. Start pnpm dev before pnpm bootstrap:workspace:dev.`,
  );
}
...
function provisionWorkspace() {
  ...
  if (!match?.[1])
    throw new Error(
      `workspace provisioning did not print a setup token\n${output}`,
    );
...
async function ensureOwner() {
  ...
  if (
    !signup.response.ok &&
    signup.body?.error?.code !== "owner_account_already_exists"
  ) {
    throw new Error(
      `owner setup signup failed: HTTP ${signup.response.status} ${JSON.stringify(signup.body)}`,
    );
  }
  ...
  if (!signup.response.ok) {
    ...
    if (!login.response.ok)
      throw new Error(
        `owner account exists but local password cannot sign in: HTTP ${login.response.status} ${JSON.stringify(login.body)}`,
      );
    ...
    if (
      ...
      activation.body?.error?.code !== "owner_setup_token_consumed"
    ) {
      throw new Error(
        `existing owner setup activation failed: HTTP ${activation.response.status} ${JSON.stringify(activation.body)}`,
      );
    }
  }
  ...
  if (!login.response.ok)
    throw new Error(
      `login validation failed: HTTP ${login.response.status} ${JSON.stringify(login.body)}`,
    );
  const bootstrap = await request(
    coreUrl,
    `/workspaces/${encodeURIComponent(workspaceId)}/bootstrap`,
  ...
  if (!bootstrap.response.ok)
    throw new Error(
      `workspace bootstrap validation failed: HTTP ${bootstrap.response.status} ${JSON.stringify(bootstrap.body)}`,
    );
}
...
async function ensureOpenDevelopmentRegistration() {
  ...
  if (!result.response.ok)
    throw new Error(
      `development auth policy update failed: HTTP ${result.response.status} ${JSON.stringify(result.body)}`,
    );
}

async function ensureDemoAuthAccounts() {
  const userIds = new Map();
  for (const account of demoAccounts.filter(
    ...
    if (
      ...
      throw new Error(
        `demo account signup failed for ${account.email}: HTTP ${signup.response.status} ${JSON.stringify(signup.body)}`,
      );
    }
    const userId = signup.body?.user?.id ?? authUserIdByEmail(account.email);
    if (!userId)
      throw new Error(`could not resolve demo user id for ${account.email}`);
    userIds.set(account.key, userId);
  }
  const inactiveIds = new Map(
    demoAccounts
      .filter((item) => item.status !== "active")
      .map((account) => [account.key, `development-${account.key}`]),
  );
...
function seedDemoCoreData(userIds) {
  const plans = [
    ...
    },
    {
      ...
      name: "Trial Preview",
      status: "draft",
      limits: { seats: 1, projects: 1, trialDays: 14 },
    },
  ];
  const roleSpecs = [
    [
  ...
  for (const [key, label, permissions] of roleSpecs) {
    ...
    for (const permission of permissions) {
      sql.push(
        `INSERT OR IGNORE INTO workspace_role_permissions (workspace_id, role_id, permission) VALUES (${sqlString(workspaceId)}, ${sqlString(`${workspaceId}:${key}`)}, ${sqlString(permission)});`,
      );
    }
  }
  sql.push(
    `UPDATE workspace_member_roles SET role_id = ${sqlString(`${workspaceId}:editor`)} WHERE workspace_id = ${sqlString(workspaceId)} AND role_id = ${sqlString(`${workspaceId}:operator`)};`,
  );
  sql.push(
    `UPDATE workspace_invitations SET role_id = ${sqlString(`${workspaceId}:editor`)} WHERE workspace_id = ${sqlString(workspaceId)} AND role_id = ${sqlString(`${workspaceId}:operator`)};`,
  );
  sql.push(
    `DELETE FROM workspace_role_permissions WHERE workspace_id = ${sqlString(workspaceId)} AND role_id = ${sqlString(`${workspaceId}:operator`)};`,
  );
  sql.push(
    `DELETE FROM workspace_roles WHERE workspace_id = ${sqlString(workspaceId)} AND id = ${sqlString(`${workspaceId}:operator`)};`,
  );
  sql.push(
  ...
  for (const account of demoAccounts) {
    ...
    if (account.workspaceRole) {
      ...
      sql.push(
        `INSERT OR IGNORE INTO workspace_member_roles (workspace_id, user_id, role_id) VALUES (${sqlString(workspaceId)}, ${sqlString(userId)}, ${sqlString(`${workspaceId}:${account.workspaceRole}`)});`,
      );
    }
    if (account.plan) {
      sql.push(
  ...
  if (viewerId)
    sql.push(
      `INSERT INTO workspace_member_permission_overrides (workspace_id, user_id, permission, effect, created_at, updated_at) VALUES (${sqlString(workspaceId)}, ${sqlString(viewerId)}, 'publication.publish', 'deny', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT(workspace_id, user_id, permission) DO UPDATE SET effect = excluded.effect, updated_at = CURRENT_TIMESTAMP;`,
    );
  coreSql(sql.join("\n"));
}
...
async function main() {
  ...
  console.log(
    "Seeded: owner, superadmin, admin, editor, viewer, invited, disabled; roles, permission overrides, plans and assignments.",
  );
